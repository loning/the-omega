\begin{conclusion}[谱四次曲线的椭圆化与极小模型的同构识别]\label{con:xi-terminal-zm-elliptic-normalization}
...
\]
因此令 \(Y:=y-\lambda^2+\tfrac12\)（亦即 \(Y=w/2\)）可得到函数域同一
\[
\QQ(\mathcal C)\cong \QQ(E),\qquad \lambda=X,\quad Y=y-\lambda^2+\tfrac12,
\]
并在仿射光滑开集上给出完全显式的双有理互逆
\[
...
\begin{enumerate}
  \item 由 \(y=0\iff w=1-2\lambda^{2}\) 得
  \[
  ...
  \]
  \item 由 \(y=1\iff w=3-2\lambda^{2}\) 得
  \[
  ...
  \]
\end{enumerate}
由主除子在 \(\mathrm{Pic}^0(E)\cong E\) 中对应单位元，亦可等价写成两条群律关系
\[
...
\end{conclusion}
\begin{proof}[证明要点]
将 \(y=(2\lambda^2+w-1)/2\) 代入 \(\Pi(\lambda,y)\) 并化简可得恒等式
\[
...
\begin{conclusion}[次数 \(4\) 线性系 \texorpdfstring{$|4O|$}{|4O|} 的射影正规模型：两二次曲面的显式交]\label{con:xi-terminal-zm-elliptic-degree4-projective-model}
...
\)。
则 \(y\) 在 \(O\) 处具有四阶极点，从而 \(1,X,X^{2},y\in H^{0}\!\bigl(E,\mathcal O_{E}(4O)\bigr)\)；
并且它们构成 \(H^{0}(E,\mathcal O_{E}(4O))\) 的一组基。
由完全线性系 \(|4O|\) 诱导的态射
\[
\kappa:E\hookrightarrow \mathbb P^{3},\qquad
P\longmapsto [U_{0}:U_{1}:U_{2}:U_{3}]=[1:X(P):X(P)^{2}:y(P)]
\]
为射影嵌入，其像为次数 \(4\) 的属一射影正规曲线，并在 \(\mathbb P^{3}\) 中由两条二次方程刻画：
\[
U_{1}^{2}-U_{0}U_{2}=0,
\qquad
(U_{2}-U_{3})^{2}-U_{0}(U_{2}-U_{3})-U_{1}U_{2}+U_{1}U_{0}=0.
\]
在该模型中，投影 \([U_{0}:U_{3}]\) 与有理函数 \(y\) 所定义的次数 \(4\) 覆叠 \(E\to\mathbb P^{1}_{y}\) 一致。
\end{conclusion}
\begin{proof}[证明要点]
由结论 \ref{con:xi-terminal-zm-elliptic-y-hurwitz-passport}，\(y\) 在 \(O\) 处仅有一处四阶极点，故
\(
y\in H^{0}(E,\mathcal O_{E}(4O))
\)。
又由于 \(\mathrm{ord}_{O}(X)=-2\)、\(\mathrm{ord}_{O}(Y)=-3\)，可知 \(1,X,X^{2}\in H^{0}(E,\mathcal O_{E}(4O))\)，且
\(
y-X^{2}=Y-\tfrac12
\)
在 \(O\) 处极阶为 \(3\)。
若 \(a_{0}+a_{1}X+a_{2}X^{2}+a_{3}y=0\)，比较 \(O\) 处的极阶先得 \(a_{3}=-a_{2}\)，再由 \(y-X^{2}=Y-\tfrac12\) 的三阶极点推出 \(a_{2}=a_{3}=0\)，继而 \(a_{1}=a_{0}=0\)，从而四者线性无关；由于 \(\dim H^{0}(E,\mathcal O_{E}(4O))=4\)，它们构成一组基。
当 \(\deg(4O)=4\ge 3\) 时 \(|4O|\) 为 very ample，故 \(\kappa\) 为嵌入；其像为椭圆正规曲线，因而为两二次曲面的完全交。
两条二次方程可由代数消元直接核验：在仿射图 \(U_{0}=1\) 上，第一式为恒等式 \(X^{2}=X^{2}\)；
第二式化为
\[
(X^{2}-y)^{2}-(X^{2}-y)-X^{3}+X=0,
\]
将 \(y=X^{2}+Y-\tfrac12\) 代入并用曲线方程 \(Y^{2}=X^{3}-X+\tfrac14\) 化简即得。
最后一条投影陈述由定义 \(U_{3}/U_{0}=y\) 直接推出。证毕。
\end{proof}
...
\begin{conclusion}[有理谱根的充要条件：\texorpdfstring{$\Pi(\lambda,y)$}{Pi(lambda,y)} 在 \texorpdfstring{$\mathbb Q$}{Q} 上有根当且仅当 \(y\in y\bigl(E(\QQ)\bigr)\)]\label{con:xi-terminal-zm-pi-rational-root-iff-y-image}
...
\end{enumerate}
并且在上述条件成立时，可取 \(\lambda=X(P)\) 作为对应的有理谱根。
特别地，此时 \(\Pi(\lambda,y_{0})\) 在 \(\QQ[\lambda]\) 中含有理线性因子 \(\lambda-X(P)\)；
...
4\Pi(\lambda,y)=w^{2}-(4\lambda^{3}-4\lambda+1)
\)，
从而同一方程在 \(y\)-方向上的判别式给出椭圆曲线模型 \(\mathcal E:w^{2}=4\lambda^{3}-4\lambda+1\)（见结论 \ref{con:xi-terminal-zm-elliptic-normalization}）。
...
据 \cite{LMFDBEllipticCurve37a1} 的记录（访问于 \(2026\text{-}02\text{-}17\)），在 Cremona/LMFDB 的标准标识下，\(E\) 为导子 \(37\) 的同源类 \(37\mathrm a\) 之强 Weil 曲线（记号 \(37\mathrm a1\)）；
...
a_p=p+1-\#E(\FF_p)
\)
与该新形式的 Hecke 本征值一致，从而 \(L(E,s)=L(f,s)\)（模性定理；亦可在 Langlands 口径下理解为与 \(\mathrm{GL}_2(\mathbb{A}_\QQ)\) 的相应自守表示对接）。\cite{LMFDBNewform372aa}
\par
因此对每个素数 \(\ell\)，椭圆曲线 \(E\) 的 \(\ell\)-进 Tate 模诱导二维伽罗瓦表示
\[
\rho_{E,\ell}:\mathrm{Gal}(\overline{\QQ}/\QQ)\longrightarrow \mathrm{GL}_2(\ZZ_\ell),
\]
其仅在 \(\{37,\ell\}\) 处可能分歧；并且对任意良素 \(p\neq 37,\ell\)，Frobenius 元的特征多项式为 \(T^2-a_pT+p\)，从而给出与新形式一致的局部 Euler 因子。
\par
...
\par
令 \(q=e^{2\pi i\tau}\)。
在上述外部锚定下，可取规范化新形式 \(f(\tau)=\sum_{n\ge 1}a_n q^n\) 使得
\(
f(q)\,\dd q/q
\)
等于 \(E_0\) 的 N\'eron 微分在模参数化下的拉回；
于是重量函数 \(y=\eta+x^2\in\QQ(E_0)\) 作为 \(X_0^+(37)\) 上的模函数在无穷远尖点具有主部 \(q^{-4}\)，其截断展开与对应新形式的 \(q\)-展开见下式：
...
\boxed{\;
y_{\mathrm{LY}}
=\frac{2\lambda_{\mathrm{LY}}^{2}+w_{\mathrm{LY}}-1}{2}\; }.
\]
