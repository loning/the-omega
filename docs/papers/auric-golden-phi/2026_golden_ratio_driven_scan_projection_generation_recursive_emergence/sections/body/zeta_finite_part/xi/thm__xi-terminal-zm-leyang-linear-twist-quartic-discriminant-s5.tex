...
\begin{remark}[公开网页层面的形式同一性检索]\label{rem:xi-terminal-zm-leyang-linear-twist-public-search-formal}
对 \(\Pi_t\) 的系数片段、判别式 \(D(t,y)\) 的首项结构以及参数函数 \(\tau\) 的显式表达式作公开网页检索时，返回结果主要为通用方程求解器页面与椭圆曲线数据库条目；
未见包含该一参数四次族、其判别式曲线与 \(E\) 的显式互逆双有理公式、以及由此导出的 \(S_5\) 单值结论的条目。
该检索仅用于排除“显式公式已在常见公开网页中出现”的情形，不能替代文献覆盖意义下的查新。
例如可参见 \url{https://www.wolframalpha.com/calculators/equation-solver-calculator}。
\end{remark}
