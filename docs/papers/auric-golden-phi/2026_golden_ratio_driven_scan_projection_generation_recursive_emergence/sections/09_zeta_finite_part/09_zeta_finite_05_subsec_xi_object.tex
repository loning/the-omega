...
\]
经典 Schur--Cohn 理论给出：若 \(p\) 在单位圆上无零点，则 \(p\) 的全部零点落在开单位圆盘 \(|y|<1\) 当且仅当 \(H(p)\succ 0\)；并且更一般地，\(H(p)\) 的惯性指数给出单位圆内/外零点计数（标量情形的综述见 \cite{DymYoung1990SchurCohnMatrixPolynomials}）。
\par
...
第二条为 CMV-like 伴随矩阵的标准结果：任意首一多项式都可被实现为某个 CMV-like Hessenberg/五对角伴随矩阵的特征多项式，并且该伴随矩阵可用 Schur（Verblunsky）参数显式参数化；在该参数化下，酉性等价于参数落在单位圆盘/单位圆的约束，从而等价于特征多项式根分布在单位圆上（参见 \cite{CanteroMoralVelazquez2003CMV,Simon2005OPUC1,BevilacquaDelCorsoGemignani2015CMVCompanion}）。证毕。
\end{proof}
...
\end{proposition}
\begin{proof}
对首一化实根多项式，存在实对称三对角（Jacobi）矩阵实现其为特征多项式的构造性结论是标准事实；并且可在不预先求根的前提下由多项式本身的欧几里得算法/连分式数据生成一份三对角行列式表示（见 \cite{Fiedler1990SymmetricCharpoly,Schmeisser1993TridiagonalGivenCharpoly}）。
三条等价性来自：\(\mathrm{Spec}(J_L)\) 恰为 \(\det(SI-J_L)\) 的根集，而 \(J_L\) 自伴使得 \(\mathrm{Spec}(J_L)\subset[-2,2]\iff \|J_L\|_{\mathrm{op}}\le 2\)。
\end{proof}
...
因此，若目标是逼近经典 \(\zeta\) 的零点计数增长阶（\(N_\zeta(T)\asymp T\log T\)），则必须至少引入一条硬升级机制：
\begin{enumerate}
