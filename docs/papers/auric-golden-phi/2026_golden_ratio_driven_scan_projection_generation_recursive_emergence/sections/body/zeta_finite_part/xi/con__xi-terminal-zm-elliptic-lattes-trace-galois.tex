...
\begin{conclusion}[Latt\`es 自映射的后临界肖像：临界值为 \texorpdfstring{$x(E[2]\setminus\{O\})$}{x(E[2]-{O})}，并在一步后坍缩至 \texorpdfstring{$\infty$}{infty}]\label{con:xi-terminal-zm-elliptic-lattes-pcf-portrait}
沿用结论 \ref{con:xi-terminal-zm-elliptic-lattes-doubling} 的记号。
则：
\begin{enumerate}
  \item \(\Phi\) 的有限临界点集合为
  \[
  ...
  \]
  其中 \(x\bigl(E[2]\setminus\{O\}\bigr)\) 恰为三次多项式 \(4X^{3}-4X+1\) 的三根。
  \item 并且
  \[
  ...
  \]
\end{enumerate}
因此 \(\Phi\) 为严格后临界有限（PCF）映射，其后临界肖像为
\[
\mathrm{Crit}(\Phi)\ \xrightarrow{\ \Phi\ }\ x(E[2]\setminus\{O\})\ \xrightarrow{\ \Phi\ }\ \{\infty\}\ \xrightarrow{\ \Phi\ }\ \{\infty\},
\]
并且全后临界集为 \(\{\infty\}\cup x\bigl(E[2]\setminus\{O\}\bigr)\)。
\end{conclusion}
\begin{proof}[证明要点]
第一点由结论 \ref{con:xi-terminal-zm-elliptic-lattes-doubling} 已证。
对任意 \(Q\in E[4]\setminus E[2]\)，有 \([2]Q\in E[2]\setminus\{O\}\)，从而
\(
\Phi(x(Q))=x([2]Q)\in x(E[2]\setminus\{O\})
\)，得到第二点。
第三点由 \(\Phi(X)=N(X)/D(X)\) 且 \(D(X)=4X^3-4X+1=4Y^2\) 推出：当 \(X\in x(E[2]\setminus\{O\})\) 时对应 \(Y=0\)，分母为零而分子非零，因此 \(\Phi(X)=\infty\)。
最后，由 \(\deg N=4\)、\(\deg D=3\) 得 \(X\to\infty\) 时 \(\Phi(X)\sim \frac14X\)，从而 \(\Phi(\infty)=\infty\)。证毕。
\end{proof}
...
\begin{enumerate}
  ...
  \]
  \item 记 \(\rho_4:\mathrm{Gal}(\overline\QQ/\QQ)\to \mathrm{Aut}(E[4])\cong GL_2(\ZZ/4\ZZ)\) 为 \(4\)-挠点的伽罗瓦表示。
  由于 \(C(X)=0\) 的六根刻画 \(x\bigl(E[4]\setminus E[2]\bigr)\) 的坐标像，其分裂域 \(L\) 等价承载 \(\rho_4\) 在商 \(E[4]/\{\pm 1\}\)（亦即去掉标量 \(\{\pm I\}\)）上的射影作用；
  因而该射影像阶为 \(48\)，并与上述 \(S_4\times C_2\) 的刻画一致。
\end{enumerate}
\end{conclusion}
\begin{proof}[证明要点]
