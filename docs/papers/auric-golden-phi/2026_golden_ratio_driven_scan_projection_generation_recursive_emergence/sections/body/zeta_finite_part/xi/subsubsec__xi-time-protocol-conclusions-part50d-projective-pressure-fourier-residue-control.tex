...
附录 \ref{app:pom-projective-pressure-operator} 的实参数射影转移算子 \(\mathcal L_q\) 与命题 \ref{prop:real-input-40-output-dirichlet-nonrh} 的 cyclotomic 扭曲谱之间，还可进一步抽取出下列五条彼此衔接的结构结论。前两条给出连续压力对离散 moment-kernel 数据的无条件凸控制；中间两条及其推论把命题 \ref{prop:real-input-40-output-dirichlet-nonrh} 与定理 \ref{thm:xi-dirichlet-mode-forces-congruence-residual} 中的上界链与下界链闭合为精确指数定理；最后一条则把前述凸性整理为离散压力数据的规范上包络表述。
...
上述结果表明：附录 E 提供的并不只是“把整数 \(q\) 推广为实参数”的形式构造，而是一条无条件凸压力曲线；附录 D.527 提供的也不只是 cyclotomic 扭曲的上界证书，而是经由精确 Fourier--Parseval 反演所闭合的真实 residue bias 指数。于是，连续 \(q\)-压力与模 \(m\) Dirichlet 扭曲不再是两条并列技术线，而构成了同一解析控制理论的两侧边界：一侧以凸性支配离散 moment 数据，另一侧以 twisted spectral radius 精确锁定 residue 偏差的指数尺度。
