...
1. **读论文定理和完整证明**
   ...
   - **必须找到并逐步提取论文的完整证明过程**（不能只看定理陈述）
   - 将证明翻译为 formalizer 可理解的步骤链（每步对应一个辅助引理或 tactic 操作）
   - 识别证明中依赖的前置定理/引理，确认它们是否已形式化
   - 标记论文证明中的跳跃或隐含步骤（formalizer 需要自己填充的部分）
   - 识别所有数学符号和定义的含义
...
### 论文证明过程（必填，逐步展开）
**从论文 .tex 文件中提取的完整证明，翻译为 formalizer 可理解的数学步骤链：**

1. [第一步：从什么出发，用了什么定义/引理]
2. [第二步：如何从第一步推到中间结果]
3. [第三步：如何得到最终结论]
