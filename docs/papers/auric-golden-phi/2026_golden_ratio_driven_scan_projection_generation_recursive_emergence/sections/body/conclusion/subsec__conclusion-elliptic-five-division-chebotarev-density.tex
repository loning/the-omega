...
\begin{theorem}[\texorpdfstring{$T_5$}{T5} 的线性因子数在模 \texorpdfstring{$5$}{5} 剩余类上的条件分布]\label{thm:conclusion-elliptic-t5-linear-factor-conditional-density-mod5}
...
再设 \(a=1\)。此时 \(G^{(1)}=\SL_2(\FF_5)\) 共有 \(120\) 个元素。恒等元固定全部 \(24\) 个非零向量，故对应 \(N_\ell=24\)，其条件密度为 \(1/120\)。若 \(g\neq I\) 且在 \(V^\times\) 上有非零不动点，则其特征多项式只能为 \((X-1)^2\)，故 \(g\) 是非平凡幺幂元，与
\[
...
120-1-24=95
\]
个 determinant 为 \(1\) 的矩阵无非零不动点，对应 \(N_\ell=0\)。遂得所述三项条件密度。证毕。
\end{proof}
...
\[
\sgn(g_\ell)=(-1)^{24-r_\ell}=(-1)^{r_\ell},
\]
因为 \(24\) 为偶数。
...
\qquad
\sigma(g):=\sgn(g\curvearrowright V^\times).
\]
这是一个群同态。先看幺幂元
\[
J=\begin{pmatrix}1&1\\0&1\end{pmatrix}.
...
\sigma(J)=1.
\]
因此 \(\sigma\) 在 \(\SL_2(\FF_5)\) 的标准幺幂生成元上取值为 \(1\)，从而在整个 \(\SL_2(\FF_5)\) 上恒等于 \(1\)。
...
任取 \(g\in \GL_2(\FF_5)\)。把它写成 \(g=sh\)，其中 \(s\in \SL_2(\FF_5)\)、\(h=D_{\det g}\)。于是
\[
...
即可得到
\[
\sgn(g_\ell)=\left(\frac{\ell}{5}\right).
\]
与前述 \(\sgn(g_\ell)=(-1)^{r_\ell}\) 比较，结论成立。证毕。
\end{proof}
