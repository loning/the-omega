...
If the universe's future is open (spiral), if the total budget $c_{FS}$ is continuously growing, then "future" is not a script already written waiting to be discovered, but a **Cloud of Potentiality** filled with infinite possibilities.
...
* **Audit**: Merely confirms that $v_{ext}^2 + v_{int}^2 = c_{FS}^2$ holds at that moment.
...
To respond to the observer's inquiry, the universe is forced to **withdraw investment** from paths B and C. All $c_{FS}$ budget originally scattered across countless paths is instantly recalled and **all-in bet** on path A.
...
* When you write "photon is a particle" at moment $t_{now}$, this stroke of ink seeps back along the spiral trajectory to $t_{past}$.
