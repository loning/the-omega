...
在定理 \ref{thm:fold-bin-min-degeneracy-fib-audited} 已把审计偶数窗上的最小退化度写成 Fibonacci 律、推论 \ref{cor:fold-bin-minsector-complement-maxfiber-audited} 已把其补层规模锁定到 \(F_{m+1}=D_{2m-2}\)、而命题 \ref{prop:fold-bin-phi-identifiable-power-divergence} 已给出相对均匀基线的幂散度极限闭式之后，还可把这两条链进一步压缩为下列四条结论：对象层的最小退化层在群胚代数中切出一个纯矩阵理想，子覆盖规模若沿无穷偶窗持续则最小退化度相对于候选 Fibonacci 律必须指数塌缩，而函数层的整个 \(f_\alpha\)-散度谱则退化为一条二指数刚性曲线，并由单个 \(\chi^2\)-型极限常数完全坐标化。
...
\noindent
因此，在 bin-fold 口径下出现了两类互补刚性。其一，审计偶数窗的最小退化层在对象层切出一个由 \(F_{m/2}\) 与 \(F_m\) 精确控制的纯矩阵理想，并以 \(F_{m+1}\) 的补层规模完成 primitive spectrum 的 Fibonacci 二裂；其二，相对均匀基线的幂散度极限谱在函数层退化为二指数系统，既满足精确二阶递推，又能由单个 \(\chi^2\)-型常数恢复 \(\varphi\) 并回收整条谱。由此，最小退化层的对象刚性与输出非均匀性的函数刚性便在同一 Fibonacci 机制下合并为彼此对应的两侧表述。
