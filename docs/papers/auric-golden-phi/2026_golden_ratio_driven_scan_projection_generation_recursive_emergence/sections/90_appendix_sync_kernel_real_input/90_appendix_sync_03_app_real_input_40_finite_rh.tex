...
表 \ref{tab:real_input_40_kernel_newman_threshold} 给出若干 \(p\) 的数值扫描结果：若在扫描区间内未观测到满足平方根界的 \(\beta\)，则给出下界 \(\beta_\ast(p)>\beta_{\max}\)，并同时列出扫描网格上 \(\Lambda_p/\sqrt{\lambda_1}\) 的最小值及其发生位置（脚本 `scripts/exp_real_input_40_kernel_newman_threshold.py` 一键生成）：
