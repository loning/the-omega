...
同一证书还给出 boundary 扇区 \(U_R\) 的割大小与稳态通量常数 \(\Phi_R=\frac1{12}\)：
\begin{itemize}
  \item \textbf{生成脚本：}\texttt{\detokenize{scripts/exp_window6_edge_flux_skeleton.py}}。
  \item \textbf{LaTeX 生成物：}\texttt{\detokenize{sections/generated/eq_window6_edge_flux_skeleton.tex}}。
  \item \textbf{审计 JSON：}\texttt{\detokenize{artifacts/export/window6_edge_flux_skeleton.json}}。
\end{itemize}
...
\begin{itemize}
  ...
  给出 \(|V|,|E|\)、度直方图、连通性与 \(\mathrm{Aut}(\Gamma_6)=\{1\}\) 的可复核结论（命题 \ref{prop:terminal-gamma6-rigidity}）。
\end{itemize}
