...
\begin{conjecture}[模性加速与折叠标量可测性的互斥原则]\label{conj:conclusion-foldgauge-modular-acceleration-measurability}
记
\[
\mathscr F_C:=\sigma(C_m:\ m\ge 1)
\]
为由折叠标量序列生成的可见标量代数。则一个 \(\pi\)-方案真正优于定理 \ref{thm:conclusion-foldgauge-pi-golden-linear-law} 的黄金线性精度律，当且仅当它携带某个不属于 \(\mathscr F_C\) 的辅助模态。预期地，AGM 情形中的该模态应表现为椭圆--模参数，Ramanujan--Chudnovsky 情形中表现为模形式输入，而 BBP 情形中表现为径向逐位寄存结构。反之，凡其全部可见输出在投影后都收缩为 \(\mathscr F_C\)-可测标量的方案，其主误差指数都应回落到 \(q=\varphi/2\)。

若此猜想成立，则“超线性算 \(\pi\)”与“超出折叠标量可测性的外部结构”将在本文框架内获得严格等价。
\end{conjecture}

\paragraph{比较读法}
上述命题链说明：折叠标量通道不仅把 \(2\pi\) 作为极限值锁定，而且把偶 Bernoulli--\(\zeta\) 塔压缩为同一条奇次黄金幂谱链。于是，纯标量层上的 \(\pi\)-逼近速率被定理 \ref{thm:conclusion-foldgauge-pi-golden-linear-law} 刚性固定；不同投影兼容序列的首个可观测分歧，则由定理 \ref{thm:conclusion-foldgauge-pi-evenzeta-first-difference} 与偶 \(\zeta\)-塔的首差阶严格对位；而远位局域提取若要发生，则定理 \ref{thm:conclusion-foldgauge-local-extraction-budget-barrier} 强制其在实现层支付线性地址预算。就此而言，在本文语言中，\(\pi\) 不是孤立常数，而是整个偶值 \(\zeta\)-塔在 fold-gauge 标量边界上的零阶读出；经典快速算法之间真正的差别，也不在极限值本身，而在于它们把多少外部结构带入、以及这些结构在投影后还能保留多少。
