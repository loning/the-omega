\begin{table}[H]
\centering
\scriptsize
\setlength{\tabcolsep}{4pt}
\caption{anymicroscopic distribution$\mu$ under's conditional expectation$E_{\mu}[\cdot\mid \Fold_m]$ andvariancedecompositionnumericalkerneldual($m=12$). Take$f$ normalization windowvalue(andbeforeTableconsistent), dualifpurely deterministic generation's  $\mu$ check$\|f\|_2^2=\|E_{\mu}f\|_2^2+\|f-E_{\mu}f\|_2^2$ $\mathbb E_{\mu}[\mathrm{Var}(f\mid\Fold_m)]=\|f-E_{\mu}f\|_2^2$. }
\label{tab:fold_conditional_variance_decomposition}
\begin{tabular}{lrrrrrr}
\toprule
$\mu$ & $\mathbb E[f]$ & $\|f\|_2$ & $\|E_{\mu}f\|_2$ & $\|f-E_{\mu}f\|_2$ & var avg & err\\
\midrule
uniform & 0.498361 & 0.554418 & 0.499976 & 0.239591 & 0.0574038 & 1.39e-17\\
dirichlet\_s1 & 0.496869 & 0.552736 & 0.500299 & 0.234984 & 0.0552176 & 2.08e-17\\
dirichlet\_s3 & 0.501058 & 0.557285 & 0.508484 & 0.228059 & 0.0520108 & 2.08e-17\\
dirichlet\_s8 & 0.495728 & 0.554716 & 0.511536 & 0.214571 & 0.0460407 & 1.39e-17\\
\bottomrule
\end{tabular}
\end{table}
