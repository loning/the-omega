...
本附录给出一组与 prime-register 容量直接相关的、完全初等且具有显式界的不等式：当用 prime-register 的 residue 轴把每条折叠纤维进一步细分为 \(P\) 个桶时，任意 \(q>1\) 的碰撞矩（\(q\)-矩）必然下降，并且下降幅度具有显式上下界。其意义是：同一 prime-register 容量变量不仅承载“无损提升/可逆回放”的残差，也在统计层面提供一条\textbf{可核验的碰撞压力削减律}，从而支撑局域并行读出与核验程序的资源预算。
...
$$
它是“碰撞/重数”强度的一个可核验标量：\(q\) 越大，对大纤维的惩罚越强。
...
在“既要承载又要打散”的自洽表述下，常出现 \(P\) 的平方根级预算前沿：若承载任务需要 \(P\gtrsim B\)，而打散任务使有效阈值约被除以 \(P\)，则两条约束联立往往要求 \(P^2\gtrsim B\) 量级（作为预算设计的启发式界）。
\end{remark}
