\paragraph{共振闭包级数、Pisot--Bernoulli 卷积与实零梯的四项解析后果}
...
\begin{corollary}[共振整函数双侧零点计数的显式求和与线性密度]\label{cor:xi-resonance-entire-zero-count-explicit-linear-density}
沿用定理 \ref{thm:fold-resonance-entire-lp} 的记号，并定义
\[
N_\varphi(R):=\#\{z\in \mathcal Z(\mathcal F_\varphi):\ |z|\le R\}.
\]
则对一切 \(R\ge \varphi^2/2\) 有精确公式
\[
\boxed{\;
N_\varphi(R)=
...
=
\Bigl\{\pm\bigl(k+\tfrac12\bigr)\varphi^n:\ k\in\ZZ_{\ge 0},\ n\ge 2\Bigr\}.
\]
对固定 \(n\ge 2\)，满足
\[
\left(k+\frac12\right)\varphi^n\le R
\]
的非负整数 \(k\) 个数恰为
\[
\left\lfloor \frac{R}{\varphi^n}+\frac12\right\rfloor.
\]
乘以 \(2\) 即得到该层对双侧计数的贡献。只有当 \(\varphi^n/2\le R\) 时该层非空，
亦即 \(n\le \lfloor\log_\varphi(2R)\rfloor\)，故得到精确求和公式。
...
N_\varphi(R)
=
2R\sum_{n=2}^{\lfloor \log_\varphi(2R)\rfloor}\varphi^{-n}
+O(\log R).
\]
而
\[
