...
上一段已解释“为何本文主要使用 cover 的 \(\zeta\)”。为回应“折叠诱导滑动块因子的谱/\(\zeta\) 计算应明确到可执行层”的审稿口径，本小节把两类对象的计算链条明确列出，并给出一个对 \(Y_m\) 内禀 \(\zeta_{Y_m}\) 的\textbf{可计算}口径（需明确其与覆盖 SFT 的行列式 \(\det(I-zA_m)\) 的定义差异）。
