...
Because this review is written for a GRG topical collection, we explicitly remove a common ambiguity: throughout the paper, the word \emph{superspace} is used exclusively in the quasicrystal/model-set sense of a \emph{cut-and-project embedding space} $G=E\times H$ (Section~\ref{sec:superspace:cps}). It is a \emph{representation space} in which an aperiodic pattern in $E$ can be realized as a projection/cut of a periodic lattice in $G$.
...
No Grassmann structure and no configuration-space-of-metrics structure is assumed or used in the mathematics below; the only ``extra dimensions'' are bookkeeping dimensions in a well-controlled cut-and-project construction. When we later discuss a \emph{causality interface} (Section~\ref{sec:causality}), the point is precisely to keep this representation geometry separate from any physical identification with Lorentzian spacetime.
