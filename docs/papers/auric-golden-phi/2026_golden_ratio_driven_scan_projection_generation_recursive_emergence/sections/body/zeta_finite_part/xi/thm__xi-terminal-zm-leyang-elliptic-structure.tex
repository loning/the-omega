...
令
\[
x:=\lambda,\qquad u:=y-\lambda^2,\qquad w:=2u+1=2y-2\lambda^2+1\quad(\text{亦记 }S:=w),
\]
...
\begin{remark}[公开文本可检索性说明]
就公开资料可检索范围而言，尚未发现与本定理中若干显式消元恒等式（例如上述分歧值结式证书）在相同记号与等价口径下直接同式的公开条目。
该观察仅提供一条可复核的文本去重策略，并不构成对相关命题“新颖性”的逻辑完备证明。
\end{remark}
