...
\begin{remark}[公开索引同形排查说明]\label{rem:fold-bernoulli-p-regeneration-closed-form-public-index-note}
围绕定理 \ref{thm:fold-bernoulli-p-regeneration-bivariate-pgf}--定理 \ref{thm:fold-bernoulli-p-finite-time-mismatch-rational-recurrence} 中出现的闭式表达
（例如 \eqref{eq:fold-bernoulli-p-regeneration-bivariate-pgf}、
\eqref{eq:fold-bernoulli-p-cycle-mismatch-pgf}、
\eqref{eq:fold-bernoulli-p-mismatch-entropy-rate}、
\eqref{eq:fold-bernoulli-p-finite-time-mgf-den} 的特定系数结构）
进行公开关键词检索时，检索结果通常回落到 Zeckendorf/Parry 正规化与一般自动机/转导文献条目，
未见在同一四态换能器语境与同一参数化下给出等价闭式或同阶同系数递推的可核对来源。
该说明仅是公开索引层面的同形排查，不替代完备文献证明；并可将检索结果的代表性入口视为如 \url{https://arxiv.org/html/2504.17299v1} 一类的综述/关联条目。
\end{remark}
