...
When we face the future, what we have is **$v_{ext}$ (external velocity/degrees of freedom)**.
...
* It transforms into **$v_{int}$ (internal structure/memory)**. It is convergent, definite, **minted gold coins**.
...
* Your $c_{FS}$ budget is frantically transforming from $v_{ext}$ to $v_{int}$.
...
* No new information enters your $v_{int}$.

* Your $v_{ext}$ has not transformed into any meaningful structure.
...
* Scrolling through short videos: This is a **"false transaction"**. Although the screen flashes and information flows, it does not truly increase your $v_{int}$ complexity. You pay attention (the most expensive currency) and only buy back a pile of $v_{env}$ (waste) forgotten in the next second.
...
Don't be afraid of buying wrong (making mistakes is also an increase in $v_{int}$, called experience). The most terrible thing is **not buying**.
