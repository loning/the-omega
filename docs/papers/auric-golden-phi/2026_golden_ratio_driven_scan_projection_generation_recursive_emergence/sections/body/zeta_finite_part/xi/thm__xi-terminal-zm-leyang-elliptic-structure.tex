...
等价地，谱支 \(\lambda(y)\) 可视为椭圆曲线 \(E_{\min}\) 上有理函数 \(y=x^2+u\) 的局部反演，从而原先的多分支谱反演被提升为椭圆曲线上的有理函数反演问题。
该椭圆曲线满足
\[
...
\par
令 \(\pi_y:E_{\min}\to\mathbb P^1_y\) 为有理函数 \(y=x^2+u\) 所诱导的次数 \(4\) 态射，则 \(y\) 在无穷远点 \(O\in E_{\min}\) 处具有唯一四阶极点，因而 \(\infty\in\mathbb P^1_y\) 为全分歧分歧值。
其有限临界点由 \(\dd y=0\) 刻画，并恰分为两类：
\begin{itemize}
  \item 两个有理临界点
  \[
...
其中首项系数满足显式代数公式
\[
\kappa_*^{2}=\frac{3\lambda_*^{2}-1}{8\lambda_*^{3}-3\lambda_*^{2}-1}.
\]
