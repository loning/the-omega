...
在同一 window-6 锚点上，环旋转 \(C_6\) 在 cyclic 扇区 \(X_6^{\mathrm{cyc}}\) 上封闭并诱导一个刚性的轨道多重集合 \(\{1,2,3,6,6\}\)（小节 \ref{subsec:bdry-tower-zeck-gut} 的结论 D）。
...
此外，dyadic 折叠 \(\mathrm{Fold}^{\mathrm{bin}}_6\) 在 boundary 扇区上强迫 \(\delta=34\) 的二层覆盖（结论 E），诱导一个可复核的 \(\ZZ_2\) sheet parity。
\begin{itemize}
...
结论 J/K 的 “\(1+8+12\)” 与 “\(12=8+4\)” 劈裂无需新增脚本：
...
\paragraph{R2e（可复核接口）：bin-fold 退化谱的双尺度选择律与 \texorpdfstring{$\dim=55$}{dim=55} 统一锚点}
结论 L 将二进制区间折叠 \(\mathrm{Fold}^{\mathrm{bin}}_m\) 的退化度谱
\(
...
\paragraph{R2e'（交叉审计接口）：\texorpdfstring{$\mathrm{Fold}^{\mathrm{bin}}$}{Foldbin}–\texorpdfstring{$\mathrm{Fold}$}{Fold} 的跨尺度补余恒等式}
终结式结论 \ref{par:gut-foldbin-fold-complement-identity} 给出一条把“低分辨率最刚性扇区补余容量”与“高分辨率最大纤维极值模尺度”直接锁定的交叉审计恒等式；
...
结论 D1/E1 把 \(X_{10}^{\mathrm{bdry}}\) 与 \(X_6\) 规范同构，并以 \(B_3\) 根字典细化为
...
\paragraph{R4a（零参数硬失败）：权 \texorpdfstring{$4$}{4} Eisenstein/\texorpdfstring{$E_8$}{E8} \texorpdfstring{$\theta$}{theta}-模的素数骨架审计}
在 R4 的“有限素数骨架判定”模板之上，一个特别刚性的基准情形是：若顶层统一对象被声明为
...
（\(p\) 为素数）。
\par
我们将最小硬失败点 \(p\in\{2,3,5\}\) 的数值表与 JSON 封装为可复现输出（可作为“零拟合”的一票否决护栏）：
\begin{itemize}
...
必须进入向量值/同余层提升分支（见结论小节 \ref{subsubsec:bridgepack-e8-hecke-lock} 的结构性出口描述）。
