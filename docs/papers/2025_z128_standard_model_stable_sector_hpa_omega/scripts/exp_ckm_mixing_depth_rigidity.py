...
Candidate family (bounded by B):
  - |V_us|  := 1/sqrt(m), where 1 <= m <= min(B, V_max) and V_max = max_{w in X6} V(w).
...
def best_triple_at_B(
    ...
    best: Tuple[float, float, int, int, int] | None = None  # (maxe, sume, comp, m, k23, k13)
    m_max = min(B, vmax)
    k_max = 2 * B
    for m in range(1, m_max + 1):
        for k23 in range(1, k_max + 1):
            for k13 in range(1, k_max + 1):
                maxe, sume = triple_objective(m, k23, k13, vus_ref, vcb_ref, vub_ref)
                comp = m + k23 + k13
    ...
    maxe, sume, _comp, m, k23, k13 = best
    return BestTriple(B=B, m=m, k23=k23, k13=k13, max_abs_log_err=maxe, sum_abs_log_err=sume)
...
def write_rows(
    ...
    for b in best_list:
        rig_rows.append(
            f"{b.B} & $(m,k_{{23}},k_{{13}})=({b.m},{b.k23},{b.k13})$ & {b.max_abs_log_err:.6f} & {b.sum_abs_log_err:.6f} \\\\"
        )
    ...
    rows: List[str] = []
    rows.append(
        f"$|V_{{us}}|$ & {vus_ref:.6g} & $1/\\sqrt{{{b20.m}}}$ & {vus:.9g} & {r_vus:.3f} & {lr_vus:.6f} \\\\"
    )
    rows.append(
