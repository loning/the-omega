...
- [ ] **黑洞信息悖论（蒸发、信息回收、Page 曲线等）— Track 路线图（BH0–BH6）**：当前未处理蒸发动力学与信息一致性条件；建议参考与入口：Page 曲线与典型纠缠结果 \cite{Page1993AverageEntropySubsystem,Page1993InformationInRadiation}，islands/QES/replica wormholes 的综述与核心工作 \cite{AlmheiriHartmanMaldacenaShaghoulianTajdini2021EntropyHawkingRadiation,Penington2020EntanglementWedgeReconstruction,AlmheiriHartmanMaldacenaShaghoulianTajdini2020ReplicaWormholes}。本文对应范围边界与引用指针集中在 `sec:limitations_related_work` 的 `subsec:open_problems_audit_tagged`；黑洞/虫洞的外部目标与接口指针见 `app:bh_wormholes_pointer`。
  - BH0（范围契约 / 不可达性语义 / 幺正路线）[ ]：明确 “因果不可达 vs 复杂度不可达” 的口径、是否坚持整体幺正、以及回退策略与最小失败点模板。建议落点：`app:minimal_failure_point_templates` 与 `sec:limitations_related_work` 的同一风格。
  ...
  - BH4（蒸发/泄漏动力学：无外部输入的辐射通道闭合）[ ]：在声明的协议状态 $(m,n,K)$ 下，定义外部可访问的“辐射记录”与质量/资源账本，并给出“必泄漏”成立的最小充分条件、失败点与回退（Iface/Audit）。
  - BH5（Page 曲线 surrogate：相关性/可恢复性闭合）[ ]：把 Page 行为写成协议可计算的互信息/压缩率/可恢复性误差的时间曲线闭合（而非仅能谱黑体），并给出 Page 转折的阈值判据（Iface/Audit）。
  - BH6（等价岛机制 / 不双计数的代数重建）[ ]：给出“内区自由度并非额外独立 Hilbert 空间”的协议级等价机制（代数重建/编码像），并标注其最小假设包与可证伪点（Iface/Math/Audit）。
  - BH-P0（可逆黑盒队列原型：Fold$_m$/纤维坐标的幺正类比）[x]：提供一个按论文对象实现的最小可逆 toy：微观态 $N\in\{0,\dots,2^m-1\}$、折叠 $w=\mathrm{Fold}_m(N)\in X_m$、辐射记录 $(w,j)$（$j$ 为 fiber coordinate）；只观测 $w$ 时粗粒化丢失信息，而完整记录可逆恢复输入。脚本：`scripts/exp_black_hole_queue_equivalence.py`（使用 `protocol_kernel.fold_m` / `all_xm` / `split_cyc_bdry` 并验证 $m=6$ 的 $|X_6|=21,18\oplus 3$）。
