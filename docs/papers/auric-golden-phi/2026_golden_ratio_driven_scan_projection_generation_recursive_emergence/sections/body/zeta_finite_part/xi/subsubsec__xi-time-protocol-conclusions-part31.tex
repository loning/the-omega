...
\]
并且上述两条递推在 \(\QQ\)-系数意义下皆为\emph{最小阶}：若某个 \(\QQ\)-系数齐次线性递推同时对全体 \(q\) 成立，则其特征多项式必分别被 \(P_m\) 与 \(Q_m\) 整除。
特别地，
\[
\deg P_m=\bigl|\mathcal B_m\cup\{L_m\}\bigr|,
\qquad
\deg Q_m=\bigl|\mathcal B_m\bigr|+2.
\]
换言之，\(\Tr((T^{(q)})^m)\) 与 \(S_m(q)\) 在 \(q\)-方向上的全部指数率分别由 \(\mathcal B_m\cup\{L_m\}\) 与 \(\mathcal B_m\)（并加上二次 Lucas--Pell 因子）完全控制，且不存在更低阶的有理系数递推能够在 \(q\)-方向同时湮灭这些指数底数。
\end{conclusion}
\input{subsubsec__xi-time-protocol-conclusions-part31j}
