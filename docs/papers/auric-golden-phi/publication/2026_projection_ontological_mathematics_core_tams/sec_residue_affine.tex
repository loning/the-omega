...
For the second formula, square the first identity and sum over \(x\in X_m\).
...
\[
  ...
  \end{cases}
\]
eliminates the cross terms and gives the displayed identity for \(\Sigma_m\).
\end{proof}
...
\begin{equation}\label{eq:indexed-generating}
  \sum_{n\ge 0}R^{\mathrm{dbl}}(n)z^n
  =
...
\]
The second coefficient is \(R^{\mathrm{dbl}}(n-F_{m+1})\), since
...
\(R^{\mathrm{dbl}}(t)=R(t)+R(t-1)\) yields the four-term formula.
\end{proof}
