...
\]
由 \(P_mQ_m=\mathrm{id}\)（命题 \ref{prop:fold-pushforward-lift}）可知：
\begin{itemize}
...
\(\rho(A)<r\) 等价改写为 LMI 可行性 \(\exists\,H\succ 0:\ A^\ast H A\prec r^2H\)，从而允许把“谱界”以一份矩阵 \(H\) 的形式作为\textbf{可复算审计证书}随结果输出（见注 \ref{rem:lyapunov-sdp}）。
\end{remark}
...
\begin{remark}[角色扭曲的交换立方图]\label{rem:twist-commuting-cube}
...
\]
在 \(|r|\) 充分小时由幂级数定义，并按解析延拓视为其在 \(|r|<1\) 内的亚纯延拓（若存在）。
...
\]
若 \(G\) 在 \(|r|<1\) 内全纯，则理论上 \(\mathrm{Res}(G;r_0)=0\)；因此任何显著偏离 \(0\) 的数值结果都可作为“HTF 失败 / NonCancel 失败 / 或候选谱点假设不成立”的 falsification 信号。
\end{algorithm}
...
\begin{enumerate}
  ...
  \]
  在 \(|r|<1\) 内可定义并满足类似命题 \ref{prop:residue-ring-explicit-error-bound} 的一致上界（从而在闭层给出单位圆盘全纯性与可审计控制）。
  \item \textbf{（正性/整数重数证书，NonCancel 自动）} 要求 \(\mathcal{L}_u\) 的离散谱点具有有限代数重数 \(m_\lambda=\Tr(P_\lambda)\in\NN\)，并使得所关心通道的主部系数由 \(\Tr(P_\lambda)\) 或更一般的 \(\Tr(WP_\lambda)\ge 0\)（某个正算子 \(W\)）给出；于是任意极点都携带不可为零的留数证书（命题 \ref{prop:resolvent-trace-integer-residue-noncancel}），同点消去被结构性排除。
