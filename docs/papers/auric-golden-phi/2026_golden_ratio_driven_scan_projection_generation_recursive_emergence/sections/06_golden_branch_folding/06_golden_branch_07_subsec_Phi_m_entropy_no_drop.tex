...
窗口层的多对一压缩并不等价于时间层的信息损失：当 $m\ge 3$ 时，定理 \ref{thm:Phi_m-conjugacy-threshold} 给出有限窗逆码 $\Psi_m$，从而被折叠抹去的自由度可迁移到时间方向的有限状态记忆并在有限延迟后恢复。这提供了“第三寄存器”的另一种实现方式：其不必以静态轴显式存储，而可作为同步滤波器的内部状态隐式承载。
\end{remark}
