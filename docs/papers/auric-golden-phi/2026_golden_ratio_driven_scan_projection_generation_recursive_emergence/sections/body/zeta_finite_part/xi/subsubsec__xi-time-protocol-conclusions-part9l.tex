...
在折叠同余的模半环表示、多重度谱的 Parseval 恒等式、bin-fold 的两态一致渐近以及真实输入 \(40\) 状态核的输出位势扭曲谱已经建立之后，还可抽出下列五条彼此兼容的结构结论。
...
对真实输入 \(40\) 状态核，沿用命题 \ref{prop:real-input-40-output-dirichlet-nonrh} 的记号。若 \(m,n\ge 2\) 且 \(m\mid n\)，则
\[
\rho_n\ge \rho_m,
\qquad
\theta_n\ge \theta_m.
...
\]
其中 \(jn/m\) 是 \(1,\dots,n-1\) 之间的整数。故每个非平凡 \(m\)-次单位根扭曲都出现在 \(n\)-次单位根扭曲集合之内。于是
\[
\rho_n
=
...
\]
命题 \ref{prop:real-input-40-output-dirichlet-nonrh} 已给出存在显式模数使 \(\theta_m>1/2\)；将上述单调性施用于其全部倍数即可。证毕。
\end{proof}
