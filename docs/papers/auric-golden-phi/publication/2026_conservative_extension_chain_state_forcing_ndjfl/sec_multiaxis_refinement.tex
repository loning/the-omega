...
The previous sections analyzed forcing at a fixed state. We now treat refinement itself as a dynamics: the forcing clause stays unchanged, but the set of admissible realizations may shrink along different axes of information. This section is subsidiary to the obstruction theory developed above, yet it shows that the same state-forcing semantics supports nontrivial update and support questions without altering the underlying truth conditions.
...
\]
\item compatible finite nerve presentations have been chosen so that the induced branch obstruction classes satisfy
\[
...
\begin{proof}
The pullback of gerbes is again a gerbe, and the induced cohomology class is the pullback of the original one \cite[Tag 06NY]{StacksProject}, \cite[Theorem~3.11]{BerghSchnurer2021}. Let $\chi\in X_v$. Then
\[
\chi_*(\omega_v)=0.
\]
Applying the pullback $f_{q,p}^*$ gives
\[
\chi_*(\omega_w)=\chi_*\bigl(f_{q,p}^*(\omega_v)\bigr)=f_{q,p}^*\bigl(\chi_*(\omega_v)\bigr)=0,
\]
...
\subsection{Finite-state complexity}

For the rest of the section, assume that the multi-axis structure is finite and that a distinguished formula $\chi$ is part of the input together with polynomial-time decidable tables for the relations
\[
q\mapsto (M,q\Vdash\chi)
\qquad\text{and}\qquad
q\mapsto (M,q\Vdash\neg\chi).
\]
This is the natural finite encoding once the underlying semantic state graph has already been computed and the support problems are asked over that graph.
