...
\midrule
\input{sections/generated/leakage_kernel_demo_rows.tex}%
\\
\bottomrule
\end{tabular}
...
\paragraph{Determinant-packaging carrier (optional audit bridge).}
\AuditTag Independently of scattering, one may extract a delay/linewidth proxy directly from a determinant package $D(z)$ by the resolvent-trace identity
$\tau(\omega)=\Im\frac{\dd}{\dd\omega}\log D(r\e^{\iu\omega})=\Im\bigl(-\iu z\,\Tr(A(I-zA)^{-1})\bigr)$ (Appendix~\ref{app:equivalence_ladder_certificates},
Tables~\ref{tab:det_phase_delay_trace_identity_audit} and \ref{tab:det_to_smatrix_ws_delay_audit}).
In this carrier, a linewidth proxy $\Gamma$ can be audited from delay peaks (FWHM) and compared to the Breit--Wigner proxy $\Gamma_\tau=4/\tau_{\max}$
(Appendix~\ref{app:equivalence_ladder_certificates}, Table~\ref{tab:det_delay_linewidth_audit}),
and then converted into a survival-kernel proxy $P(t)=\exp(-\Gamma t)$ and a finite-family CAP selection entry
(Tables~\ref{tab:linewidth_survival_kernel_audit} and \ref{tab:survival_finite_family_audit}).
\AuditTag Gate: phase-difference readouts must be declared away from determinant zeros (ill-conditioned phase), and linewidth extraction requires the existence of a resolvable peak and half-maximum crossings on the declared grid.
\AuditTag Fallback: if these gates fail, revert to the finite-family vocabulary at the level of Definition~\ref{def:survival_kernel_family}
and the minimal demo generator (Table~\ref{tab:leakage_kernel_demo}) without invoking frequency-domain packaging.
...
\midrule
\input{sections/generated/leakage_kernel_m6_trap_exit_rows.tex}%
\\
\bottomrule
\end{tabular}
...
\input{sections/generated/leakage_kernel_m6_trap_exit_summary.tex}%
