\begin{remark}
下述陈述仅以文内既定的四次谱多项式 \(\Pi(\lambda,y)\)（作为 \(\{Z_m(y)\}_{m\ge 0}\) 的最小湮灭多项式）及其 \(\lambda\)-判别式分解为输入；分歧集机制在算术几何层面可等价表述为固定椭圆曲线上的次数 \(4\) 有理覆盖与其单值护照数据，而无需复述既有推导细节。\cite{Internal2026TerminalZmLeeYangEllipticS4Audit}
\end{remark}
...
由于 \([2^n]^{-1}(O)=E_{\min}[2^n]\) 且一般地 \([2^n]^{-1}(P_i)=P_i+E_{\min}[2^n]\)，得到所示除子等式。
