...
\noindent
上述三条结果把三域 Frobenius 直积结构进一步压缩为一条纯符号层的刚性主线：全部二次信息恰由 \(C_2^4\) 立方体承载，其中只有 \(\chi_{\mathrm{LY}}\) 真正控制 \(S_3\) 分解型，而 Latt\`es 六次域的全分裂则对任意 \(S_{10}\times S_3\) 事件只实施一个无偏的 \(1/48\) 乘法缩放。
