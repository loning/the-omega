...
% 日期与时间：2026-02-17 14:25:12（Asia/Singapore）
...
\begin{enumerate}
  ...
  由于 \(y=\infty\) 处的局部分歧单值在四张纸上的循环型为 \(4\)-循环，而任一有限分歧值处为换位，
  故几何单值群由一个 \(4\)-循环与一个换位生成，从而必为 \(S_4\)。
...
\]
因此三次多项式 \(S(y)\) 精确刻画 \(E[2]\setminus\{O\}\) 在 \(\pi_y\) 下的像；并且由于 \(D(X)=(2Y)^2\) 在 \(K\) 中为平方，其相对范数在 \(k\) 中必为完全平方，上式给出该平方化的最小整系数证书。
\par
