...
\]
并且 \(\Pi(\lambda,y)=0\) 的规范化与 \(E_{\min}\) 双有理同构；该同构可由显式代换
\[
...
\]
直接给出。
因此 \(\Pi(\lambda,y)=0\) 给出 \(E_{\min}\) 的一组平面模型；其射影闭包在无穷远处可以出现奇点，但规范化后即为光滑椭圆曲线。
...
反之，若 \(P\in E(\QQ)\)，则 \((\lambda,y)=(X(P),y(P))\) 满足 \(\Pi(\lambda,y)=0\)，从而 \(\lambda=X(P)\) 给出 \(\Pi(\lambda,y_{0})\) 的有理根。证毕。
\end{proof}
