...
下列结论以四次特征方程 \(\Pi(\lambda,y)=0\)（结论 \ref{con:xi-terminal-zm-order4-rational}）及其 \(\lambda\)-判别式分解（结论 \ref{con:xi-terminal-zm-discriminant-leyang}）为基础，
不再复述 \(\Pi\) 的定义、Lee--Yang 实临界值的数值定位与相关消元恒等式。
其核心是：谱曲线 \(\Pi(\lambda,y)=0\) 的归一化为一条判别式 \(37\) 的固定椭圆曲线，
重量投影 \(y\) 作为该椭圆曲线上的次数 \(4\) 有理函数，其分歧像在整系数层面被同一 Lee--Yang 三次因子唯一锁定。
