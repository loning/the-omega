...
\begin{conclusion}[重量投影 \texorpdfstring{$\pi_y:E\to\mathbb P^1_y$}{pi\_y} 的分歧除子完全显式：Lee--Yang 三分歧支在 \texorpdfstring{$X$}{X}-坐标上收缩为不可约三次式]\label{con:xi-terminal-zm-elliptic-y-hurwitz-passport}
沿用结论 \ref{con:xi-terminal-zm-elliptic-normalization} 的椭圆模型
\(
E:\ Y^2=X^3-X+\tfrac14
\)
与重量函数 \(y=X^2+Y-\tfrac12\)，并记首一四次
\[
F(X,y):=X^{4}-X^{3}-(2y+1)X^{2}+X+y(y+1)\in\QQ[X,y],
\qquad
P_{\mathrm{LY}}(y):=256y^3+411y^2+165y+32.
\]
则 \(F(X,y)=0\) 在 \(\QQ(E)\) 中恒成立（结论 \ref{con:xi-terminal-zm-leyang-conjugate-fiber-cubic}），从而 \(\pi_y\)（由 \(P\mapsto y(P)\) 给出）为次数 \(4\) 的有限态射。其分歧数据满足：
\begin{enumerate}
  \item \textbf{极点结构。}
  \(y\) 在无穷远点 \(O\) 处有唯一极点，且
  \[
  ...
  并且 \(\{y(Q_1),y(Q_2),y(Q_3)\}\)（按同一嵌入与 \(\{\alpha_i\}\) 配对）恰为 \(P_{\mathrm{LY}}(y)=0\) 的三根。
...
\end{enumerate}
此外，在三条 Lee--Yang 分歧支 \(P_{\mathrm{LY}}(y)=0\) 上，对应并合谱根（亦即分歧点之 \(X\)-坐标）满足一次消元子
\[
\boxed{\ 279X+512y^{2}+518y+5=0\ } \qquad (\text{结论 \ref{con:xi-terminal-zm-branch-double-root-linear-closure}}).
\]
\end{conclusion}
\begin{proof}[证明要点]
