...
\par
在 Hardy/Smirnov 的标准正则性口径下（此处仅作结构对照），\(F_{\zeta}\) 还可被组织为典型的内外因子分解
\[
F_{\zeta}=B\cdot S\cdot O,
\]
其中 \(B\) 为由盘内零点集合 \(\{a_k\}\subset\mathbb{D}\) 生成的 Blaschke 内因子，\(S\) 为由边界奇异测度生成的奇异内因子，\(O\) 为外因子（由边界模长给出）。
于是“盘内零点数据为空”可等价重述为 \(B\equiv 1\)；在更强正则性假设下还可进一步要求 \(S\equiv 1\)。
在本文语言中，这一分解强调了一个刚性障碍：内因子（零点数据）并不由边界模长决定，因而若希望在 \(\mathbf{PW}\)/unitary-slice 的可见轴上核验“盘内零点禁入”，就必须通过正交记录轴把该隐藏数据外置为可审计对象；假设 \ref{assump:bridge-rh} 即是对该外置桥接的合同化编译。
