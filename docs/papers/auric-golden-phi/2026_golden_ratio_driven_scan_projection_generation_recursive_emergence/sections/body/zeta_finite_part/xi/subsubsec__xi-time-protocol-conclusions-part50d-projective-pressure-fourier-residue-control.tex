...
上述结果表明：附录 E 提供的并不只是“把整数 \(q\) 推广为实参数”的形式构造，而是一条无条件凸压力曲线；附录 D.527 提供的也不只是 cyclotomic 扭曲的上界证书，而是经由精确 Fourier--Parseval 反演所闭合的真实 residue bias 指数。于是，连续 \(q\)-压力与模 \(m\) Dirichlet 扭曲不再是两条并列技术线，而构成了同一解析控制理论的两侧边界：一侧以凸性支配离散 moment 数据，另一侧以 twisted spectral radius 精确锁定 residue 偏差的指数尺度，并把平方根门槛的失效直接转写为模分布局部极限定理的不可实现性。
