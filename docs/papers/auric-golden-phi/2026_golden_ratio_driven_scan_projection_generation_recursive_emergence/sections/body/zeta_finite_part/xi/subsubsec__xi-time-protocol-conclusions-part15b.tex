...
下述结果建立于文中已引入之配分函数序列 \(Z_m(y)\) 及其四次特征多项式 \(\Pi(\lambda,y)\)，并以关于 \(\lambda\) 的判别式分解
\(\mathrm{Disc}_{\lambda}\bigl(\Pi(\lambda,y)\bigr)=-y(y-1)\bigl(256y^3+411y^2+165y+32\bigr)\)
及唯一负实临界值 \(y_{\mathrm{LY}}\) 为出发点（结论 \ref{con:xi-terminal-zm-order4-rational}，\ref{con:xi-terminal-zm-discriminant-leyang}）。
其陈述旨在对该特定谱结构作进一步结构性推进，而非对既有公开定理的复述；相关推导细节参见内部文献 \cite{Internal2026TerminalZmLeeYangEllipticS4Audit}（2026-02-18 14:22:16，Asia/Singapore）。
其核心在于：谱曲线 \(\Pi(\lambda,y)=0\) 的归一化为判别式 \(37\) 的固定椭圆曲线，重量投影 \(y\) 为其上的次数 \(4\) 有理函数，其分歧像在整系数层面由同一 Lee--Yang 三次因子唯一锁定。
