...
abelian side of the primitive-orbit calculus. For finite-group
extensions the right language is that of class functions. The first
point is to fix the normalisation of the conjugacy-class expansion;
once this is done, the twisted determinants, the Adams operations, and
M\"obius inversion combine into a parallel non-abelian theory.
...
\noindent
The determinant factorisation linearises the periodic orbit data after
passing from group elements to class functions.

\begin{theorem}[Class-function determinant linearisation and
  Adams--M\"obius inversion]
  \label{thm:class-function-adams-mobius}
...
Applying \Cref{thm:class-function-adams-mobius} to irreducible
...
\begin{lemma}\label{lem:adams-coefficients-bounded}
  ...
  \[
    \abs{a_{\varrho,\sigma}^{(m)}}\le (\dim\varrho)^2.
  \]
\end{lemma}
...
\begin{theorem}[Primitive Peter--Weyl determinant formula]
  \label{thm:primitive-peter-weyl-determinant}
...
\begin{proof}
  Apply \Cref{thm:class-function-adams-mobius} with
...
\begin{theorem}[Explicit class Mertens theorem under twisted spectral
  gap]
...
\begin{proof}
  ...
  \(\sigma\). Since \(n/m\le n/2\) for \(m>1\),
  \[
  ...
  correction term equals \(\mathsf M(A)-\gamma\). This yields
...
The constants \(\mathsf M_C(A)\) therefore encode more than the leading
...
\begin{theorem}[Non-abelian Fourier reconstruction of class Mertens
  ...
  \[
    \Delta_C(A)
    :=\mathsf M_C(A)-\frac{|C|}{|G|}\mathsf M(A).
  \]
  Then
