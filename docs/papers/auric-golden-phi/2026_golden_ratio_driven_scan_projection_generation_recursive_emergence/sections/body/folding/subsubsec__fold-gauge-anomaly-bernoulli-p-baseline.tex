...
数值上
\[
p_\star\approx 0.770917,\qquad g_\ast(p_\star)=p_\star^2\approx 0.594299.
\]
\end{corollary}
\begin{proof}
...
\begin{proposition}[失配密度的全局单峰性与过半阈值]\label{prop:fold-gauge-anomaly-bernoulli-p-density-unimodal-threshold}
令 \(g_\ast(p)=\frac{p^2(3-2p)}{1+p^3}\) 为定理 \ref{thm:fold-gauge-anomaly-bernoulli-p-laws} 的失配密度。
则 \(g_\ast\) 在 \((0,1)\) 上严格单峰：存在唯一 \(p_\star\in(0,1)\) 使 \(g_\ast\) 在 \((0,p_\star)\) 严格递增、在 \((p_\star,1)\) 严格递减；
并且 \(p_\star\) 由不可约三次方程
\[
\boxed{\;p_\star^3+2p_\star-2=0\;}
\]
唯一确定。
此外存在精确阈值
\[
\boxed{\;g_\ast(p)>\frac12\ \Longleftrightarrow\ p\in\Bigl(\frac{1+\sqrt{21}}{10},\,1\Bigr).\;}
\]
\end{proposition}
\begin{proof}
直接求导得
...
\]
分母在 \((0,1)\) 恒正，故单调性由 \(p^3+2p-2\) 的符号唯一决定；该三次在 \((0,1)\) 上严格递增且跨越零点，故存在唯一根 \(p_\star\) 并得到单峰性。
解方程 \(g_\ast(p)=\tfrac12\) 等价于 \(5p^3-6p^2+1=0=(p-1)(5p^2-p-1)\)，从而在 \((0,1)\) 上仅余根 \(p=(1+\sqrt{21})/10\)，据此得到阈值刻画。证毕。
\end{proof}
...
\begin{corollary}[失配自协方差的显式奇偶分解]\label{cor:fold-gauge-anomaly-bernoulli-p-covariance-explicit-even-odd}
保持命题 \ref{prop:fold-gauge-anomaly-bernoulli-p-covariance-recurrence} 的记号，并假设 \(p\neq\tfrac12\)。
定义
\[
A(p):=\frac{p^2(p^2-3p+1)^2}{(p+1)^2(2p-1)(p^2-p+1)},
\]
\[
R_{\mathrm{even}}(p):=\frac{p(1-p)\bigl(p^5-p^4-6p^3+3p^2+2p-1\bigr)}{(p+1)(2p-1)(p^2-p+1)^2},
\]
\[
R_{\mathrm{odd}}(p):=\frac{p^2(1-p)\bigl(p^5+4p^4-4p^3-5p^2+7p-2\bigr)}{(p+1)(2p-1)(p^2-p+1)^2}.
\]
则对任意整数 \(m\ge 0\) 有
\[
\boxed{\;
c_{2m+1}(p)
=A(p)(-p)^{2m+1}+\bigl(p(1-p)\bigr)^m R_{\mathrm{odd}}(p)\;},
\]
并且对任意整数 \(m\ge 1\) 有
\[
\boxed{\;
c_{2m}(p)
=A(p)(-p)^{2m}+\bigl(p(1-p)\bigr)^m R_{\mathrm{even}}(p)\;}.
\]
...
对平行边按 Bernoulli\((p)\) 权重分解并逐边求和，即得所示四项闭式；对 \(Y_0=1\) 汇总得到边缘密度；再将 \((0,1)\) 与 \((1,0)\) 两项相加即得 \(\PP(X_0\neq Y_0)=g_\ast(p)\)。证毕。
\end{proof}
...
\input{subsubsec__fold-gauge-anomaly-sofic-memory}

\begin{remark}[审计脚本]\label{rem:fold-gauge-anomaly-bernoulli-p-audit-script}
定理 \ref{thm:fold-gauge-anomaly-bernoulli-p-laws}--推论 \ref{cor:fold-gauge-anomaly-bernoulli-p-specialize-half} 中出现的闭式表达式（包括三阶密度累积量、协方差生成函数、压力四次方程以及 GC 远场常数的临界点核验）可由脚本 \texttt{scripts/exp\_fold\_gauge\_anomaly\_bernoulli\_p\_closed\_form.py} 进行一致性核验。
\end{remark}

\begin{remark}[公开检索核验口径]\label{rem:fold-gauge-anomaly-bernoulli-p-public-search-note}
<<<<<<< Current (Your changes)
以 \(g_\ast(p)\)、\(\sigma_G^2(p)\) 与 \(P_{G,p}^{(3)}(0)\) 的闭式表达、\(\chi_{P(p)}(\lambda)\) 的完全分解、压力四次方程 \eqref{eq:fold-gauge-anomaly-bernoulli-p-pressure-quartic} 的系数结构，以及命题 \ref{prop:fold-gauge-anomaly-bernoulli-p-gc-defect-farfield-critical} 的临界偏置 \(p=\varphi/2\) 为检索词，对公开资料作关键词与精确字符串检索，未见与本节同型的直接表述。
=======
以 \(g_\ast(p)\)、\(\sigma_G^2(p)\) 的闭式表达、\(\chi_{P(p)}(\lambda)\) 的完全分解、压力四次方程 \eqref{eq:fold-gauge-anomaly-bernoulli-p-pressure-quartic} 的系数结构，以及定理 \ref{thm:fold-gauge-anomaly-zero-run-fibonacci}--\ref{thm:fold-gauge-anomaly-g-support-sofic} 中出现的零段条件闭式与 sofic-cover 特征因子分解为检索词，对公开资料作关键词与精确字符串检索，未见与本节同型的直接表述。
>>>>>>> Incoming (Background Agent changes)
由于公开文献体量巨大且检索覆盖有限，该类核验不构成穷尽性断言，亦不应被视为“唯一性”证明。
\end{remark}
