...
Therefore the average above depends on $f$ only through its block averages $P_nf$, and the resulting linear combination of these block averages is exactly the matrix action $F_n(P_nf)$.
\end{proof}
...
Define the recomputed next block state $v_{\mathrm{true}}:=P_{n+1}\tilde q_{n+1}$, the operator prediction $v_{\mathrm{pred}}:=F_n v_n$, and the micro-uplifted coarse state $v_{\mathrm{uplift}}:=P_{n+1}(U_n\tilde q_n)$.
Then
\[
v_{\mathrm{true}}-v_{\mathrm{pred}}
\;=\;
\underbrace{\bigl(v_{\mathrm{true}}-v_{\mathrm{uplift}}\bigr)}_{\text{\emph{recompute/refinement}}}
\;+\;
\underbrace{\bigl(v_{\mathrm{uplift}}-v_{\mathrm{pred}}\bigr)}_{\text{\emph{coarse-graining loss}}}.
\]
\end{lemma}
\begin{proof}
Add and subtract $v_{\mathrm{uplift}}$.
\end{proof}
