...
from common import gauss_digit_prob, gauss_step
...
def build(out_dir: Path, *, png: bool = False) -> None:
    ...
    # Deterministic seed point (as in the reproduction scripts, a generic irrational).
    ...
    for t in range(1, n_max + 1):
        ...
        if t in ns:
            ...
            diff = np.abs(phat[1:] - p_theory[1:])
            errors[t] = (float(np.sum(diff)), float(np.max(diff)))
    ...
    ax1.set_title(f"Gauss digit law (N={n_max:,})")
    ax1.legend(frameon=False, loc="best")

    ns_sorted = sorted(ns)
    l1s = [errors[n][0] for n in ns_sorted]
    linfs = [errors[n][1] for n in ns_sorted]
    ...
    out_pdf = out_dir / "fig08_gauss_digit_law_fit.pdf"
    fig.savefig(out_pdf, bbox_inches="tight")
    if png:
        fig.savefig(out_dir / "fig08_gauss_digit_law_fit.png", bbox_inches="tight")
    plt.close(fig)
