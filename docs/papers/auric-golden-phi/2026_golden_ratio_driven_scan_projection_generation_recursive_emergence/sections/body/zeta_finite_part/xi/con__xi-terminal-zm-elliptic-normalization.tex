...
w:=2\lambda^{2}-1-2y.
\]
则存在定义于 \(\QQ\) 上的双有理同构，其在坐标 \((x,u)\) 下的逆变换由 \(y=u+x^2\) 给出。
等价地，在 \((x,u)\) 坐标下有恒等式
\[
\Pi\bigl(x,\ u+x^{2}\bigr)=u^{2}+u-(x^{3}-x),
\]
从而 \(\Pi(\lambda,y)=0\) 的归一化同构可由显式双有理变换
\[
...
\]
直接给出。
进一步引入 \(w=-(2u+1)\)（等价于上式定义的 \(w=2\lambda^{2}-1-2y\)）后，得到同构
\[
...
\[
\boxed{\;
y=\frac{2\lambda^{2}-1-w}{2}=X^2+Y-\frac12\ \in\ \QQ(E)\; } ,
