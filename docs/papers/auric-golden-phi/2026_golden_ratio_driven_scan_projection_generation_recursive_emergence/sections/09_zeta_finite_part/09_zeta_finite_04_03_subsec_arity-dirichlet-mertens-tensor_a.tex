...
$$
\begin{lemma}[边际--角色投影引理（边际求和杀死非零角色）]\label{lem:arity-character-projection}
设 \(m_1,m_2,m_3\in\NN\)，\(\omega_{m}:=e^{2\pi i/m}\)，并令常数张量由三维离散傅里叶反演给出
\[
C_{a,b,c}
=\frac{\mathsf{M}}{m_1m_2m_3}
+\frac{1}{m_1m_2m_3}\!\!\sum_{\substack{(j_1,j_2,j_3)\in(\ZZ/m_1)\times(\ZZ/m_2)\times(\ZZ/m_3)\\(j_1,j_2,j_3)\ne(0,0,0)}}
\omega_{m_1}^{-j_1a}\omega_{m_2}^{-j_2b}\omega_{m_3}^{-j_3c}\,C(j_1,j_2,j_3).
\]
则对任何边际求和，角色空间自动投影到被保留坐标对应的子群上。特别地：
\[
\sum_{b\in\ZZ/m_2}\sum_{c\in\ZZ/m_3} C_{a,b,c}
=\frac{\mathsf{M}}{m_1}
+\frac{1}{m_1}\sum_{j_1\in(\ZZ/m_1)\setminus\{0\}}\omega_{m_1}^{-j_1a}\,C(j_1,0,0),
\]
以及
\[
\sum_{a\in\ZZ/m_1}\sum_{b\in\ZZ/m_2} C_{a,b,c}
=\frac{\mathsf{M}}{m_3}
+\frac{1}{m_3}\sum_{j_3\in(\ZZ/m_3)\setminus\{0\}}\omega_{m_3}^{-j_3c}\,C(0,0,j_3).
\]
\end{lemma}
\begin{proof}
把反演式代入边际求和，并用有限群角色的正交性：
\(
\sum_{x=0}^{m-1}\omega_m^{-jx}=m
\)
当且仅当 \(j\equiv 0\pmod m\)，否则为 \(0\)。因此一旦对某坐标求和，该坐标上所有非平凡角色（对应 \(j\ne0\)）全部消失，仅留下该坐标上平凡角色的模态。
\end{proof}
