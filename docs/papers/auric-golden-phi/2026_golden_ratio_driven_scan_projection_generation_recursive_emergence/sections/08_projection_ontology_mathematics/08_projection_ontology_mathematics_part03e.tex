...
\begin{itemize}
  \item 值横截面投影 \(\Fold_\infty\) 至多调用一次（仅在末端把结果落回 \(X_\infty\)）；
...
因此任何中间步骤对同一值纤维反复“先落回再继续演化”的正规化，都不会改变末端按最终值落回 \(X_\infty\) 的结果，可统一推迟为末端一次。
...
\le
2\|f\|_\infty\cdot \mathbb P_{\omega\sim\mu}\!\left[\mathcal{K}_{m+1\to m}(\omega)=1\right].\;}
\]
...
脚本 \texttt{scripts/exp\_fold\_truncation\_curvature.py} 在均匀微态基线下对 \(m\le 20\) 做精确枚举并生成表格片段；对一般数据源亦可按同一定义以抽样估计。
...
\IfFileExists{sections/generated/tab_fold_truncation_curvature_stats.tex}{%
\input{sections/generated/tab_fold_truncation_curvature_stats}%
}{}
