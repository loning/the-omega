...
\]
其平方自由部分为 \(1931\cdot 34319=66269989\)，从而 \(P_{10}\) 的分裂域 \(L/\QQ\) 具有唯一的非平凡正规子域
\[
L^{A_{10}}=\QQ\!\left(\sqrt{66269989}\right)
=\QQ\!\left(\sqrt{\mathrm{Disc}(P_{10})}\right).
\]
并且 \(L\) 的最大阿贝尔子扩张恰为上述二次子域（亦即任意阿贝尔正规子扩张均包含于其中）。
因此，除去平凡分支 \(u\in\{0,1\}\) 后，谱四次曲线的非平凡分歧位置在 \(\QQ\) 上达到满对称的算术一般性。
\end{proposition}
\begin{proof}[证明要点]
取素数 \(p\nmid 27\cdot\mathrm{Disc}(P_{10})\)。
由 Dedekind 分解型与 Frobenius 循环型的对应，\(P_{10}\bmod p\) 的不可约因子次数分解等于某个 Frobenius 元在 \(S_{10}\) 中的循环分解型。
直接计算给出如下三种分解型：
\[
P_{10}\bmod 13\ \text{不可约}\ (10),\qquad
P_{10}\bmod 7\ \text{分解型为}\ (9,1),\qquad
P_{10}\bmod 11\ \text{分解型为}\ (7,3).
\]
因此 \(\mathrm{Gal}(P_{10}/\QQ)\) 含一 \(10\)-循环，从而传递且含奇置换；并含一 \((9)(1)\)-型元素，从而该传递子群为本原群。
又由 \((7)(3)\)-型元素可取其三次幂得到一条 \(7\)-循环；Jordan 定理断言：含素数长度 \(p\le 10-3\) 的 \(p\)-循环之本原子群必包含 \(A_{10}\)。
故 \(A_{10}\subseteq \mathrm{Gal}(P_{10}/\QQ)\)。
再由群中含奇置换，推出 \(\mathrm{Gal}(P_{10}/\QQ)=S_{10}\)。
\par
...
而判别式的平方根生成符号表示的二次子域，故 \(L^{A_{10}}=\QQ(\sqrt{\mathrm{Disc}(P_{10})})=\QQ(\sqrt{66269989})\)。证毕。
\end{proof}
...
\(
1-\sum_{k=0}^{10}(-1)^k/k!=28319/44800
\)。证毕。
\end{proof}
...
\begin{conclusion}[对数奇点的超越性]\label{con:fold-gauge-anomaly-log-transcendence}
令 \(u_\ast\) 为命题 \ref{prop:fold-gauge-anomaly-branch-point-classification} 所选主导共轭分支点，并取任意复对数 \(\theta_\ast\) 满足 \(e^{\theta_\ast}=u_\ast\)。
则 \(\theta_\ast\) 为超越数。
进一步，若 \(|u_\ast|\neq 1\)（例如在定理 \ref{thm:fold-gauge-anomaly-branch-radius-cumulant-growth} 的主值对数选择下 \(\Re(\theta_\ast)\neq 0\)），则
\[
\Re(\theta_\ast)=\log|u_\ast|
\]
亦为超越数。
\end{conclusion}
\begin{proof}
由于 \(u_\ast\neq 1\)，故任何满足 \(e^{\theta_\ast}=u_\ast\) 的复对数 \(\theta_\ast\) 均非零。
若 \(\theta_\ast\) 为非零代数数，则由 Lindemann--Weierstrass 定理知 \(e^{\theta_\ast}\) 为超越数，与 \(e^{\theta_\ast}=u_\ast\)（代数数）矛盾；
故 \(\theta_\ast\) 必为超越数。
同理，若 \(\log|u_\ast|\) 为非零代数数，则 \(e^{\log|u_\ast|}=|u_\ast|\) 为超越数，与 \(|u_\ast|\) 的代数性矛盾；
故在 \(|u_\ast|\neq 1\) 下 \(\log|u_\ast|\) 为超越数。证毕。
\end{proof}
...
在可公开检索的多项式条目与关键词索引范围内，未见与 \(P_{10}\) 的整系数串、判别式指纹及满对称伽罗瓦性断言完全一致的同构表述；该核验仅提供“可检索性层面未见同构”的否证接口，并不构成对未收录文献的穷尽性排除。
\end{remark}
