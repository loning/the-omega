...
\par
为强调该函数方程来自完成化接口的投影系统自同构（而非几何“对称轴”隐喻），记二变量接口上的 involution
\[
\boxed{\ \tau:(z,u)\mapsto(uz,u^{-1})\ }.
\]
则自对偶恒等式 \(\Delta(z,u)=\Delta(uz,1/u)\) 等价于 \(\Delta=\Delta\circ\tau\)。
对 Dirichlet 截面 \((z_L(s),u_L(s))=(L^{-s},L^{2s-1})\)，有
\[
\boxed{\ u_L(1-s)=u_L(s)^{-1},\qquad z_L(1-s)=u_L(s)\,z_L(s)\ },
\]
从而 \((z_L(1-s),u_L(1-s))=\tau\bigl(z_L(s),u_L(s)\bigr)\)。
...
\end{proposition}
\begin{proof}
由自对偶恒等式 \(\Delta=\Delta\circ\tau\) 以及 \((z_L(1-s),u_L(1-s))=\tau(z_L(s),u_L(s))\)，
\[
