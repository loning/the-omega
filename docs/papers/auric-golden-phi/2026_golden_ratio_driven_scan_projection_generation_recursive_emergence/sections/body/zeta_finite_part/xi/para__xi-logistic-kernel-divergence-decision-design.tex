...
\par
对碰撞渐近，记 \(f(s):=\sqrt{\phi(s)}\)，则 \(\psi_j(s)=f(s-\sigma_\star-\varepsilon t_j)\)。
由 \(f\in C^\infty(\RR)\cap L^2(\RR)\) 与 Taylor 展开，\(\{\psi_j\}_{j=1}^\kappa\) 在 \(L^2\) 中的张成在 \(\varepsilon\to 0\) 时收敛到导数塔
\(\{f^{(r)}(\cdot-\sigma_\star)\}_{r=0}^{\kappa-1}\) 的张成；
相应 Gram 行列式的首阶非零项由 confluent Vandermonde 结构给出，因而出现
\(
\det G^{(1/2)}=C\cdot \varepsilon^{\kappa(\kappa-1)}\prod_{j<k}(t_k-t_j)^2\cdot(1+o(1))
\)
（其中 \(C>0\) 仅依赖于 \(\kappa\) 与核 \(f\)）。
取负对数即得所示展开。证毕。
\end{proof}
