...
Scans artifacts/<experiment>/<run_id>/manifest.json and produces:
  ...
  - sections/generated/artifact_hash_registry_summary.tex  (PyLaTeX)
"""
...
def main() -> None:
    ...
    out_json.write_text(json.dumps({"version": 1, "records": records}, indent=2, sort_keys=True, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    write_tabular_fragment(
        ...
        rows=[
            [Command("texttt", r["experiment"].replace("_", r"\_")), Command("texttt", r["run_id"]), Command("texttt", r["manifest_sha256"][:12])]
            for r in records
        ],
        booktabs=True,
    )
