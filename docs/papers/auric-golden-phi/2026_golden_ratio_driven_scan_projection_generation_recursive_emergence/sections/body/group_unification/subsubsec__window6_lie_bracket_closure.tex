...
而命题 \ref{prop:terminal-foldbin6-tail-cube-section} 表明任意纤维的偏移集合包含于 \(\{0,21,34,55\}\)，因此同一纤维中任意两点差值只能落在
\(
\{21,34,55,34-21\}=\{21,34,55,13\}
\)。
...
另一方面，脚本 \texttt{\detokenize{scripts/exp_window6_lie_envelope_orthogonal_part.py}} 以有限穷举构造 \(L_i\) 并在模素数口径下对 \(\mathfrak{k}_6\) 的闭包维数给出满维证书（见 \eqref{eq:window6_lie_envelope_orthogonal_part_dim}）。
结合引理 \ref{lem:modp-independence-lift} 的提升论证，可将该模素数线性无关性提升到特征 \(0\)，从而得到 \(\dim\mathfrak{k}_6\ge 210\)。
...
\end{theorem}
\begin{proof}
由引理 \ref{lem:window6-li-traceless}，\(\mathfrak{g}_6\subseteq\mathfrak{sl}(V)\)，故 \(\dim\mathfrak{g}_6\le \dim\mathfrak{sl}(V)=21^2-1\)。\par
另一方面，脚本 \texttt{\detokenize{scripts/exp_window6_lie_envelope.py}} 以有限穷举构造 \(L_i\)，并给出一组位于 \(\mathfrak{g}_6\) 中的矩阵族，其模素数约化在线性意义下达到维数上界（证书片段见 \eqref{eq:window6_lie_envelope_invariants}）。
由引理 \ref{lem:modp-independence-lift}，该线性无关性证书可提升到特征 \(0\)，从而得到 \(\dim_{\QQ}\mathfrak{g}_6\ge 440\)。
结合上界 \(\dim\mathfrak{g}_6\le 440\) 得 \(\dim\mathfrak{g}_6=440\)，故 \(\mathfrak{g}_6=\mathfrak{sl}(V)\)。证毕。
\end{proof}
...
\input{subsubsec__window6_leyang_certificate_proof_chain}
\input{subsubsec__window6_rootcartan_boundarytower_uniqueness}
\input{subsubsec__window6_lie_bracket_unique_closure_synthesis}
