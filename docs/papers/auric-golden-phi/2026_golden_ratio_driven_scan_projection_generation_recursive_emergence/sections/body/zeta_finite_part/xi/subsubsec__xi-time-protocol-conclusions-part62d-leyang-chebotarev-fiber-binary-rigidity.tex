...
在 Lee--Yang 分支三次数域的振幅平方刚性、三域 Chebotarev 直积结构以及纤维独立集复形的同伦分类已经确立之后，还可进一步抽取出下列九条彼此衔接的结构后果。一方面，有符号振幅相对于无符号分支数据给出一个不能在三次基域内消去的真正二次抬升；中间数条把三域 Frobenius 观测从张量独立进一步压缩为二次符号层与 \(L_4\) 共轭类层上的 Lee--Yang 分裂型刚性及其联合密度闭式；最后几条则把纤维多重度的奇偶、最大非可缩层的相对损失及其模 \(6\) 的 Fibonacci 拓扑税分别锁定为 \(\FF_2\)-同伦理指纹、四常数谱与显式缺口公式。
...
\begin{theorem}[三域 Frobenius 类函数观测的完全张量独立]\label{thm:xi-time-part62d-triple-frobenius-classfunction-tensor-independence}
...
\qquad
\sigma_4(p):=\mathrm{Frob}_p|_{L_4}.
\]
任取有界类函数
\[
f:S_{10}\to\CC,
...
\widetilde h\!\bigl(\sigma_4(p)\bigr)
=0.
\]
更一般地，在相应的素数密度极限分布下，一切涉及至少两个不同因子的混合 cumulant 全部消失。
\end{theorem}
\begin{proof}
...
\[
\boxed{\;
\delta\!\bigl((1)(2)\,\middle|\,\chi_{10}(p)=\varepsilon,\ \sigma_4(p)\in C\bigr)=\frac12,\;}
\]
\[
\boxed{\;
\delta\!\bigl((1)(1)(1)\,\middle|\,\chi_{10}(p)=\varepsilon,\ \sigma_4(p)\in C\bigr)=\frac16,\;}
\]
\[
\boxed{\;
\delta\!\bigl((3)\,\middle|\,\chi_{10}(p)=\varepsilon,\ \sigma_4(p)\in C\bigr)=\frac13.\;}
\]
...
\]
上的 Frobenius 极限分布是三个因子的乘积分布。因而 \(\chi_{10}(p)\) 只依赖 \(S_{10}\) 因子，事件 \(\sigma_4(p)\in C\) 只依赖 \(S_4\times C_2\) 因子，而 \(g\bmod p\) 的分裂型只依赖 \(S_3\) 因子。
...
\[
\boxed{\;
\delta\!\bigl(\chi_{10}(p)=\varepsilon,\ \sigma_4(p)\in C,\ g\bmod p\text{ 型为 }(1)(2)\bigr)
=\frac{|C|}{192},\;}
\]
\[
\boxed{\;
\delta\!\bigl(\chi_{10}(p)=\varepsilon,\ \sigma_4(p)\in C,\ g\bmod p\text{ 型为 }(1)(1)(1)\bigr)
=\frac{|C|}{576},\;}
\]
\[
\boxed{\;
\delta\!\bigl(\chi_{10}(p)=\varepsilon,\ \sigma_4(p)\in C,\ g\bmod p\text{ 型为 }(3)\bigr)
=\frac{|C|}{288}.\;}
\]
特别地，
\[
\delta\!\bigl(\chi_{10}(p)=\varepsilon,\ \sigma_4(p)\in C\bigr)=\frac{|C|}{96}.
\]
\end{corollary}
\begin{proof}
...
由定理 \ref{thm:xi-time-part62d-triple-frobenius-classfunction-tensor-independence}，
\[
\delta\!\bigl(\sigma_4(p)\in C\bigr)=\frac{|C|}{48}.
\]
