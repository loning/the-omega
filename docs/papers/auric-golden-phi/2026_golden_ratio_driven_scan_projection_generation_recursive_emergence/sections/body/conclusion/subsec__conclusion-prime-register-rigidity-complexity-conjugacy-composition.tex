...
\noindent
因此，prime-register 机制之所以必须以可数素数轴出现，并非因为某种编码习惯，而是因为一旦乘法账本具有无界素支撑，就不存在任何有限维加法寄存器能够在保真意义下吸收其全部算术状态。进一步，fixed-\(m\) 的 fold/time 界面即使再接任意有限状态后处理，也既不能降低 exact reversibilization 的侧信息阈值，也不能凭空生成精确的素性识别能力。就信息结构而言，可由有限容量/Wedderburn 数据完全呈现的 bulk 常数，与必须保留在可数素数估值账本中的 arithmetic side-data，确实构成两类不可互相压缩的对象。
