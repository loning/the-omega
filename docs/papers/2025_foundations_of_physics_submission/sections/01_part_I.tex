...
\begin{itemize}
    ...
    \item \textbf{O5 (Scan--projection readout)}: Operational time is obtained from finite-resolution scan and projection of intrinsic phase data, yielding Sturmian readouts and canonical Ostrowski/Zeckendorf tick coordinates.
    \item \textbf{O6 (Unitary scan algebra)}: The scan is implemented by a nontrivial unitary $U_{\mathrm{scan}}$ with a conjugate unitary $V$ forming a Weyl pair, encoding intrinsic noncommutativity and uncertainty tradeoffs.
    \item \textbf{O7 (Projection-induced measure)}: Finite-resolution readout induces outcome probabilities via an instrument/POVM family rather than via external sampling postulates.
    \item \textbf{O8 (Orbit regularization)}: Regulated-to-continuum passages use orbit traces and Abel finite parts as a canonical regularization convention.
\end{itemize}
