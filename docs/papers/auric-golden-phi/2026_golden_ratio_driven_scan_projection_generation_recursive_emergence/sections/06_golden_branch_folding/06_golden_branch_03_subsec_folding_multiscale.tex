...
作为对“大 \(m\)”收敛速度的补充审计，脚本 \texttt{scripts/exp\_fold\_gauge\_anomaly\_mc.py} 对 \(m\in\{40,100,200\}\) 做 Monte Carlo 估计并报告标准误差，结果与 \(4/9\) 一致（表 \ref{tab:fold_gauge_anomaly_mc}）。
\end{remark}
...
\]
因此在二元组（order-1）统计层面，均匀基线折叠输出几乎已等同于 Parry 最大熵链；若存在系统性偏差，其主要承载必须出现在更长程依赖（order \(\ge 2\)）上。
\end{remark}
