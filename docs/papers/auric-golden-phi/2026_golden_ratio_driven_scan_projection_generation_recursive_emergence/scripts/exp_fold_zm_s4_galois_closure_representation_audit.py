...
def main() -> None:
    ...
    if g_X != 16 or g_C != 2 or g_C6 != 4 or g_E != 1:
        raise RuntimeError(f"unexpected genera: gX={g_X}, gC={g_C}, gC6={g_C6}, gE={g_E}")
    ...
    tex = "\n".join(
        [
            ...
            "\\[",
            "g(\\widetilde X)=16,\\qquad H^{0}(\\widetilde X,\\Omega^{1})\\cong 2\\cdot\\mathrm{sgn}\\oplus V_{2}\\oplus V_{3}\\oplus 3V_{3}'.",
            "\\]",
            "",
        ]
    )
