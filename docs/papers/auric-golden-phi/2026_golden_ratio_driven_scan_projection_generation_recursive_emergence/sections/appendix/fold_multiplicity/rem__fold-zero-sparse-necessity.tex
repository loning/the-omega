...
\begin{remark}[最小退化扇区的正则子覆盖候选（可证伪）]\label{rem:fold-bin-min-sector-cover-candidate}
...
\)。
该同基数刚性提示：最小退化扇区可望被描述为一个由短后缀约束刻画的正则像；等价地，存在规范单射
\(\iota_{m-2}:X_{m-2}\hookrightarrow X_m\) 使
\(
...
该候选并非解释性选择：其真伪可由 digit DP/退化谱枚举在同一协议下直接否定或确认。
\end{remark}
