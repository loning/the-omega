...
\begin{proof}
由定义 \ref{def:Phi_m} 直接可得；这也是滑动块码“记忆/预期性（左/右半径）”的标准口径（参见 \cite{LindMarcus1995,Kitchens1998SymbolicDynamics}）。
\end{proof}
...
\begin{proof}
像空间是经典符号动力系统事实：任意滑动块码作用于有限字母表全移位，其像是 sofic 子移位（参见 \cite{LindMarcus1995} 中关于滑动块码与 sofic 系统的标准章节）。为便于审计，我们在下一个命题中给出一个可计算的有限图构造，从而将该标准事实转为本文所需的“可复现转移结构描述”。证毕。
\end{proof}
