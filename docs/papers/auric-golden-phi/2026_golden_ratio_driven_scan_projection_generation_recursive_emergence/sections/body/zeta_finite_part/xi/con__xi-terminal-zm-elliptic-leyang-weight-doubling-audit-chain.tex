% (User input window)
% 日期与时间：2026-02-17 14:57:32（Asia/Singapore）
...
\]
则 \(y^{\iota}:E\to\mathbb P^{1}\) 为次数 \(4\) 的覆盖，并满足以下全分歧剖面：
\begin{enumerate}
  \item 在无穷远点 \(O\) 处，\(y^{\iota}\) 具有唯一极点且极点阶为 \(4\)，从而在 \(y^{\iota}=\infty\) 处全分歧（\(e_{O}=4\)）。
  \item 有限分歧值集合恰为
  \[
  y^{\iota}\in\{0,1\}\ \cup\ V\!\bigl(P_{\mathrm{LY}}\bigr),
  \]
  并且每个有限分歧值均为单分歧（上方存在唯一分歧点，分歧指标 \(2\)）。
  \item 两个有理分歧值的纤维分解完全显式：
  \[
  (y^{\iota})^{-1}(0)=\Bigl\{\Bigl(0,\frac12\Bigr),\ \Bigl(-1,-\frac12\Bigr),\ \underbrace{\Bigl(1,-\frac12\Bigr)}_{e=2}\Bigr\},
  \qquad
  (y^{\iota})^{-1}(1)=\Bigl\{\Bigl(1,\frac12\Bigr),\ \Bigl(2,-\frac52\Bigr),\ \underbrace{\Bigl(-1,\frac12\Bigr)}_{e=2}\Bigr\}.
  \]
\end{enumerate}
\end{conclusion}
\begin{proof}[证明要点]
由结论 \ref{con:xi-terminal-zm-y-quadratic-extension-minpoly}，\(y^{\iota}\) 与物理主支 \(y=X^{2}-Y-\tfrac12\) 互为共轭，且由椭圆反演 \(\iota:(X,Y)\mapsto(X,-Y)\) 交换。
结论 \ref{con:xi-terminal-zm-elliptic-y-hurwitz-passport} 给出 \(y\) 的 Hurwitz 护照与两个有理分歧纤维；
对其施加 \(\iota\) 即得到 \(y^{\iota}\) 的分歧剖面与两条纤维分解。
...
g(X):=16X^{3}-9X^{2}+1.
\]
则 \(y^{\iota}:E\to\mathbb P^{1}\) 在 \(V\!\bigl(P_{\mathrm{LY}}\bigr)\) 上方的三条非有理分歧点恰为满足
\[
...
\[
4Xy^{\iota}=4X^{3}-3X^{2}-2X+1,
\]
并且满足结果式证书
\[
\mathrm{Res}_{X}\!\Bigl(g(X),\ 4Xy^{\iota}-\bigl(4X^{3}-3X^{2}-2X+1\bigr)\Bigr)=-64\,P_{\mathrm{LY}}(y^{\iota}).
\]
...
线性关系 \(4Xy^{\iota}=4X^{3}-3X^{2}-2X+1\) 由代回 \(Y=(1-3X^{2})/(4X)\) 得到；
对变量 \(X\) 作消元即得结果式恒等式。
...
沿用结论 \ref{con:xi-terminal-zm-elliptic-normalization-y-iota-ramification-portrait} 的共轭重量函数 \(y^{\iota}\)，并令
\[
y^{\iota}_0:=y^{\iota}(P),\qquad y^{\iota}_1:=y^{\iota}([2]P)\qquad(P\in E(\overline{\QQ})).
\]
则 \([2]\) 在重量坐标上的推送不诱导单值有理自映射，而给出一个由不可约四次整系数消元方程编码的四值对应：
存在 \(\mathcal H\in\ZZ[y^{\iota}_0,y^{\iota}_1]\)（关于 \(y^{\iota}_1\) 为四次）使得对所有 \(P\) 有
\[
\mathcal H\bigl(y^{\iota}_0,y^{\iota}_1\bigr)=0,
\]
并且 \(\mathcal H\) 的显式系数证书为：
\input{sections/generated/eq_fold_zm_elliptic_weight_doubling_H}
其首项系数为完全四次幂
\[
A_{4}(y^{\iota}_0)=\bigl(16(y^{\iota}_0)^{3}-8(y^{\iota}_0)^{2}-4y^{\iota}_0+1\bigr)^{4},
\]
而常数项具有非平凡因式分解
\[
\boxed{\;
A_{0}(y^{\iota}_0)
=(y^{\iota}_0+1)^{2}\bigl((y^{\iota}_0)^{3}-7(y^{\iota}_0)^{2}+13y^{\iota}_0-5\bigr)^{2}
\bigl((y^{\iota}_0)^{4}-6(y^{\iota}_0)^{3}+66(y^{\iota}_0)^{2}+28y^{\iota}_0+9\bigr)
\bigl((y^{\iota}_0)^{4}+2(y^{\iota}_0)^{3}+8(y^{\iota}_0)^{2}+8y^{\iota}_0+8\bigr)\; }.
\]
最关键者在于判别式的平方自由核出现刚性坍缩：将 \(\mathcal H(y,v)\) 视为关于 \(v\) 的四次多项式，则其判别式满足严格分解
\[
...
\end{conclusion}
\begin{proof}[证明要点]
该四次对应由在函数域 \(\QQ(E)\) 中联立倍点公式与 \(y^{\iota}=X^{2}+Y-\tfrac12\) 并对 \((X,Y)\) 消元得到；
...
\[
y^{\iota}(Q)=y_0,\qquad \dd y^{\iota}(Q)=0.
\]
其坐标可由 \(y_0\) 显式表示为
\[
\boxed{\;
...
\)，从而给出 \(X(Q)\) 的显式表达；
再由 \(y^{\iota}=X^{2}+Y-\tfrac12\) 回代得到 \(Y(Q)\)。
