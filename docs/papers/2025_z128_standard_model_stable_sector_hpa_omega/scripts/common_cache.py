...
This cache is designed to speed up repeated runs of the paper's scripts by
persisting expensive intermediate data (e.g., X_m enumerations, Fold_m outputs,
Hilbert paths).
...
def load_or_compute(path: Path, compute: Callable[[], T]) -> T:
    if not cache_disabled() and path.is_file():
        try:
            ...
            # Corrupt cache; fall back to recompute.
            pass
    obj = compute()
    if not cache_disabled():
        try:
            ...
            # Cache writes are best-effort; never fail the pipeline because of caching.
            pass
    return obj
