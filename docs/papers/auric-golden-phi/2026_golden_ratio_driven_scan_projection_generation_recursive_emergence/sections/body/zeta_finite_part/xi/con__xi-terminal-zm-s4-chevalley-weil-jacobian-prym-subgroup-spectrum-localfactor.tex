...
\begin{conclusion}[\texorpdfstring{$S_4$}{S4} 共轭类定点与局部特征值的全计数]\label{con:xi-terminal-zm-s4-conjugacy-fixedpoints-local-eigenvalue-full-count}
设 \(g\in S_4\)。
则有如下定点与局部特征值刻画：
\begin{enumerate}
  \item 若 \(g\) 为换位（循环型 \((2)\)），则 \(|\mathrm{Fix}(g)|=10\)，且每个定点处切空间特征值为 \(-1\)；
  \item 若 \(g\) 为双换位（循环型 \((2,2)\)），则 \(|\mathrm{Fix}(g)|=2\)，且每个定点处切空间特征值为 \(-1\)；
  \item 若 \(g\) 为三循环（循环型 \((3)\)），则 \(\mathrm{Fix}(g)=\varnothing\)；
  \item 若 \(g\) 为四循环（循环型 \((4)\)），则 \(|\mathrm{Fix}(g)|=2\)，且两定点切空间特征值分别为 \(i\) 与 \(-i\)。
\end{enumerate}
\end{conclusion}
\begin{proof}[证明要点]
由结论 \ref{con:xi-terminal-zm-s4-hurwitz-subgroup-spectrum-cycletypes} 可读出
\[
g\!\bigl(X/\langle(12)\rangle\bigr)=6,\quad
g\!\bigl(X/\langle(12)(34)\rangle\bigr)=8,\quad
g\!\bigl(X/\langle(123)\rangle\bigr)=6,\quad
g\!\bigl(X/\langle(1234)\rangle\bigr)=4.
\]
对 \(\mathrm{ord}(g)=m\) 的循环作用应用 Riemann--Hurwitz 公式
\[
2g(X)-2=m\bigl(2g(X/\langle g\rangle)-2\bigr)+|\mathrm{Fix}(g)|(m-1),
\]
在 \(g(X)=16\) 下得到对应定点数 \(10,2,0,2\)。
阶 \(2\) 元在定点处切空间特征值必为 \(-1\)。
阶 \(4\) 元在定点处特征值只能取 \(\pm i\)。
由结论 \ref{con:xi-terminal-zm-s4-chevalley-weil-explicit-decomposition} 得
...
\paragraph{公开检索说明}
针对本段结论中的特征性数值组合（包括
\(H^0(X,2K_X)\) 的重数向量 \((3,1,4,7,4)\)、\(Q=I_2\) 的重数向量 \((8,2,9,13,8)\)、\(\mathrm{Sym}^3V\) 与 \(\mathrm{Sym}^4V\) 的重数向量、
以及 \(\mathcal S_3,\mathcal K_4\) 的等型分量重数）进行公开检索时，未检得与这些数值组合完全一致的公开表述。
鉴于检索覆盖度的客观限制，该事实仅可作为“未发现直接重复”的经验性证据，而不能构成严格的全域排他性证明。
