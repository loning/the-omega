...
|---|---|---|---|---:|---|
| `E₁. 无公理结构扩张链与状态 forcing` | `logic_expansion_chain/` 单独成文 | 11 层保守扩张链：事实 = 状态 forcing、NULL = 形成链断裂、时间 = 细化序参数、观察者无特权——第一次把"事实相对于谁/在哪/知道多少才成立"从哲学直觉变成有证明链的数学结构 | 所有具体动力系统内容；SPG/folding/POM 的具体实例化 | **1** | **数理逻辑 / 物理基础，框架奠基稿** |
...
### E₁. 无公理结构扩张链与状态 forcing（最高优先）

- **核心材料：**
  - `sections/body/logic_expansion_chain/` 全部 7 个子节（9 files, 1432 lines）
- **拆法：**
  - 纯逻辑/基础论文，**不包含任何具体动力系统内容**
  - 主线：11 层保守扩张链 L₀ ⪯ ··· ⪯ L₁₀^OST
  - 核心定理：forcing 持久性、单点保守性、NULL 三分解、更新保持 forcing、语义保真
  - **必须补充的深度内容（当前缺失）：**
    1. 与 Kripke 语义的严格比较定理（退化回收）
    2. 局部对象系 F_{p,s} 与 Sheaf/topos 语义的显式联系
    3. NULL 三分解在至少一个非平凡具体模型中的完整实例
    4. 轴不可替代性的判定复杂性结果
  - 引言定位为"对象层事实的语义基础问题"，面向数理逻辑/物理基础读者
- **为什么最高优先：**
  - 这是定义**整个理论范式的语义地基**
  - 核心洞见（事实 = 状态 forcing；NULL = 形成链断裂；时间 = 细化序参数）对数理逻辑和物理基础都有独立价值
  - 已发表 7 篇技术稿和后续所有论文都在此语义框架下工作
- **为什么必须拆出来单独发：**
  - 读者群完全不同：E₁ 面向数理逻辑/物理基础，E₂ 面向遍历论/动力系统
  - 捆绑会让两边审稿人都不满意——逻辑审稿人不关心 golden mean shift，动力系统审稿人不关心 forcing 语义
- **成稿定位与期刊：**
  - 首选：Notre Dame Journal of Formal Logic（接受 forcing 变体、保守扩张与形式语义，技术门槛适中，与 E₁ 的纯逻辑定位高度吻合）
  - 次选：Studia Logica（广义逻辑，包括哲学逻辑/形式本体论，NULL 三分解可作为"结构性否定"的形式刻画投稿）
  - 强刊路线：Review of Symbolic Logic（需补充 Kripke 退化定理与判定复杂性结果）
  - 纯技术路线：Annals of Pure and Applied Logic（需补充非平凡模型论结果）
  - ~~Foundations of Physics~~（已排除：E₁ 不做物理预测、不解决物理概念难题、forcing 持久性与 QM 互补性冲突——投 FoP 会被要求回答"对物理学有何新贡献"，而 E₁ 的贡献在形式逻辑层面而非物理层面）
- **风险与对策：**
  - 风险：当前定理偏浅（多为定义的直接推论），顶级逻辑期刊可能要求更深结果
  - 对策：补充上述四项深度内容后可投 Review of Symbolic Logic / Annals；若不补充，先投 Notre Dame Journal of Formal Logic 或 Studia Logica（更看重框架贡献与概念清晰度）
...
|---|---|---:|---|---|---|---:|---:|---:|---:|
| `E₁` | `Notre Dame Journal of Formal Logic` 常规 | 滚动 | forcing、保守扩张、形式语义、模型论 | 纯逻辑/语义框架，不含具体动力系统 | E₁ 首选；框架贡献即可 | 6 | 7 | 7 | 5 |
| `E₁` | `Studia Logica` 常规 | 滚动 | 广义逻辑、哲学逻辑、形式本体论 | 同上；NULL 三分解作为"结构性否定" | E₁ 的稳健回退 | 7 | 6 | 6 | 6 |
| `E₁` | `Review of Symbolic Logic` 常规 | 滚动 | 数理逻辑、哲学逻辑、形式语义 | 同上；需补充深度定理 | 若补充了 Kripke 退化定理和判定复杂性结果 | 5 | 7 | 7 | 4 |
| `E₁` | `Annals of Pure and Applied Logic` 常规 | 滚动 | 纯数理逻辑 | 同上；需非平凡模型论结果 | E₁ 的纯技术强刊路线 | 4 | 8 | 7 | 3 |
...
1. **第一批（2026-04 至 2026-06）：E₁ + E₂ + F 三篇并行**
   - `E₁`（逻辑框架）面向数理逻辑读者 → Notre Dame Journal of Formal Logic
...
2. **第二批（2026-06 至 2026-08）：G + H 并行**
   ...
   - `H` → ETDS
3. **第三批（2026-08 以后）：I + J**
   - `I`（群论统一窗）和 `J`（ζ-completion/Ξ 零点）是更深层的细分结果
   - 应在 E/F/G/H 有初步审稿反馈后再定稿
...
E 是全部后续论文的逻辑基础；F 是 G/H/J 的必要前驱；G/H/I 之间无硬依赖。

## 7. 时间线

| 时间 | 动作 | 产出 |
|---|---|---|
| 2026-03-18 至 2026-03-31 | 并行拆 E 和 F 的最小独立稿；构建各自独立目录、定理清单、参考文献 | E/F 各自骨架稿 |
| 2026-04-01 至 2026-04-15 | 完成 E 的首版投稿稿 | E 可进入投稿 |
| 2026-04-15 至 2026-04-30 | 完成 F 的首版投稿稿 | F 可进入投稿 |
| 2026-05-01 至 2026-05-15 | E 投出（ETDS/Advances）；开始准备 G 和 H | E 首投完成 |
| 2026-05-15 至 2026-06-15 | F 投出（Trans. AMS/SIADS）；G 和 H 骨架稿完成 | F 首投完成 |
| 2026-06-15 至 2026-07-31 | G 投出（JFA）；H 投出（ETDS） | G/H 首投完成 |
| 2026-08-01 至 2026-09-30 | 根据 E/F 审稿反馈调整 I/J；G/H 专题回退窗口 | 第二批定稿 |

## 8. 核心 Pitch 句子（新版）

- 已发表 7 篇技术稿（折叠、直方图证书、Zeckendorf 算法、Fibonacci 模结构、Parry-Gibbs 刚性、S4/Prym 算术、GRG 壳几何）。
- 下一批优先发表的不是更多细分结果，而是**定义整个理论范式的框架性论文**。
- `E` 是范式地基稿：回答"什么是可读的"——地址先于值、NULL 为结构性失效、σ-代数不扩张。
- `F` 是中央引擎稿：回答"读到之后如何闭环"——信息账本、重写可判定、共振判别。
- `G`/`H` 是两个独立技术模块：圆维（分析/群论）、ζ 有限部（谱论/动力系统）。
- `I`/`J` 是第二批深层分支：群论统一窗、ζ-completion。
