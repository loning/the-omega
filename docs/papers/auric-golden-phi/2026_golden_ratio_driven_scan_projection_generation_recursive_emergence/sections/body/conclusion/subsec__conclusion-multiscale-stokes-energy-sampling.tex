...
\input{con__conclusion-stokes-stein-leyang-inversion-monodromy}
...
\]
再定义立方体 Bianchi 缺陷
\begin{align}\label{eq:conclusion-bianchi-defect-cube}
...
令 \(X\) 为有限、连通的立方体复形，记 \(C^k(X;\mathbb R)\) 为 \(k\)-余链群，\(\delta:C^k\to C^{k+1}\) 为余边界算子。
对任意 \(\beta\in C^1(X;\mathbb R)\) 定义离散曲率
\[
\kappa:=\delta\beta\in C^2(X;\mathbb R).
\]
设 \(E\) 为有向 \(1\)-胞腔集合、\(F\) 为有向 \(2\)-胞腔集合，并以 \(\|\cdot\|_{\ell^\infty(E)}\)、\(\|\cdot\|_{\ell^\infty(F)}\) 表示相应上确界范数。
则存在 \(\varphi\in C^0(X;\mathbb R)\) 使得
\[
r:=\beta-\delta\varphi
\]
满足显式估计
\[
\|r\|_{\ell^\infty(E)}\ \le\ D(X)\,\|\kappa\|_{\ell^\infty(F)},
\]
其中 \(D(X)\) 为 \(X\) 的 \(1\)-骨架图直径。
特别地，若 \(\kappa=0\) 则 \(r=0\)，即闭 \(1\)-余链在可收缩立方体复形上必为恰当余链。
\end{proposition}
\begin{proof}[证明要点]
取 \(1\)-骨架的一棵生成树 \(T\) 并固定根顶点 \(v_0\)。
定义 \(\varphi(v)\) 为沿树上从 \(v_0\) 到 \(v\) 的唯一路径对 \(\beta\) 的路径和，则对树边有 \(r=0\)。
对任意非树边 \(e\)，由树路径与 \(e\) 组成的回路 \(\ell_e\) 可由若干 \(2\)-胞腔填充为 \(2\)-链 \(S_e\)；
余边界--边界互伴随给出
\(
r(e)=-\sum_{Q\in S_e}\kappa(Q)
\)，
从而 \(|r(e)|\le |S_e|\,\|\kappa\|_{\ell^\infty(F)}\)。
选取标准填充可保证 \(|S_e|\le D(X)\)，遂得所示界。证毕。
\end{proof}
