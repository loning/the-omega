...
\paragraph{(E1b) Resolution-origin shift (scale coordinate).}
\InterfaceTag In the Fibonacci scale map $\mu(r)=\mu_0\varphi^{\,r}$ used throughout the matching dictionaries, changing the reference scale $\mu_0$ is an additive shift of the coordinate $r$.
Likewise, a scheme rescaling of a transmutation scale $\Lambda'\!=c\Lambda$ induces the same additive shift in the $\Lambda$-relative coordinate $r_\Lambda(\mu)=\log_\varphi(\mu/\Lambda)$ (Lemma~\ref{lem:lambda_rescaling_shift_r}).
This is treated as a coordinate convention under the scheme contract (Appendix~\ref{app:scheme_invariance_audit_contract}) and is recorded as a reusable bridge statement in Corollary~\ref{cor:rigidity_scheme_shift_origin_r}.
...
\subsection{Geometric capacity as a CAP-closed boundary principle}
\label{subsec:cap_geometric_capacity_boundary_principle}

\paragraph{The finite geometric capacity statement (no gravity input).}
\MathTag Fix integers $n\ge 1$ and $D_{\mathrm{face}}\ge 1$ and consider a $D_{\mathrm{face}}$-dimensional dyadic boundary face of side length $2^n$.
The number of distinct boundary sites is
\[
N_{\max}(n,D_{\mathrm{face}})=(2^n)^{D_{\mathrm{face}}}=2^{D_{\mathrm{face}}n},
\]
so any injective encoding of microstates into sites obeys the cardinality bound
\[
2^m\le N_{\max}(n,D_{\mathrm{face}})\qquad\Longleftrightarrow\qquad m\le D_{\mathrm{face}}n.
\]
Equivalently, for given $(m,n)$ the minimal face dimension that can carry $2^m$ distinct microstates without additional conventions is
\[
D_{\mathrm{face}}^{\min}(m,n)=\left\lceil \frac{m}{n}\right\rceil.
\]
\AuditTag This is a finite counting statement; it is the minimal failure point for any claim of ``lossless display'' at fixed $(m,n)$.
If $D_{\mathrm{face}}n<m$, then any display must either be non-injective (many-to-one), or introduce additional structure (e.g.\ multi-occupancy/time-multiplexing, error-correcting codes, or an explicitly declared coarse-graining preorder).

\paragraph{Protocol entropy bounds as a finite ``area-law surrogate'' (PT).}
\InterfaceTag The same content can be expressed at the level of entropy bounds from finite boundary dimension.
Appendix~\ref{app:holo_capacity_bound} records the protocol version used in this manuscript:
under the per-site register convention $d_{\mathrm{site}}(m)=2^m$ and a tensor-product locality subclass, a region $R$ on a boundary screen carries
$\mathcal{H}_R\cong (\CC^{2^m})^{\otimes |R|}$ and hence
\[
S_{\partial}(R)\ \le\ \log_2\dim(\mathcal{H}_R)\ =\ m|R|\ =\ I_{\mathrm{prot}}(m,n;R)
\]
(Corollary~\ref{cor:holo_protocol_capacity_bound}).
This is the admissible ``area-law'' content internal to the protocol layer: it is an entropy upper bound forced by finite boundary dimension, not a gravitational field-equation claim.

\paragraph{Boundary-face displays and CAP selection (auditable visualization dictionary).}
\InterfaceTag For arbitrary $(m,n)$, Appendix~\ref{app:generated_tables} records two deterministic boundary-face display conventions:
the VFS boundary-face embedding (Figure~\ref{fig:universal_screen_vfs_holo_face_gallery}) and the locality-optimal Hilbert-face embedding (Figure~\ref{fig:universal_screen_holo_hilbert_face_gallery}).
The candidate family is explicit and finite, and Table~\ref{tab:cap_visualization_selection} reports a CAP selection audit under two priority orders.
In particular, under locality-first priorities, Hilbert-face is selected because its scan is unit-step adjacent (max $L^1$ jump $=1$) on the boundary face.
This is a CAP-closed choice of a \emph{display dictionary}: it does not alter theorem-level folding statements, but it makes multi-$(m,n)$ readouts reproducible and locality-auditable.

\paragraph{Bridge to black-hole area entropy (external input, matching-scoped).}
\MatchTag In this manuscript, black-hole language is handled via declared equivalence semantics and explicit matching dictionaries.
The object-level unification ``black holes as high-energy particles'' is treated as a semantic identification within the same protocol vocabulary (Appendix~\ref{app:bh_wormholes_pointer}).
The area law itself,
\[
S_{\mathrm{BH}}=\frac{k_BA}{4\ell_P^2},
\]
is treated as a standard external input at the matching layer (Appendix~\ref{app:bh_wormholes_pointer}).
\AuditTag The connection to the finite protocol capacity statements above is therefore:
\emph{boundary channel counting} supplies a finite-dimensional entropy bound, while black-hole thermodynamics supplies a target saturation scaling and coefficient under explicit assumptions.
Any numerical identification of $I_{\mathrm{prot}}(m,n;R)$ with an $A$-dependent capacity must specify (i) the information functional being matched (bits vs.\ $k_B$ units), (ii) the region-to-area map (discrete area representative), and (iii) the calibration scope and error budget (Appendix~\ref{app:bh_planck_capacity_calibration}).
\MatchTag For a cosmological ``maximum information'' target treated in the same area-law dictionary, Appendix~\ref{app:bh_planck_capacity_calibration} records an explicit two-convention horizon family (Hubble vs.\ de Sitter proxy) and the corresponding CAP-minimized resolution assignments; see Table~\ref{tab:universe_planck_capacity_targets}.

\paragraph{Minimal falsifiability checks (audit form).}
\AuditTag The bridge above yields two immediate audit checks without introducing new dynamics:
\begin{itemize}
\item \textbf{Capacity feasibility.} For any proposed lossless boundary encoding at fixed $(m,n)$, verify $m\le D_{\mathrm{face}}n$ for the declared boundary face dimension; otherwise the claim must explicitly invoke additional structure beyond an injective site map.
\item \textbf{Locality optimality within a bounded family.} If a candidate family is declared and locality-first CAP selects Hilbert-face, then any claimed improvement must exhibit a scheme in the same family with strictly smaller jump statistics (or else expand the candidate family with an explicit description-complexity accounting).
\end{itemize}
