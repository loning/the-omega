...
This paragraph builds upon the anomaly cohomology framework for the three-generator fragment (Corollary \ref{cor:pom-anom-cohomology-class}) and the moment amplification law (Proposition \ref{prop:pom-anom-moment-amplification}),
and provides a suite of discrimination and coordinate-representation results directly related to constant-layer auditing:
the first gives a \emph{one-point zero-anomaly test} in the torsion-free case;
the second provides a \emph{complete two-point coprime test} without assuming torsion-free, thereby reducing ``all-moment zero anomaly'' to a two-point finite oracle collapse;
the third characterizes \emph{torsion eliminability and minimal elimination order} on general abelian groups, and provides the least common multiple law for denominators on torus-type target groups;
the fourth organizes the higher-order cross-anomaly family \(\{\Delta_S\}\) into \emph{minimal linear coordinates} for Euler/direct-product decomposability (quotient space isomorphism and dimension formula).
...
Then \(\mathcal P(w)\) and the existence of coprime integers \(q_1,q_2\ge 2\) such that
\(
...
Any condition that relies logically on proposition \(\mathcal P(w)\) as a sufficient condition—thereby depending on ``infinite audit over all moment orders \(q\ge 2\)''—can be compressed to verifying only two coprime moment orders;
in other words, the audit complexity of the ``all-moment anomaly axis'' collapses logically to two points.
\end{corollary}
