...
5. **native_decide 轻量优化**
   - 统计本轮新增的 `native_decide` 调用数
   - 如果 ≥ 3 个且为**纯数值等式验证**（形如 `X = Y` 其中 X、Y 可归约为具体数值）：
     a. 将每个 native_decide 结果提取为独立的 `@[simp]` 引理（命名格式：`cached_[原定理名]_[序号]`）
     b. 原定理中的 `native_decide` 替换为 `simp [cached_原定理名_0, ...]`
     c. 重新 `lake build` 确认通过
   - 如果 < 3 个或非纯数值类型 → 跳过，留给深度优化处理
   - 不触碰本轮以外的文件，不重构已有代码
