...
上述结果把该接口进一步压缩到 residue 概率与 primitive residue 生成函数两层：periodic residue law 在偶长度上只是 core law 与移动 Dirac 质量的凸分解，中心化后的原子扰动在实空间与 Fourier 空间均呈现完全刚性的单轨剖面，而 primitive residue 生成函数则仅多出一项 \(z^2\mathbf 1_{\{r\equiv 1\;(\mathrm{mod}\,m)\}}\)，因此全部 residue-resolved 主导奇点与指数阈值仍由 core 决定。与此并行，escort 温度线在极限中退化为可显式平直化的 Fisher--Rao 曲线，而 periodic residue law 的变形又等价于把一块精确质量搬运到 \(\ell\bmod m\) 的最优传输操作；Abel 常数差额则把被剥离的原子贡献保留为一个严格正的 primitive 质量。于是，真实输入 \(40\) 状态核中的 arithmetic surgery、twisted spectrum 与 information geometry 便被压缩为同一条可审计的分层结构：指数门槛与非多项式奇性由 core 决定，而局部概率偏移与常数层差额则由单原子以完全显式的方式记账。
