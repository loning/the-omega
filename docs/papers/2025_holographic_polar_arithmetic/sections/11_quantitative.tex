...
Assume \ref{ass:radial_growth_gaussian}. Then for every $v\in\CC$ and every $\varepsilon>0$ the normalizing sum
$$
Z_\varepsilon(v):=\sum_{n\in M}\exp\!\bigl(-|v-\mathcal{Z}(n)|^2/\varepsilon^2\bigr)
$$
converges and satisfies $0<Z_\varepsilon(v)<\infty$.
\end{proposition}
...
$$
\exp\!\bigl(-|v-\mathcal{Z}(n)|^2/\varepsilon^2\bigr)\le \exp\!\Bigl(-\frac{c^2}{4\varepsilon^2}\,n^{2s}\Bigr),
$$
and the series $\sum_{n\ge 1}\exp(-C n^{2s})$ converges for every $C>0$.
Thus $Z_\varepsilon(v)<\infty$.
\end{proof}
...
\begin{figure}[t]
    ...
    \caption{Examples of the soft projection kernel $\pi_\varepsilon(\cdot\mid v)$ (Definition~\ref{def:projection_kernel}) for $v=\mathcal{Z}(3000)+\mathcal{Z}(4000)$ with $\varepsilon=120$, plotted on a logarithmic vertical scale.}
