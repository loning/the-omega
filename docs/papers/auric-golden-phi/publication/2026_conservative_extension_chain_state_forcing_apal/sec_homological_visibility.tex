...
The unique-branch case $|\Vis_{p,s}(r)|=1$ deserves separate attention because it connects the present theory to the sheaf-theoretic analysis of contextuality. In the framework of Abramsky and Brandenburger \cite{AbramskyBrandenburger2011}, a measurement scenario is contextual precisely when compatible local sections of a presheaf of outcomes fail to extend to a global section---that is, in the present terminology, when the admitted reference is in gluing-level absence with a unique visible branch. Car\`u \cite{Caru2017} showed that the cohomological obstruction associated with such a scenario is not a complete invariant of contextuality: there exist contextual scenarios whose cohomology class vanishes.

The intrinsic visible quotient $A_{\mathrm{vis}}^{\omega}=\operatorname{coker}(\mathrm{ev}_{\omega})$ provides a precise diagnosis. By \Cref{cor:single-branch-reduction}, in the unique-branch case all three aggregate quotients coincide with the single-branch intrinsic visible quotient. \Cref{thm:character-blind-obstructions} then shows that a nontrivial obstruction class $\omega\neq 0$ can have $K_{\omega}=0$ (hence $A_{\mathrm{vis}}^{\omega}=A$) if and only if $\omega$ lies in the $\operatorname{Ext}^1(H_1(N(\mathcal U),\mathbb Z),A)$-summand of the universal coefficient sequence. This is the exact mechanism behind incomplete cohomological detection: the obstruction exists but is invisible to all character channels because it lives entirely in the $\operatorname{Ext}$-part rather than the $\mathrm{Hom}$-part.
