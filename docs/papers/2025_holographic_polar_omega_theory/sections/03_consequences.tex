...
\subsection{Rotation algebra viewpoint and canonical trace}
The Weyl relation in O6 defines the (irrational) rotation algebra $\mathcal{A}_{\alpha}$, the universal $C^{*}$-algebra generated by unitaries $U,V$ with $UV=\e^{2\pi \iu \alpha}VU$ (a noncommutative $2$-torus). For irrational $\alpha$, $\mathcal{A}_{\alpha}$ admits a canonical tracial state $\tau$ characterized on Fourier monomials by
\begin{equation}
    \tau(U^{m}V^{n})=
    \begin{cases}
        1, & (m,n)=(0,0),\\
    ...
    \end{cases}
\end{equation}
and this trace is unique among tracial states \cite{Rieffel1981,Connes1994}. In the covariant continuum model of O6, restricting $\tau$ to the commutative subalgebra generated by $V$ recovers the Haar average on the circle, matching the orbit-trace normalization used in Convention~R1.
\par
For rational $\alpha=p/q$, the corresponding rotation algebra is represented by the $q$-dimensional clock/shift Weyl pair (hence by a matrix algebra), and the canonical trace reduces to the normalized matrix trace, matching the regulated averaging prescription.
