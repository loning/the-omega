...
\]
第一环给出复杂度的可测坐标；第二环把全局体量障碍与局部峰值障碍分开；第三环说明低特征奇偶阴影只能提供有限状态投影；第四环排除了用单一连通圆维替代离散账本支撑的可能；最后一环则表明多尺度共振成本不能被平方可和尾项吸收。因而在本文固定的折叠口径下，逆向困难并不是单一复杂度标签，而是由容量几何、峰值纤维、有限域阴影、prime-register 核与尺度复访共同锁定的复合障碍。
