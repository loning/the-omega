...
\begin{theorem}[等价闭包的二元 Horn 规则基最小性]\label{thm:pom-eq-closure-horn-binary-min}
将边集 \(E_n\) 视为命题变量集合。考虑仅含二元前提的 Horn 蕴含规则族 \(\mathcal{R}\)，每条规则形如
\[
\{e_1,e_2\}\to e_3,\qquad e_1,e_2,e_3\in E_n.
\]
由 \(\mathcal{R}\) 生成的闭包算子记为 \(\mathrm{cl}_{\mathcal{R}}\)。
若要求
\[
...
则 \(|\mathcal{R}|\) 的最小可能值为
\[
\boxed{\ |\mathcal{R}|_{\min}=3\binom{n}{3}=\frac{n(n-1)(n-2)}{2}\ }.
\]
并且（在边重标号意义下）唯一达到该最小值的规则基为三角补全族
\[
\mathcal{R}_{\triangle}:=\bigl\{\ \{ij,jk\}\to ik:\ i,j,k\ \text{两两不同}\ \bigr\},
\]
其中 \(ij\) 表示边 \(\{i,j\}\)。
\end{theorem}
\begin{proof}
规则族 \(\mathcal{R}_{\triangle}\) 生成 \(\mathrm{cl}\)：
任取连通分支内一条长度为 \(2\) 的路径 \(i\!-\!j\!-\!k\)，由规则推出边 \(ik\)；迭代可补全分支为团，得到 \(\mathrm{cl}\)。
因此存在规则基规模上界 \(|\mathcal{R}|\le 3\binom{n}{3}\)。
\par
为证下界，固定互异三点 \(i,j,k\)，取初始边集 \(F=\{ij,jk\}\)。
由定义 \ref{def:pom-edge-closure}，有 \(ik\in \mathrm{cl}(F)\)。
若 \(\mathrm{cl}_{\mathcal{R}}=\mathrm{cl}\)，则在 \(\mathrm{cl}_{\mathcal{R}}\) 的推演中必须从 \(F\) 推出 \(ik\)。
由于每条规则前提大小为 \(2\)，且在首次产生任何新边之前可用前提只能是 \(\{ij,jk\}\)，因此规则库中必须包含 \(\{ij,jk\}\to ik\)。
对所有互异 \(i,j,k\) 同理，故 \(\mathcal{R}\) 必包含全部三角补全规则，条数至少为 \(3\binom{n}{3}\)。
唯一性由该强制性立即得到。证毕。
\end{proof}
...
\begin{conjecture}[Horn 规则基的全局最小性]\label{conj:pom-horn-basis-global-min}
令 \(\mathrm{cl}\) 为定义 \ref{def:pom-edge-closure} 的等价闭包。记 \(\mathrm{IB}(\mathrm{cl})\) 为所有 Horn 蕴含规则基（允许任意有限前提大小）中能生成 \(\mathrm{cl}\) 的规则族集合。
猜想对任意 \(n\ge 3\) 与任意 \(\mathcal{R}\in \mathrm{IB}(\mathrm{cl})\) 都有
\[
|\mathcal{R}|\ \ge\ 3\binom{n}{3},
\]
且等号仅由三角补全规则族 \(\mathcal{R}_\triangle\)（在重标号意义下）达到。
\end{conjecture}
