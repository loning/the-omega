...
\]
因此若记 \(\rho_{\min}:=\min_{j<k}\rho(a_j,a_k)\in(0,1)\)，则对任意排列 \(\sigma\) 必存在某个 \(m\) 使
\[
\boxed{\;
\frac{\pi_m(\sigma)}{p_{\sigma(m)}}\le \rho_{\min}^{\,\kappa-1}\; } .
\]
\end{conclusion}
\begin{proof}
...
\par
再由 \(\prod_{j<k}\rho(a_j,a_k)^2\le \rho_{\min}^{\,\kappa(\kappa-1)}\) 得
\[
\prod_{m=1}^{\kappa}\frac{\pi_m(\sigma)}{p_{\sigma(m)}}
\le \rho_{\min}^{\,\kappa(\kappa-1)}.
\]
若对所有 \(m\) 都有 \(\pi_m(\sigma)/p_{\sigma(m)}>\rho_{\min}^{\,\kappa-1}\)，则左端 \(>\rho_{\min}^{\,\kappa(\kappa-1)}\)，矛盾。
故存在某个 \(m\) 满足所示不等式。证毕。
\end{proof}
