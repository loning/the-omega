...
下列条目把折叠碰撞缺口、整数共振梯的可认证逼近、真实输入 \(40\) 状态核的原子 Witt 手术、黄金分支的一阶传输审计，以及 window-\(6\) 根字典的线性障碍统一压缩为可独立引用的终端结论。
...
\begin{theorem}[输出位势的显式 Edgeworth 非高斯性]\label{thm:xi-real-input-40-output-edgeworth-nongaussian}
...
\]
因此一阶 Edgeworth 修正不消失，修正方向为负偏度，而第四累积量为正，故输出计数分布呈正超峰态。
\end{theorem}
\begin{proof}
