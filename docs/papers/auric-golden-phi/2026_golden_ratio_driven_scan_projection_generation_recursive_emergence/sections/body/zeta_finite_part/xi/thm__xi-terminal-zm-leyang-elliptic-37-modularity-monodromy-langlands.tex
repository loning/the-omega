...
\begin{corollary}[Lee--Yang \texorpdfstring{$\Rightarrow$}{=>} 两类朗兰兹参数：\texorpdfstring{$\mathrm{GL}_2$}{GL2} 模对象与有限八面体参数]\label{cor:xi-terminal-zm-leyang-langlands-level37}
...
则：
\begin{enumerate}
  \item 存在权 \(2\)、level \(37\) 的规范化新形式 \(g\in S_2(\Gamma_0(37))\) 及对应自守表示 \(\pi_{37}\)，并相应存在与 \(E_0\) 相容的 \(\ell\)-进 Galois 表示族 \(\{\rho_{E_0,\ell}\}_{\ell}\)，使得
  \[
