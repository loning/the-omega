...
结论 \ref{con:xi-terminal-zm-elliptic-weight-modular-equation-double-explicit}
与结论 \ref{con:xi-terminal-zm-elliptic-weight-modular-equation-triple-explicit}。
\par
...
\)
控制，见结论
\ref{con:xi-terminal-zm-elliptic-weight-modular-leading-coeff-torsion}。
证毕。
\end{proof}
...
将余因子分解即得上述平方项结构。
详式与分解结果见结论
\ref{con:xi-terminal-zm-elliptic-weight-modular-discriminant-rigidity}。
证毕。
\end{proof}
...
\end{theorem}
\begin{proof}
\(S_4\) 泛型伽罗瓦群结论由特化判别法与 resolvent 不可约性给出，见结论
\ref{con:xi-terminal-zm-elliptic-weight-modular-galois-s4-double-triple}。
\par
...
详见结论
\ref{con:xi-terminal-zm-elliptic-weight-modular-delta-budget}。
证毕。
\end{proof}
