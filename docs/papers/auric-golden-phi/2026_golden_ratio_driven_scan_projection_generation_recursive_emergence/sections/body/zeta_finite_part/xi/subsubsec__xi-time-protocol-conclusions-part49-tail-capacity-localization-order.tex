...
\end{cases}\;}
\]
并且当 \(S\subseteq T\) 时，该同构可由
\[
\Phi:\operatorname{Hom}(G_S,G_T)\longrightarrow G_T,
\qquad
\Phi(\varphi):=\varphi(1)
\]
给出。
...
\varphi_r(x):=rx.
\]
由于 \(x\in G_S\) 的分母素数只来自 \(S\subseteq T\)，而 \(r\in G_T\) 的分母素数也只来自
\(T\)，故乘积 \(rx\) 仍属于 \(G_T\)，从而 \(\varphi_r\) 为良定义的加法同态。
并且 \(\Phi(\varphi_r)=r\)，所以 \(\Phi\) 满射。
\par
另一方面，若 \(\varphi\in\operatorname{Hom}(G_S,G_T)\) 满足 \(\varphi(1)=0\)，对任意
\(x=a/b\in G_S\)（其中 \(a\in\ZZ\)，\(b\) 的全部素因子属于 \(S\)）有
\[
b\,\varphi(x)=\varphi(bx)=\varphi(a)=a\,\varphi(1)=0.
\]
而 \(G_T\subset\QQ\) 为无挠群，故 \(\varphi(x)=0\)。
这说明 \(\Phi\) 单射，因而
\(
