# -*- coding: utf-8 -*-
"""
Hydrogen 21cm hyperfine energy scale -> resolution coordinate r(mu) and m_eff (audit helper).
...
def _load_21cm_energy_eV(path: Path) -> float:
    raw = _read_json(path)
    for it in raw.get("entries", []):
        if str(it.get("id", "")) == "hydrogen_21cm_energy":
            if str(it.get("unit", "")) != "eV":
                raise RuntimeError("Expected 21cm energy unit to be eV.")
            return float(it["value"])
    for it in raw.get("entries", []):
        if str(it.get("quantity", "")) == "hyperfine_21cm_energy" and str(it.get("unit", "")) == "eV":
            return float(it["value"])
    raise RuntimeError("Could not find hydrogen 21cm energy entry.")


def _fmt_mu_GeV(mu: float) -> str:
    if mu == 0.0:
        return "0"
    if mu < 1.0e-12 or mu >= 1.0e3:
...
def main() -> None:
    ...
    e_eV = _load_21cm_energy_eV(data_path)
    ...
    note = "composite hyperfine scale"
    if m_eff < 6:
        note = "composite hyperfine scale (below m=6 single-window admissibility)"

    row = Row(
        obj="hydrogen 21cm hyperfine",
        e_eV=float(e_eV),
        mu_GeV=float(mu),
        r_mu=float(r_mu),
        m_eff=int(m_eff),
    ...
    write_lines(generated_dir() / "hydrogen_21cm_resolution_rows.tex", lines)

    summary = [
        r"\paragraph{Audit summary (hydrogen 21cm $\mu\to r\to m_{\mathrm{eff}}$).} \AuditTag "
        r"We map the 21cm hyperfine splitting energy (a composite low-energy scale) from the vendored miniset "
        ...
        r"Interpretation boundary: $m_{\mathrm{eff}}<6$ is recorded only as a scale label for a composite effect.",
    ]
    write_lines(generated_dir() / "hydrogen_21cm_resolution_summary.tex", summary)
