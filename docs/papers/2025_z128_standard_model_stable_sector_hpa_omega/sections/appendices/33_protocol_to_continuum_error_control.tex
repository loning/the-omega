...
\AuditTag All expectation/probability statements in this appendix are conditional on a declared protocol state $(m,n,K)$ and on an explicit sampling rule for the data stream (Appendix~\ref{subsec:tick_cap_joint_key_contract}): the kernel component $K$ fixes the weighting rule under which $\mathbb{E}(\cdot)$ is taken, while $n$ and $m$ fix the discretization scale and windowed observables.
