\section[统一编码与证明携带审计]{地址—残差—证书的统一 Gödel 化编码与证明携带审计：单码残差压缩、WSOS 证书、Rouché 模板、见证提取与统一编译}\label{app:godel-wsos-rouche}

本附录补齐并推进两类此前缺口：\textbf{(i)} 地址/残差/证书材料的统一自然数编码层（Gödel 化、可判定互逆），并由此给出“残差账本可压缩为单一自然数”的\textbf{单码残差}终局口径（正文第 \ref{sec:conclusion-cluster} 节）；\textbf{(ii)} unitary slice 审计接口的\textbf{证明携带化}与\textbf{可复现}数值证书协议，并在此基础上给出一个可执行的 BridgeRH \textbf{有限区域零点计数证书模板}以及“第四寄存器需求”的\textbf{可判定见证提取机制}。最后给出统一的“运算涌现编译定理”，覆盖四则、商余、布尔运算与（在可计算语义域内的）ZFC 构造性运算族。
...
设某一可审计求值语义在输入 $\omega$ 上产生输出值 $x(\omega)$ 与事件序列 $e_{1:T}(\omega)$，并且存在确定性回放器 $\mathrm{Replay}_0$ 使
$$
...
\Phi(z;x)\in\QQ[x][z].
$$
记 $\Phi(\cdot;x)$ 的“Schur 稳定性”（全部根满足 $\abs{z}\le 1$）为 $\mathsf{SchurStable}(\Phi(\cdot;x))$。经典 Schur--Cohn 判据给出如下接口化归约：
...
进一步地，若另能以独立的边界审计排除 $\abs{z}=1$ 上的零点（例如在单位圆上做区间算术网格核验，或通过结果式/判别式给出有限证书），则由命题 \ref{prop:schur-cohn-reduction} 还可推出严格稳定性 $\abs{z}<1$。
并且核验器 $\mathrm{CheckWSOS}$ 仅用到整数多项式加乘与相等判定，故可被编译为本文可判定核验层的一部分（例如在附录 \ref{app:pw-fragment} 的检查器框架上添加整数/多项式编码运算）。
\end{theorem}
