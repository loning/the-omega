...
本节仅处理一个结构性问题：在给定一族可测概念之后，如何由其有限时间窗读出递归生成更细的概念族，以及该过程是否会在可测结构层面引入“外加对象”。本文在可测动力系统的标准框架下工作（第 \ref{sec:preliminaries} 节），并证明递归地址化严格等价于在既有柱事件族所生成的可见 \(\sigma\)-代数内部对读出前缀作内生化重选（命题 \ref{prop:recursive-addressing-refinement}）。从拓扑直觉看，柱事件前缀亦对应一条前缀超度量过滤：长度前缀等价于更小半径的超度量球，见第 \ref{sec:spg} 节段落 \ref{par:spg-prefix-ultrametric}。

本节采用的语义视角是：递归地址化应被视为一种“可读出前缀”的内生化机制。
其核心并非扩张可见对象，而是在既有可见 \(\sigma\)-代数内部重选并组织可见事件：
派生柱事件始终属于旧层可见 \(\sigma\)-代数 \(\mathcal{G}^{(L)}\)（命题 \ref{prop:recursive-addressing-refinement}），
从而由选定派生族生成的可见 \(\sigma\)-代数满足 \(\mathcal{G}^{(L+1)}\subseteq \mathcal{G}^{(L)}\)（推论 \ref{cor:recursive-addressing-visible-sigma-nonexpansion}）。
...
\begin{proposition}[可见 \(\sigma\)-代数不扩张：递归地址化不引入外加结构]\label{prop:recursive-addressing-refinement}
...
\begin{corollary}[可见 \(\sigma\)-代数的不扩张性]\label{cor:recursive-addressing-visible-sigma-nonexpansion}
在命题 \ref{prop:recursive-addressing-refinement} 的设定下，
令 \(r_{L+1}:=|\mathcal{C}^{(L+1)}|\)，并将所选派生概念族记为
\(
\mathcal{C}^{(L+1)}=\{C^{(L+1)}_{1},\dots,C^{(L+1)}_{r_{L+1}}\}
\)。
定义其可见 \(\sigma\)-代数为
\[
\mathcal{G}^{(L+1)}:=\sigma\bigl(T^{-t}C^{(L+1)}_{j}:\ 1\le j\le r_{L+1},\ t\in\ZZ\bigr).
\]
则有包含关系
\begin{equation}\label{eq:recursive-addressing-visible-sigma-nonexpansion}
\boxed{\ \mathcal{G}^{(L+1)}\subseteq \mathcal{G}^{(L)}\ }.
\end{equation}
换言之，递归地址化仅在旧层可见 \(\sigma\)-代数内部对柱事件作重选与组织，而不引入新的可测集。
\end{corollary}

\begin{proof}
由命题 \ref{prop:recursive-addressing-refinement}，每个 \(C^{(L+1)}_{j}\in\mathcal{G}^{(L)}\)，从而其任意平移 \(T^{-t}C^{(L+1)}_{j}\) 仍属 \(\mathcal{G}^{(L)}\)。
因此由这些集合生成的 \(\sigma\)-代数是 \(\mathcal{G}^{(L)}\) 的子 \(\sigma\)-代数，即得 \eqref{eq:recursive-addressing-visible-sigma-nonexpansion}。证毕。
\end{proof}
...
\end{itemize}
在该约定下，评价映射 \(a\mapsto \ind_{C_a^{(L+1)}}\)（亦可记作指示函数 \(\mathbf{1}_{C_a^{(L+1)}}\)）在语义上是偏定义的：
地址未给定时，并非把 \(\mathbf{1}_{C_a^{(L+1)}}\) 判为假或取值 \(0\)，而是根本不进入可评价域；
其不可寻址性以空值 \(\mathsf{NULL}\) 体现。
