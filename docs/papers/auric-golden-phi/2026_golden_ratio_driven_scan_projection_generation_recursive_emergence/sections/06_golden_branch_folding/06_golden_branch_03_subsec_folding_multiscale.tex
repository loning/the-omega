...
固定常数 \(R\ge 0\)。设某一替代映射 \(T_{m,R}:\Omega_{m+1}\to\Omega_m\) 满足：对任意 \(\omega\in\Omega_{m+1}\)，字符串 \(T_{m,R}(\omega)\) 与直接截断 \(r_{m+1\to m}(\omega)\) 的不同位只可能落在末端 \(R\) 个坐标上（即最多改动末端 \(R\) 位）。
...
\par
在“地址先于概念值”的语义（公理 \ref{ax:pommin-address-null}）下，这给出一个严格的不可局部修补范式：
...
该结果表明：大规模“降分辨率重写”并非仅由极端构造触发，在无结构输入下也会以正的密度铺满全串。
\par
...
作为对“大 \(m\)”收敛速度的补充审计，脚本 \texttt{scripts/exp\_fold\_gauge\_anomaly\_mc.py} 对 \(m\in\{40,100,200\}\) 做 Monte Carlo 估计并给出标准误差，结果与 \(4/9\) 一致（表 \ref{tab:fold_gauge_anomaly_mc}）。
\end{remark}
