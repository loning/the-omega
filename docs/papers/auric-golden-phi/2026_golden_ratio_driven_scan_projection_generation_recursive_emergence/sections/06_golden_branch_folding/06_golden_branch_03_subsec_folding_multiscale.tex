...
更进一步，这一“正密度”现象可被封口为一个闭式常数律：均匀基线下
\(
\bar G_m/m
\)
收敛且极限为 \(4/9\)（定理 \ref{thm:fold-gauge-anomaly-density}，并由脚本 \texttt{scripts/exp\_fold\_gauge\_anomaly\_density\_49.py} 做可复现审计）。
因此，大规模“降分辨率重写”并非仅由极端构造触发：在无结构输入下也会以固定比例铺满全串。
\end{remark}
