...
下述结论以配分函数序列 \(Z_m(y)\) 及其谱多项式 \(\Pi(\lambda,y)\) 为出发点，并以判别式分解
\(\mathrm{Disc}_{\lambda}\bigl(\Pi(\lambda,y)\bigr)=-y(y-1)\bigl(256y^3+411y^2+165y+32\bigr)\)
及唯一负实临界值 \(y_{\mathrm{LY}}\) 为既有事实（结论 \ref{con:xi-terminal-zm-order4-rational}，\ref{con:xi-terminal-zm-discriminant-leyang}）。
相关推导细节可参见内部文献 \cite{Internal2026TerminalZmLeeYangEllipticS4Audit}。
其核心是：谱曲线 \(\Pi(\lambda,y)=0\) 的归一化为判别式 \(37\) 的固定椭圆曲线，重量投影 \(y\) 为其上的次数 \(4\) 有理函数，
其分歧像在整系数层面由同一 Lee--Yang 三次因子唯一锁定。
