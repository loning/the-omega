...
def _tail_sum_power(r: float, k: int, n0: int) -> float:
    # Sum_{n>n0} n^k r^n, stopping when terms are negligible.
    s = 0.0
    n = n0 + 1
...
def build(out_dir: Path, *, png: bool = False) -> None:
    ...
    ax.semilogy(n_list, diffs, marker="o", label=r"observed $|j^{(N)}(\tau)-j^{(N)}(-1/\tau)|$")
    ax.semilogy(n_list, bounds, marker="s", linestyle="--", label="certified truncation-only bound")
    ax.set_xlabel(r"truncation depth $N$")
    ax.set_ylabel("absolute error")
    ax.set_title(r"$j$-invariant numerical $S$-invariance vs truncation depth")
    ax.legend(frameon=False, loc="best")

    out_pdf = out_dir / "fig10_j_invariant_truncation_decay.pdf"
    fig.savefig(out_pdf, bbox_inches="tight")
    if png:
        fig.savefig(out_dir / "fig10_j_invariant_truncation_decay.png", bbox_inches="tight")
    plt.close(fig)
