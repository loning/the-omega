...
\begin{proposition}[\texorpdfstring{$\mathsf{NULL}$}{NULL} 的对象论判据：局部截面存在性]\label{prop:cdim-nonnull-section-criterion}
在上述站点/（预）层表述下，地址 \(a\) 的读出在类型层非 \(\mathsf{NULL}\) 当且仅当
\[
\boxed{\ F(a)\neq\varnothing\ }.
\]
并且对任意有限柱覆盖 \(C_a\subseteq \bigcup_{i=1}^{n}C_{a_i}\)，若存在局部对象 \(s_i\in F(a_i)\) 使其在任意交叠 \(C_{a_i}\cap C_{a_j}\) 上相容（在可核验等价意义下具有共同细化），则存在唯一的粘接对象 \(s\in F(a)\) 使其限制回各 \(s_i\)（粘接唯一性）。
\end{proposition}
\begin{proof}
第一条是 \(\mathsf{NULL}\) 的定义性译文：在地址 \(a\) 上，“读出可定义且可核验”被组织为存在某个可接受局部证书对象。
\par
第二条为可核验语义的粘接纪律：相容性以“在重叠上存在共同细化”表达（定义 \ref{def:xi-cert-refinement-equivalence}），从而可将 \((s_i)\) 压缩为一个由有限对象编码、能被确定性验证器复核的全局证书对象；唯一性同样由共同细化的可核验等价类唯一确定。证毕。
\end{proof}
...
在站点/（预）层表述下，\(\mathsf{NULL}\) 的来源可被统一为“局部截面不存在”的结构性失败；并且该失败在体系内可被分解为互不混淆的三类离线可核验见证（定理 \ref{thm:xi-null-complete-trichotomy-offline}）。
与三轴地址语义对齐，可将其表述为：
\begin{enumerate}
  \item \textbf{语义空值：}地址未给定或 \(a\notin A_{\mathrm{adm}}\)。此时 \(C_a\) 不存在（指称缺失），故 \(F(a)\) 无从定义或按约定为空，从而读出为 \(\mathsf{NULL}\)。
  \item \textbf{协议空值：}地址 \(a\in A_{\mathrm{adm}}\) 但协议/类型系统拒绝在 \(C_a\) 上建立读出（典型为 unitary slice 锁定与 \(\mathbf{PW}\) 闭包下的“无未记录连续轴”原则，定理 \ref{thm:xi-pw-no-continuous-hair}），或因模式轴/最小实现维数门槛被违反而触发维数型 \(\mathsf{NULL}\)（定理 \ref{thm:xi-completeness-iff-hankel-bounded}；注 \ref{rem:xi-acc-rank}），或局部对象虽逐片存在但因二阶一致性失败而无法全球化；其失败可由 \(H^2\) 粘合障碍类 \([\alpha]\neq 0\) 给出可核验见证（命题 \ref{prop:null-as-h2-obstruction}）。
  \item \textbf{碰撞空值：}地址 \(a\in A_{\mathrm{adm}}\) 且通过检查器，但底层读出存在多对一压缩而未外置足够残差轴使扩展读出单射化，从而仍等价表现为 \(F(a)=\varnothing\)。其最小侧信息预算由地址—缺陷定律（定理 \ref{thm:address-defect-law}）与分位配额阈值（推论 \ref{cor:pom-prime-register-quantile-budget}）给出；端点侧则由二次分辨率屏障（定理 \ref{thm:xi-offcritical-endpoint-resolution-lower-bound}）强制。
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{对象构造与粘接：}在有限柱覆盖上构造局部证书对象族并核验交叠相容性；若相容则由粘接唯一性得到唯一全局对象 \(s\in F(a)\) 并返回 \(s\)（命题 \ref{prop:cdim-nonnull-section-criterion}）。
\end{enumerate}
\end{algorithm}
...
\begin{itemize}
  ...
  \item 解除盲区的两条可审计路径分别是：外置可寻址前缀以共动局部化（消除二次红移），或进入可逆层析通道（把二维参数编码为一维指数谱并可反演，定理 \ref{thm:xi-comoving-horizon-scan-fourier-inversion}）；
...
解除盲区存在两条可审计路径：其一是外置可寻址前缀并实施共动局部化，使高度红移被消解为与偏移 \(\delta\) 同阶的线性可见量（命题 \ref{prop:cdim-comoving-defect-implies-delta-bound}，推论 \ref{cor:xi-comoving-prefix-bit-budget-gain}）；其二是进入可逆层析通道，把二维参数编码为一维指数谱并可由 Fourier 反演恢复（定理 \ref{thm:xi-comoving-horizon-scan-fourier-inversion}）。
