...
1.  **Infinite loot tables (ultraviolet divergence)**: If you want to calculate all possible particle interactions, and in continuous space, wavelengths can be infinitely short (infinitely high frequency), this means you need to calculate infinite energy. This is like a BOSS dropping infinite equipment after death, instantly freezing the client.

2.  **Data overflow (singularities)**: General relativity predicts that at the center of a black hole, matter density tends toward infinity. To a programmer, this is not a physical phenomenon at all, but a typical **Divide by Zero Error**. This is a sign of model collapse, not a feature of reality.
...
Here $l_P$ is the Planck length, which you can understand as **the smallest pixel in all of Azeroth**.
...
1.  **Discreteness (pixelation)**: The universe is not an oil painting on paper, but a bitmap displayed on a screen. Each Planck area unit ($l_P^2$) can store approximately 1/4 bit of information.

2.  **Finiteness (slot limit)**: No matter how you compress, the maximum number of states in a region is finite. Your bag is full when it's full; you can't stuff anything more in.
