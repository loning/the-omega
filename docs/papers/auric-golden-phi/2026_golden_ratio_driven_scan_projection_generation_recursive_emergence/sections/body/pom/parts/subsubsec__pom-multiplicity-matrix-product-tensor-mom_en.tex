...
\begin{proposition}[Strict Symmetrization under Binomial Weight Inner Product: Weighted Self-Adjointness of \texorpdfstring{$B_q$}{Bq}]\label{prop:pom-bq-weighted-selfadjoint-binomial}
Following the notation of Proposition \ref{prop:pom-disjointness-symmetric-compression-bq}, let \(B_q\in M_{q+1}(\ZZ)\) be the matrix satisfying \(B_q(i,j)=\binom{q-j}{i}\).
...
\]
so \(B_q\) is self-adjoint under the weighted inner product \(\langle u,v\rangle_{W_q}:=u^{\mathsf T}W_qv\).
Equivalently, let
\[
...
J_q:=D_q^{-1}B_qD_q,
\]
then \(J_q\) is a real symmetric matrix.
Therefore all spectrum of \(B_q\) are real numbers, and we can take a set of \(W_q\)-orthogonal eigenvector basis.
\end{proposition}
\begin{proof}
For any \(0\le i,j\le q\), from \(B_q(i,j)=\binom{q-j}{i}\) we obtain
\[
(W_qB_q)(i,j)=\binom{q}{i}^{-1}\binom{q-j}{i},
\qquad
(B_q^{\mathsf T}W_q)(i,j)=\binom{q-i}{j}\binom{q}{j}^{-1}.
\]
When \(i+j>q\) both are \(0\).
When \(i+j\le q\) direct simplification gives
\[
\binom{q}{i}^{-1}\binom{q-j}{i}
=\frac{(q-i)!(q-j)!}{q!(q-i-j)!}
=\binom{q}{j}^{-1}\binom{q-i}{j},
\]
so \(W_qB_q=B_q^{\mathsf T}W_q\).
On the other hand, from \(W_q=D_q^{-2}\), the above formula is equivalent to \(J_q^{\mathsf T}=J_q\), thus \(J_q\) can be orthogonally diagonalized; returning to similarity transformation yields the reality of the spectrum of \(B_q\) and the existence of a \(W_q\)-orthogonal eigenbasis.
\end{proof}
...
\begin{proposition}[Two Eigenlinear Forms Generate the Full Spectrum: Closed-Form Eigenvectors of \texorpdfstring{$B_q$}{Bq}]\label{prop:pom-bq-explicit-diagonalization-linear-forms}
Let \(\varphi=(1+\sqrt5)/2\), \(\psi=-\varphi^{-1}\), and define the linear forms
\[
\ell_+(x,y)=\varphi x+y,\qquad \ell_-(x,y)=\psi x+y.
\]
Then under the substitution action \((x,y)\mapsto(x+y,x)\) we have
\(
\ell_\pm(x+y,x)=\lambda_\pm\,\ell_\pm(x,y)
\), where \(\lambda_+=\varphi\), \(\lambda_-=\psi\).
For each \(k\in\{0,1,\dots,q\}\), let
\[
P_{q,k}(x,y):=\ell_+(x,y)^{\,q-k}\,\ell_-(x,y)^{\,k}\in\QQ(\sqrt5)[x,y]_q.
\]
Then \(P_{q,k}\) is an eigenvector of \(B_q\), and satisfies the strict eigenrelation
\[
\boxed{\;P_{q,k}(x+y,x)=(-1)^k\varphi^{\,q-2k}\,P_{q,k}(x,y)\;}
\]
with eigenvalue consistent with \(\mu_k\) of Proposition \ref{prop:pom-bq-is-symq-and-spectrum}.
Writing \(P_{q,k}=\sum_{j=0}^{q} v^{(k)}_j\,e_j\) in the basis \(e_j=x^{q-j}y^j\), the coordinate components have the closed form
\[
\boxed{\;
v^{(k)}_j
=\sum_{a=0}^{j}
...
\]
Moreover, by the \(W_q\)-self-adjointness of Proposition \ref{prop:pom-bq-weighted-selfadjoint-binomial} and the simplicity of the spectrum, the vector family \(\{v^{(k)}\}_{k=0}^{q}\) can be taken as a set of pairwise orthogonal eigenbasis under \(\langle\cdot,\cdot\rangle_{W_q}\).
\end{proposition}
\begin{proof}
By definition,
\[
...
\]
where \(\lambda_\pm\) satisfies \(\lambda_\pm^2=\lambda_\pm+1\), so \(\lambda_+=\varphi\), \(\lambda_-=\psi\).
Thus
\[
P_{q,k}(x+y,x)=\lambda_+^{\,q-k}\lambda_-^{\,k}\,P_{q,k}(x,y)=\varphi^{\,q-k}\psi^{\,k}\,P_{q,k}(x,y)=(-1)^k\varphi^{\,q-2k}P_{q,k}(x,y),
\]
giving the eigenrelation and eigenvalue.
Expanding
\(
(\varphi x+y)^{q-k}(\psi x+y)^k
\)
binomially and collecting by powers of \(y\) yields the stated closed form of \(v^{(k)}_j\).
Finally, by Proposition \ref{prop:pom-bq-weighted-selfadjoint-binomial}, \(B_q\) is self-adjoint under \(\langle\cdot,\cdot\rangle_{W_q}\); since the eigenvalues are pairwise distinct (Proposition \ref{prop:pom-bq-is-symq-and-spectrum}), eigenvectors corresponding to different eigenvalues must be pairwise orthogonal, thus can be taken as a \(W_q\)-orthogonal eigenbasis.
\end{proof}
