...
\begin{proposition}[规范差速率曲线的代数参数化与物理分支选择]\label{prop:fold-gauge-anomaly-rate-curve-param}
...
\]
此外，消元证书 $R(\alpha,u)=0$ 作为平面代数曲线包含多个代数分支；在 $u=1$ 处出现的另一个根 $\alpha=-\tfrac14$ 是\emph{非物理分支}，因为失配密度必须属于区间 $[0,1]$。因此可用以下规则选出物理分支：
\[
\mu(u)>0,\qquad \alpha(u)\in[0,1],\qquad (\mu(1),\alpha(1))=(2,4/9).
\]
该规则给出一个可审计的代数相图参数化 $(u,\alpha(u),I(\alpha(u)))$，并显式排除非物理根支。
\end{proposition}
