...
\begin{theorem}[prime-register 向量的 Gödel 整数化具有不可避免的对数开销]\label{thm:xi-godel-integerization-log-overhead}
设 \(\tau=(a_1,\dots,a_T)\) 为长度 \(T\) 的事件码序列，且 \(a_t\in\NN_{\ge1}\)。
定义 Gödel 整数
\[
G(\tau):=\prod_{t=1}^{T}p_t^{a_t},
\]
其中 \(p_t\) 为第 \(t\) 个素数。则存在绝对常数 \(c_0>0\) 与 \(T_0\)，使对 \(T\ge T_0\) 有
\[
\log_2 G(\tau)\ge c_0\,T\log_2 T.
\]
因而，把长度 \(T\) 的寄存器向量压成单整数的位长下界为 \(\Omega(T\log T)\)。
\end{theorem}
\begin{proof}
由 \(a_t\ge 1\) 得
\[
\log G(\tau)\ge \sum_{t=1}^{T}\log p_t=\vartheta(p_T),
\]
其中 \(\vartheta(x)=\sum_{p\le x}\log p\) 为 Chebyshev 函数。
由素数定理的等价形式 \(\vartheta(x)\sim x\) 与 \(p_T\sim T\log T\)，可得
\[
\sum_{t=1}^{T}\log p_t\sim T\log T.
\]
换底到 \(\log_2\) 即得结论。证毕。
\end{proof}
...
\ \ge\
\frac{c_0\,T\log_2 T}{\lceil\log_2 d_m(X)\rceil+O(1)},
\]
其中 \(c_0>0\) 为定理 \ref{thm:xi-godel-integerization-log-overhead} 的常数。
...
\(\log_2 G(\tau(\omega))\ge c_0T\log_2 T\)（\(T\) 充分大）。
