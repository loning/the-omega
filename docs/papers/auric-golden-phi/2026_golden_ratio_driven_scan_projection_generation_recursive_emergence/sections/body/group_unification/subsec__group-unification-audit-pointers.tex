...
\)。
另一方面，推论 \ref{cor:terminal-foldbin6-geo-stabilizer-z2} 给出 window-\(6\) 上唯一非平凡几何对合（等价地：一个可审计的 \(\ZZ_2\) sheet parity 约束）；
将其在连续包络的最小化口径下视为对右手块 \(\mathfrak{su}(2)_R\) 的非平凡对合自同构，则其不动点 Lie 代数必为一维。
事实上 \(\mathfrak{su}(2)\cong\mathfrak{so}(3)\)，任一非平凡对合在 \(\mathfrak{so}(3)\) 上共轭于关于某一轴的反射，其不动点恰为该轴的转动方向，故同构于 \(\mathfrak u(1)\)。
取该不动点方向为 \(\mathfrak u(1)_{\mathrm{eff}}\subset \mathfrak{su}(2)_R\) 即得结论。证毕。
\end{proof}
