...
\[
d_{\mathrm{add}}(K):=\cdim(\mathcal O_K,+),\qquad
d_{\mathrm{mult}}(K):=\cdim(\mathcal O_K^\times).
\]
则 \(d_{\mathrm{add}}(K)\) 与 \(d_{\mathrm{mult}}(K)\) 唯一确定签名 \((r_1,r_2)\)，且
\[
r_2=d_{\mathrm{add}}(K)-d_{\mathrm{mult}}(K)-1,\qquad
r_1=2d_{\mathrm{mult}}(K)+2-d_{\mathrm{add}}(K).
\]
\end{theorem}
\begin{proof}
作为 \(\ZZ\)-模，
...
\]
代回 \(n=d_{\mathrm{add}},u=d_{\mathrm{mult}}\) 即得结论。证毕。
\end{proof}
...
\begin{enumerate}
  ...
  \[
  d_{\mathrm{add}}(K)=d_{\mathrm{mult}}(K)+1.
  \]
  \item \(K\) 全复（\(r_1=0\)）当且仅当
  \[
  d_{\mathrm{add}}(K)=2d_{\mathrm{mult}}(K)+2.
  \]
  \item 复嵌入数满足
  \[
  r_2=d_{\mathrm{add}}(K)-d_{\mathrm{mult}}(K)-1.
  \]
\end{enumerate}
\end{corollary}
\begin{proof}
...
\input{subsubsec__conclusion-phase-channel-crowding-singlephase-ucdim}
