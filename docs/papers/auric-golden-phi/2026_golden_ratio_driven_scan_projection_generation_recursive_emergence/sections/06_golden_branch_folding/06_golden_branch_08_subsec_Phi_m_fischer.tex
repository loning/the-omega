...
\begin{proposition}[右 Fischer cover 的最小性（标准）]\label{prop:fischer-minimal}
对 $Y_m$，存在一个右解析且 follower-separated 的标记图表示 $H_m^{\mathrm{Fis}}$（右 Fischer cover），其同构类由 $Y_m$ 唯一确定；并且它在所有呈现 $Y_m$ 的右解析表示中状态数最小。更具体地，对任意右解析呈现 $G$（其标记移位为 $Y_m$），存在一个满射的标记图态射 $G\twoheadrightarrow H_m^{\mathrm{Fis}}$。
\end{proposition}

\begin{proof}
这是 Fischer cover 的标准结论；可由“确定化 + follower 最小化”构造得到，并满足最小性与唯一性。参见 \cite{LindMarcus1995,Kitchens1998SymbolicDynamics}。
\end{proof}
