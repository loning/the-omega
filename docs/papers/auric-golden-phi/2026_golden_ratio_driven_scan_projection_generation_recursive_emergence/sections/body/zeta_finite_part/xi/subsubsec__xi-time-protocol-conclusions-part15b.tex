...
\begin{conclusion}[有限窗 Sturm 实根交错：\texorpdfstring{$m\le 60$}{m<=60} 时 \texorpdfstring{$Z_m(y)$}{Z_m(y)} 的负实根性]\label{con:xi-terminal-zm-sturm-real-roots-interlacing-m60}
对每个整数 \(1\le m\le 60\)，多项式 \(Z_m(y)\in\ZZ[y]\) 具有 \(\lceil m/2\rceil\) 个互异实根，且全部位于 \((-\infty,0)\)。
并且对每个 \(0\le m\le 58\)，\(Z_m\) 与 \(Z_{m+2}\) 的零点集合严格交错；因此固定奇偶类的两条子序列 \(\{Z_{2k}\}_{k\ge 0}\) 与 \(\{Z_{2k+1}\}_{k\ge 0}\)（在该核验窗内）各自形成一条 Sturm 实根链。
\par
...
\begin{conjecture}[全尺度 Sturm 实根交错]\label{conj:xi-terminal-zm-sturm-real-roots-interlacing}
...
若能把 \(\{Z_m(y)\}\) 实现为某个平面网络的路径权和（等价地，某个带状全正矩阵/振荡矩阵的主子式链），则全正性与 Sturm 理论可推出负实根性与交错。
本体系中已核验的二维局域递推
\[
...
\paragraph{谱曲线的椭圆化与分歧单值化}
<<<<<<< Current (Your changes)
下列结论以配分函数序列 \(Z_m(y)\) 及其谱多项式 \(\Pi(\lambda,y)\) 为出发点，并以判别式分解 \(\mathrm{Disc}_{\lambda}\bigl(\Pi(\lambda,y)\bigr)=-y(y-1)\bigl(256y^3+411y^2+165y+32\bigr)\) 及唯一负实临界值 \(y_{\mathrm{LY}}\) 为既有事实（结论 \ref{con:xi-terminal-zm-order4-rational}，\ref{con:xi-terminal-zm-discriminant-leyang}）；相关推导细节可参见日期为 \(2026\text{-}02\text{-}17\) 的内部文献 \cite{Internal2026TerminalZmLeeYangEllipticS4Audit}。
=======
下列结论以配分函数序列 \(Z_m(y)\) 及其谱多项式 \(\Pi(\lambda,y)\) 为出发点，并以判别式分解 \(\mathrm{Disc}_{\lambda}\bigl(\Pi(\lambda,y)\bigr)=-y(y-1)\bigl(256y^3+411y^2+165y+32\bigr)\) 及唯一负实临界值 \(y_{\mathrm{LY}}\) 为既有事实（结论 \ref{con:xi-terminal-zm-order4-rational}，\ref{con:xi-terminal-zm-discriminant-leyang}）；相关推导细节参见内部文献 \cite{Internal2026TerminalZmLeeYangEllipticS4Audit}。
>>>>>>> Incoming (Background Agent changes)
其核心是：谱曲线 \(\Pi(\lambda,y)=0\) 的归一化为判别式 \(37\) 的固定椭圆曲线，重量投影 \(y\) 为其上的次数 \(4\) 有理函数，其分歧像在整系数层面由同一 Lee--Yang 三次因子唯一锁定。
...
上述椭圆化—分歧链条在结构上形成闭合：结论 \ref{con:xi-terminal-zm-elliptic-normalization} 将谱四次曲线的规范化识别为判别式 \(37\) 的固定椭圆曲线；
结论 \ref{con:xi-terminal-zm-elliptic-y-hurwitz-passport} 将 Lee--Yang 判别核解释为次数 \(4\) 覆叠的分歧像；
结论 \ref{con:xi-terminal-zm-leyang-ramification-critical-values} 与结论 \ref{con:xi-terminal-zm-leyang-cubic-field-isomorphism} 给出临界点与临界值的统一参数化，并实现 Lee--Yang 三次域的规范同构；
结论 \ref{con:xi-terminal-zm-generic-galois-s4} 与结论 \ref{con:xi-terminal-zm-s4-splitting-curve-genus16} 确定泛型单值群为 \(S_4\)，并给出其伽罗瓦闭包曲线的属；
结论 \ref{con:xi-terminal-zm-s4-modp-splitting-density} 与结论 \ref{con:xi-terminal-zm-cover-minimal-bad-reduction-set} 进一步把 \(S_4\) 单值性推送到模素分解型统计与整系数约化层面的最小坏约化控制，而结论 \ref{con:xi-terminal-zm-hecke-eigenvalue-fiber-deviation} 将相应 Hecke 本征值译码为纤维根数对均值的总偏差；
结论 \ref{con:xi-terminal-zm-leyang-ramification-divisor-principalization-trace} 将 Lee--Yang 三分歧点的加法迹提升为 \(E(\QQ)\) 中的显式无限阶元；
结论 \ref{con:xi-terminal-zm-branch-puiseux-uniformization} 则给出分歧处 Puiseux 首项系数的闭式表达，从而把临界平方根分歧的定量振幅固定为可审计的代数常数。

综上，实 Lee--Yang 边界 \(y_{\mathrm{LY}}\) 不仅是零点凝聚的最近实奇点，而且在算术层面与导子 \(37\) 的模椭圆曲线 \(E\) 及其八面体单值化同时对齐：
分歧集合由 \(P_{\mathrm{LY}}(y)\) 锁定，泛型单值群为 \(S_4\)，而坏约化素数与临界判别式的平方自由部分均由 \(37\) 标记；并且倍点 Latt\`es 自映射在该素数处出现稳定约化降阶，与 Lee--Yang 分歧的重根塌缩同步（结论 \ref{con:xi-terminal-zm-elliptic-lattes-badprime-37-degree-drop}）。
结合注记 \ref{rem:xi-terminal-elliptic-37a1-modular-anchor} 的模性锚定，
由 Lee--Yang 递推所携带的谱—分歧数据可在本文的“物理实现”语义下（参见 \cite{KapustinWitten2006GeometricLanglands}）提升为一个 \(\mathrm{GL}_2\) 自守对象的局部—整体数据包，从而给出一条完全由有限维谱关系内禀导出的可核验范例。
