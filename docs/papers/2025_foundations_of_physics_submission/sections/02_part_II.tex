...
\end{equation}
where $\hat{W}_s$ are isometric "inflation tensors" and $\hat{D}_s$ are local unitary "disentanglers". In Appendix \ref{app:isometry}, we rigorously define these as \textbf{Perfect Tensors} and prove the exact recoverability conditions.
...
\begin{enumerate}
    ...
    \item \textbf{Code Distance}: The topological protection of the bulk is guaranteed by the code distance $d \sim L^{\frac{1}{\phi}}$. The error term $\mathcal{E}$ is exponentially suppressed as $e^{-d \cdot \ln \chi}$ (see Proposition 3 in Appendix C).
\end{enumerate}
