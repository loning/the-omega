...
To test whether the stop-context Uplift signal replicates across phylogenetically diverse species, we perform random-effects meta-analysis on 18 Tier-1 species from three domains (Eukarya, Bacteria, Archaea) using annotated CDS sequences from NCBI RefSeq.
...
\paragraph{UTR-inclusive eukaryotic replication (H2-1b).}
To partially address the CDS-only limitation, we additionally analyze UTR-inclusive RefSeq transcript sequences for the eukaryotic tier using the \texttt{*\_rna\_from\_genomic.fna.gz} files.
For each transcript we select a best ORF (AUG start; canonical stops) and compute the uplift difference $\Delta U := U_{\mathrm{after}}-U_{\mathrm{before}}$ at the terminal stop with the same window radius.
At $k=10$ codons, the cross-species random-effects meta-analysis on the $\Delta U$ contrast between $\mathrm{UAA}$ and $\mathrm{UGA}$ yields a small but nonzero effect with high heterogeneity:
\input{sections/generated/cross_species_stop_context_mrna_eukarya_k10}
This indicates that incorporating the downstream (UTR) window does not remove heterogeneity, but does provide a reproducible, directionally consistent endpoint within this eukaryotic subset.
...
\caption{Uplift trace $U(i)$ along the best-ORF of HBB (\texttt{NM\_000518.5}) under $\mu^\ast$.}
...
\caption{Uplift trace $U(i)$ along the best-ORF of INS (\texttt{ENST00000381330.5}) under $\mu^\ast$.}
