...
\begin{corollary}[有限维 Ramanujan 证书序列 \texorpdfstring{$\Rightarrow$}{=>} RH 的可审计路线]\label{cor:xi-finite-ramanujan-carath-implies-rh}
若存在一列有限正原子测度 \(\mu_N\)（\(N\ge 1\)）满足自对偶对称
\[
\mu_N\circ(\xi\mapsto\xi^{-1})^{-1}=\mu_N,
\]
并定义其 Carath\'eodory 变换
\[
\mathscr{C}_N(w):=\int_{\TT}\frac{\xi+w}{\xi-w}\,\dd\mu_N(\xi),\qquad w\in\mathbb{D},
\]
且 \(\mathscr{C}_N\to \mathscr{C}_{\zeta}\) 在 \(\mathbb{D}\) 的每个紧子集上一致收敛，则 RH 成立。
\end{corollary}

\begin{proof}
由 \eqref{eq:app-horizon-herglotz-disk} 的一般理论（或直接由核 \(\frac{\xi+w}{\xi-w}\) 的正实部性），每个 \(\mathscr{C}_N\) 均为 Carath\'eodory 函数。
由定理 \ref{thm:xi-horizon-carath-closure} 得 \(\mathscr{C}_{\zeta}\) 亦为 Carath\'eodory；再由推论 \ref{cor:app-horizon-carath-rh} 即推出 RH。证毕。
\end{proof}
...
而定理 \ref{thm:xi-horizon-carath-closure} 则以正性闭包给出一条并行的极限转移原则：若能构造一列有限维 Carath\'eodory/Ramanujan 对象在 \(\mathbb{D}\) 内紧致一致收敛到 \(\mathscr{C}_{\zeta}\)，则 RH 由推论 \ref{cor:app-horizon-carath-rh} 直接推出。
