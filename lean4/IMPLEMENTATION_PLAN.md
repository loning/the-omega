...
**crossWeightCorrelation + E00 递推与望远镜和（Round 78，Phase 81）**：Folding/FiberWeightCount.lean（拓展至 764 行，接近 800 行上限，下轮需拆分）——crossWeightCorrelation（def:pom-crossWeightCorrelation：Σ_{r<F_{m+2}} ewc(m,r)·ewc(m,r+F_{m+2})，交叉权重相关定义）; exactWeightCollision_succ（thm:pom-e00-succ：E00(m+1) = E00(m) + S_2(m)，通过 collision_same_eq_exactWeightCollision + momentSum_two_eq_collision + 末位 (b1,b2) 四类分划 + truncate 双射，weight < 2F_{m+2} 约束下 mod 等价为有界整数等式，hmod_split 三路分解，htt 取消等量项，omega 合并）; exactWeightCollision_eq_sum（thm:pom-e00-telescoping：E00(m) = 1 + Σ_{k<m} S_2(k)，强归纳，base m=0 直接展开 ewc(0,0)^2+ewc(0,1)^2=1，step exactWeightCollision_succ + Finset.sum_range_succ + ring）——POM 覆盖率 ~19.0% → ~19.2%（+3 条目）（Phase 81）
