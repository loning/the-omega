# -*- coding: utf-8 -*-
"""
Render the concrete wiring/folding geometry for the best *full-graph* GI match.

Reads:
  figures/adaptive/sm_hilbert_isomorphism/data/sm_hilbert_gi_search_bestcurve_report.json
  field: best_match_signature_equal_full

Outputs:
  figures/adaptive/sm_hilbert_isomorphism/wiring_fold_geometry_full_gi/
    - wiring_2d.svg
    - wiring_3d_xy.svg
    - wiring_3d_xz.svg
    - wiring_3d_yz.svg
...
def _read_best_match_full() -> Dict[str, Any]:
    in_json = (
        figures_dir()
        / "adaptive"
        / "sm_hilbert_isomorphism"
        / "data"
        / "sm_hilbert_gi_search_bestcurve_report.json"
    )
    obj = json.loads(in_json.read_text(encoding="utf-8"))
    bm = obj.get("best_match_signature_equal_full")
    if not isinstance(bm, dict):
        raise RuntimeError("No best_match_signature_equal_full in report (run exp_hilbert_sm_gi_search_bestcurve.py first).")
    return bm
...
def main() -> None:
    bm = _read_best_match_full()
    m_sched = _as_int_dict(bm.get("m_schedule_by_k", {}))
    choice2 = _as_int_dict(bm.get("choice2", {}))
    choice3 = _as_int_dict(bm.get("choice3", {}))
    cons_d = bm.get("constraints", {})
    ...
    out_dir = figures_dir() / "adaptive" / "sm_hilbert_isomorphism" / "wiring_fold_geometry_full_gi"
    ...
    _plot_polyline_2d(xs2, ys2, out_dir / "wiring_2d.svg", title="Wiring/Folding geometry (2D) [full GI exact]")
    ...
    _plot_polyline_2d(xs3, ys3, out_dir / "wiring_3d_xy.svg", title="Wiring/Folding geometry (3D, XY projection) [full GI exact]")
    _plot_polyline_2d(xs3, zs3, out_dir / "wiring_3d_xz.svg", title="Wiring/Folding geometry (3D, XZ projection) [full GI exact]")
    _plot_polyline_2d(ys3, zs3, out_dir / "wiring_3d_yz.svg", title="Wiring/Folding geometry (3D, YZ projection) [full GI exact]")
