...
\[
\Phi:E_1\times E_2\times E_3\times E_4\longrightarrow \Jac(X),\qquad
(P_1,\dots,P_4)\longmapsto \sum_{i=1}^4 \pi_i^*(P_i-O_i).
\]
令 \(A_{V_3}:=\operatorname{Im}(\Phi)\)。则：
\begin{enumerate}
  \item \(A_{V_3}\) 为 \(\Jac(X)\) 的 \(S_4\)-稳定阿贝尔子簇，其余切表示同构于标准不可约表示 \(V_3\)；
...
\paragraph{公开检索说明}
针对上述结论涉及的关键词组合（包括 \(A_4\)-子覆盖单点分歧、\(S_3\)-子覆盖在亏格 \(1\) 底曲线上的 \(10\) 点简单分歧、以及四个椭圆商生成标准表示因子 \(E^3\)）进行公开检索时，可检索材料主要给出单点分歧覆盖的一般存在性与模空间背景，未见与本文覆盖塔及其分歧—表示分解同时一致的公开陈述。
该观察仅构成“未发现直接重复”的经验性证据，不构成全域排他性证明。可参见一般背景文献 \url{https://arxiv.org/pdf/1408.0859}。
