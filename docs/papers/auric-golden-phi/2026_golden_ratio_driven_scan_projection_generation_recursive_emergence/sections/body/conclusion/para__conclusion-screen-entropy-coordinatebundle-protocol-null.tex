...
定义最优二进审计编码
\[
\Enc_S(x):=\bigl(f_S(x),\kappa_S(x)\bigr),
\]
其中 \(\kappa_S(x)\in \FF_2^{\,r_S}\) 表示 \(x\) 在上述分解下的核坐标。则存在仅依赖于 \(S,m,n\) 与所选通用机/分解的常数 \(C_S\)，使得对任意 \(x\in C_n(\mathcal Q_m;\FF_2)\) 都有
\[
\bigl|K(x\mid m,n,S)-K(\Enc_S(x)\mid m,n,S)\bigr|\le C_S,
\]
并且
\[
K\bigl(x\mid \Enc_S(x),m,n,S\bigr)\le C_S.
\]
...
由构造，\(\Enc_S\) 是可计算映射，故
\[
K(\Enc_S(x)\mid m,n,S)\le K(x\mid m,n,S)+O(1).
\]
...
\]
故从 \(\Enc_S(x)\) 恢复 \(x\) 只需执行固定的有限维线性代数运算，因而其逆映射在像上可计算。于是
\[
K(x\mid m,n,S)\le K(\Enc_S(x)\mid m,n,S)+O(1),
\qquad
K\bigl(x\mid \Enc_S(x),m,n,S\bigr)\le O(1).
\]
合并即得前两式。
...
\[
\Read_S(y)\in \{\mathrm{Tot},\mathrm{ProtNULL},\mathrm{CollNULL}\}
\]
为
\[
\Read_S(y)=
...
则有严格分类：
\[
\Read_S(y)=\mathrm{ProtNULL}
\iff
y\notin \operatorname{im}f_S,
\]
\[
\Read_S(y)=\mathrm{CollNULL}
\iff
...
定义其决定时刻
\[
\tau:=\inf\{t\ge 0:\Read_{S_t}(y_t)=\mathrm{Tot}\}.
\]
则 \(\tau\) 与 \(x\) 无关，并且有显式公式
\[
...
\begin{enumerate}
  \item 若 \(t<\tau\)，则
        \[
        \Read_{S_t}(y_t)=\mathrm{CollNULL},
        \qquad
        \#f_{S_t}^{-1}(y_t)=2^{\,c(\Gamma_{S_t})-1};
        \]
  \item 若 \(t\ge \tau\)，则
        \[
        \Read_{S_t}(y_t)=\mathrm{Tot},
        \qquad
        f_{S_t}^{-1}(y_t)=\{x\};
        \]
