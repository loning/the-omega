...
上一节已把 $Y_m$ 组织为标记图 $G_m$ 的像（定理 \ref{thm:Phi_m-sofic-graph}），并证明熵率不降（推论 \ref{cor:Phi_m-entropy-no-drop}）。
接下来给出一个把“未来工作：研究 $\Phi_m$ 的可逆性/纤维结构”直接封口的硬结论：在 Zeckendorf 折叠 $\Fold_m$ 的口径下，除去 $m=2$ 的两点退化外，$\Phi_m$ 在 $m\ge 3$ 时实际上是共轭。
...
当 $S_t$ 为单点集时，意味着该长度-$t$ 输出块已把内部后缀同步到唯一值。
...
其中 $m\ge 3$ 时“non-singleton ends”为 $0$，而 $m=2$ 时出现唯一一个非单点端态，对应常值输出块所导致的两点退化。
...
$$
因此当 $m\ge 3$ 时，$Y_m$ 自身的 prime-orbit/\(\zeta\) 指纹完全退化为全移位骨架；本文中出现的非平凡 prime shadow 必然来自“核 + 势/权重 + 扭曲通道”（而非单纯来自折叠因子 $Y_m$）。
\end{corollary}
