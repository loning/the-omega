...
\begin{conclusion}[例外幂和的全阶组合分解：循环子图独立集矩与 \texorpdfstring{$\Omega_m$}{Omega\_m} 修正]\label{con:xi-terminal-replica-softcore-exceptional-power-sum-cycle-sector-replacement}
固定 \(q\ge2\)。记
\[
S_m(q):=\sum_{i=0}^{q}\bigl(\lambda_i^{(q)}\bigr)^m=\Tr\!\bigl((A^{(q)})^m\bigr),\qquad m\ge1.
\]
对任意 \(S\subseteq E(C_m)\)，定义
\[
I(S):=\#\Ind\!\bigl(C_m[S]\bigr)=I\!\bigl(C_m[S]\bigr).
\]
则对一切 \(m\ge1\) 有严格恒等式
\[
\boxed{\;
2^mS_m(q)=\Omega_m(q)+\sum_{\substack{S\subseteq E(C_m)\\ S\neq E(C_m)}} I(S)^{\,q}\; } ,
\]
亦即
\[
\boxed{\;
S_m(q)=2^{-m}\!\left(\Omega_m(q)+\sum_{S\subsetneq E(C_m)} I(S)^{\,q}\right)\; }.
\]
\end{conclusion}
\begin{proof}
由结论 \ref{con:xi-terminal-replica-softcore-trace-moment-cycle-subgraph-independent-sets}，
\[
2^m\Tr\!\bigl((T^{(q)})^m\bigr)=\sum_{S\subseteq E(C_m)}I(S)^q.
\]
另一方面，结论 \ref{con:xi-terminal-replica-softcore-trace-exceptional-difference} 给出
\[
\Tr\!\bigl((T^{(q)})^m\bigr)-S_m(q)=2^{-m}\bigl(I(C_m)^q-\Omega_m(q)\bigr).
\]
将第一式代入并移项，可得
\[
2^mS_m(q)=\Omega_m(q)+\sum_{S\subseteq E(C_m)}I(S)^q-I(C_m)^q.
\]
注意 \(I(C_m)=I(E(C_m))\)，即全边子图 \(S=E(C_m)\) 对应的唯一扇区，
故上式正是
\[
2^mS_m(q)=\Omega_m(q)+\sum_{S\subsetneq E(C_m)}I(S)^q.
\]
两边同除 \(2^m\) 即得第二式。证毕。
\end{proof}
