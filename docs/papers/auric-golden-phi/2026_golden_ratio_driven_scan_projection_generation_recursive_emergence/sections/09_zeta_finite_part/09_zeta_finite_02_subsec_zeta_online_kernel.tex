...
\begin{corollary}[Big-Witt 同余护栏：necklace 整除律与 Dwork 型 \texorpdfstring{$p$}{p}-幂同余]\label{cor:big-witt-audit-guardrails}
...
\end{corollary}
\begin{proof}
直接应用附录命题 \ref{prop:witt-necklace-dwork-R}。证毕。
\end{proof}
...
\begin{proposition}[完成化零点的对偶--共轭对称（核版 RH 的基本域）]\label{prop:completed-zero-symmetry}
...
\end{proposition}
\begin{proof}
（1）由 \(\Delta(z,u)=\Delta(uz,1/u)\) 得
\[
\widehat{\Delta}(w,1/u)
=(1/u)^{-N/2}\Delta(w(1/u)^{-1/2},1/u)
=u^{N/2}\Delta(wu^{1/2},1/u).
\]
再用 \(\Delta(z,1/u)=\Delta((1/u)z,u)\) 得 \(\Delta(wu^{1/2},1/u)=\Delta(wu^{-1/2},u)\)，从而
\(\widehat{\Delta}(w,1/u)=u^{N}\widehat{\Delta}(w,u)\)，零点集一致。

（2）由于 \(\Delta\) 的系数在 \(\ZZ[u]\)（从而满足 \(\overline{\Delta(z,u)}=\Delta(\overline{z},\overline{u})\)），若 \(\widehat{\Delta}(w_0,u)=0\) 则 \(\widehat{\Delta}(\overline{w_0},\overline{u})=0\)。
当 \(|u|=1\) 时 \(\overline{u}=1/u\)，由（1）零点集对 \(u\leftrightarrow 1/u\) 不变，故 \(\widehat{\Delta}(\overline{w_0},u)=0\)。

（3）由（1）与（2）直接得到闭合性与基本域约化。证毕。
\end{proof}
...
\end{proposition}
\begin{proof}
由命题 \ref{prop:sync-bivariate-euler-product}，每个 \(p_{n,k}>0\) 对应一条 primitive 回路 \(\tau\) 满足 \((|\tau|,S_\tau\varphi)=(n,k)\)，故 \(k/n\) 必为某条回路（从而某条有向环）的平均能量密度；反之任意有向环分解为 primitive 回路，其平均能量位于这些 primitive 分量的凸包内，故 \(\alpha_{\min},\alpha_{\max}\) 等于所有回路平均能量的极值。
端点等式与 \(P(\theta)=\log\rho(B(e^\theta))\) 在 \(\theta\to\pm\infty\) 的主导项由极端平均回路给出是有限图热力学形式的标准结论（可由 max-plus Perron--Frobenius 或直接比较回路权重得到）；组合可计算性由 Karp 算法给出。证毕。
\end{proof}
