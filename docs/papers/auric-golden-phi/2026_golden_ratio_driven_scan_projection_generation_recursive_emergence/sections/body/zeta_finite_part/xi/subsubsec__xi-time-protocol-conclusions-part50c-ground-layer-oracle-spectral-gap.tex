...
\paragraph{零温极端层的显式尺度与端点几何}
上述诸条结果表明：\(\log c\) 同时控制了极端饱和层的最小完备外置率与无偏采样等待代价；\(u_R\) 与 \(c^2=\rho(B_\infty)\) 之间的有理可审计谱缝说明 RH/Ramanujan 失效严格先于零温四次 Perron 尺度；而归一化速率函数在两端又分别呈现主系数为 \(1\) 与 \(2\) 的对数尖点，从而在本文固定的 \(\alpha\)-坐标下排除了任何端点交换的主导阶对称。与此同时，\(\Delta_\mu\) 继续刻画极端层与平衡均值之间的线性代数距离，却并不消灭其正的指数熵。于是极端层的可见性、谱窗口的破裂与端点大偏差几何不再是彼此分离的现象，而是同一零温 witness 结构的互补投影。
