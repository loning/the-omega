...
以 \(g_\ast(p)\)、\(\sigma_G^2(p)\) 的闭式表达、\(\chi_{P(p)}(\lambda)\) 的完全分解以及压力四次方程 \eqref{eq:fold-gauge-anomaly-bernoulli-p-pressure-quartic} 的系数结构为检索词，对公开资料作关键词与精确字符串检索，未见与本节同型的直接表述。
