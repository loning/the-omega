...
\begin{enumerate}
  ...
  \item \textbf{均匀基线：}在审计窗口 \(m\le 20\) 的精确枚举中，均匀输入下的平均规范差 \(\bar G_m=\mathbb{E}G_m(\omega)\) 满足 \(\bar G_m/m\) 上升并在 \(m=20\) 达到约 \(0.4176\)（注 \ref{rem:fold-gauge-anomaly-typical} 与表 \ref{tab:fold_gauge_anomaly_stats}）。
\end{enumerate}
