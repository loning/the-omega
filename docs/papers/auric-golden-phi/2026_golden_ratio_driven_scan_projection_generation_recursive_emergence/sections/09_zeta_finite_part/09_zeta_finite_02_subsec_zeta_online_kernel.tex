...
\begin{remark}[可复现输出：\texorpdfstring{$m=3,\dots,8$}{m=3..8} 的 \texorpdfstring{$R_m$}{Rm} 与 \texorpdfstring{$\rho_m$}{rhom}]\label{rem:sync-cyclotomic-elimination-table}
脚本 \texttt{scripts/exp\_sync\_kernel\_cyclotomic\_elimination.py} 由附录 \ref{app:pressure-analytic} 的 $F(\lambda,u)$ 构造 $\widehat{\Delta}(w,s)$，并对 $m=3,4,5,6,7,8$ 计算 $R_m(w)$ 与 $\rho_m$；输出见表 \ref{tab:sync_kernel_cyclotomic_elimination_summary} 与多项式块 \texttt{tab\_sync\_kernel\_cyclotomic\_elimination\_polys.tex}（可直接回归测试）。
\end{remark}
