...
以及稳定语法的前缀投影
\[
\rho:X_{m+1}\twoheadrightarrow X_m,\qquad
(x_1\cdots x_{m+1})\mapsto(x_1\cdots x_m).
\]
则相容关系
\[
\rho\circ\Fold_{m+1}=\Fold_m\circ\pi
\]
成立。
记纤维多重度
\[
...
=
\prod_{w\in X_m}
\binom{2d_m(w)}{\bigl(d_{m+1}(u)\bigr)_{u\in\rho^{-1}(w)}}
\in\NN.
\;}
\]
...
=
\sum_{w\in X_m}
\mathrm{Carr}_p\!\Bigl(\bigl(d_{m+1}(u)\bigr)_{u\in\rho^{-1}(w)}\Bigr),
\;}
\]
...
\begin{proof}
令 \(h:=\rho\circ\Fold_{m+1}=\Fold_m\circ\pi:\Omega_{m+1}\twoheadrightarrow X_m\)。
由于 \(\pi^{-1}(\omega)\) 对每个 \(\omega\in\Omega_m\) 恰含两点，且 \(h\) 仅依赖于前缀 \(\pi(\cdot)\)，故对任意 \(w\in X_m\) 有
\[
\card{h^{-1}(w)}=2\,\card{\Fold_m^{-1}(w)}=2d_m(w).
\]
对满射 \(\Fold_{m+1}:\Omega_{m+1}\twoheadrightarrow X_{m+1}\) 与 \(\rho:X_{m+1}\twoheadrightarrow X_m\) 应用定理 \ref{thm:fiber-gauge-volume-chain-factorization}，
