...
对 sofic shift，存在标准的\textbf{右解析}标记图表示，并可进一步最小化为右 Fischer cover。本文只需要其“可计算接口”与两个结构性结论；其完整构造（子集确定化 + follower 最小化）属于符号动力系统的标准内容（\cite{LindMarcus1995,Kitchens1998SymbolicDynamics}）。
...
本节把推论 \ref{cor:Ym-ambiguity-shell-low-entropy} 的“歧义壳低熵”进一步细化为一个更强、可审计的结构命题：在本文的 Zeckendorf 折叠下（$m\ge 3$），歧义不是可循环的“二义语法”，而是\textbf{有限步同步的纯瞬态层}；因此它不承载任何周期点，也不改变内禀 \(\zeta_{Y_m}\) 的周期点谱（见推论 \ref{cor:Phi_m-zeta-closed}）。
...
\par
更一般地，条件 \(Y_m^{\mathrm{amb}}=\varnothing\) 可在确定化状态图上提升为一个\textbf{可计算的同步延迟定理}：
...
从而：
\begin{enumerate}
  \item \textbf{期望同步时间有限：}\(\mathbb{E}_{\nu_m}[T_m]<\infty\)，并且通常规模为 \(O(1/\varepsilon_m)\)。
...
\nu_m(T_m>n)
\le
\sum_{w\in\mathcal{W}_{m}^{\mathrm{unsync}}(n)}\nu_m([w])
\le
|\mathcal{W}_{m}^{\mathrm{unsync}}(n)|\cdot C_m^{\mathrm{cyl}}2^{-n}
\le
