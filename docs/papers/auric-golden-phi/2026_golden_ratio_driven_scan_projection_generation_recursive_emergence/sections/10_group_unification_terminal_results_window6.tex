...
\begin{enumerate}
  ...
  该直方图的递归尺度由 \(\varphi\) 控制，因而适合作为跨分辨率比较的无量纲不变量（附录 \ref{app:high-dimensional-cut-project-interface} 与附录 \ref{app:cut_project_background}）。
  \end{proposition}
...
\begin{itemize}
  ...
  附录 \ref{app:high-dimensional-cut-project-interface}；附录 \ref{app:cut_project_background}（注 \ref{rem:app-cutproject-phason-time}）。
\end{itemize}
