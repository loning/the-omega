...
本节沿用第 \ref{sec:preliminaries} 节小节 \ref{subsec:freq-setting} 中固定的保测系统读出口径。
\par
...
\]
另一方面，黄金均值约束 $X_\infty$ 的标准最大熵平稳分布由 Perron--Frobenius 本征结构给出（见第 \ref{sec:group-unification} 节及注 \ref{prop:parry-measure-golden}；参见 \cite{LindMarcus1995}），其在长度 $m$ 上诱导的柱分布记为 $\pi_m$。于是可用如下两个无量纲偏差作为“拟合优度”：
\[
