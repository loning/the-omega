\paragraph{整除链二分律、约数格指标容斥与线性零点增量}
在零余类族的有限交截判据与谱零点集合的加权 clique--Euler 公式已经建立之后，还可继续沿约数格方向抽取出下列三条直接后果。它们分别刻画整除链上的严格嵌套--互斥二分律、以相容指标书写的约数格精确容斥公式，以及 pairwise-incompatible 约数族所导致的线性零点增量下界。

\begin{theorem}[零余类沿整除链的严格嵌套--互斥二分律]\label{thm:xi-time-part72-zero-coset-divisibility-nested-disjoint-dichotomy}
设 \(M\) 为偶整数，并令
\[
...
\noindent
上述三条结果表明，零余类几何在有限交截判据与加权 clique--Euler 公式之外，还具有一条更细的约数格层级结构：沿整除链，余类并不会出现任意复杂的部分重叠，而只能在“奇商嵌套”与“偶商互斥”两种情形之间二分；而一旦把这一二分律与相容指标容斥结合，任何 pairwise-incompatible 约数簇都会立刻转化为 \(\mathcal Z_m\) 的线性可加增量。于是，谱零点集合的算术几何便不仅可被精确计数，而且可在约数格上作系统的局部增量构造。
