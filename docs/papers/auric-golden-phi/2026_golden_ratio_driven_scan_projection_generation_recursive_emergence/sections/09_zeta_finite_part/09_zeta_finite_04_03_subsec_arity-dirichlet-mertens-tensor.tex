...
\begin{proposition}[Chebotarev--Mertens 常数张量的离散傅里叶闭式]\label{prop:arity-chebotarev-mertens-fourier}
...
\begin{proof}
离散傅里叶互换来自角色正交性：由 $p_{n,\chi}=\sum_{g}\overline{\chi(g)}p_{n,g}$ 与反演公式
$
p_{n,g}=\frac{1}{|G|}\sum_{\chi\in\widehat G}\chi(g)\,p_{n,\chi}
$
逐项乘以 $(r/\lambda)^n$ 并对 $n\ge 1$ 求和即可得到 $P_g(r)$ 的反演式。

奇性分解来自“平凡块给出对数发散，非平凡块在 $r=1$ 收敛”。更具体地：平凡角色对应总计数，按总体 Mertens 定理的 Abel 口径有
$
P_{\chi_0}(r)=-\log(1-r)+\log\mathfrak{M}+o(1)
$
（参见主文与附录 \ref{app:real-input-40-abel} 的通用口径）。而对任意非平凡角色 $\chi\neq\chi_0$，由本节对各分层给出的指数级等分布误差估计
\(
p_{n,g}=\frac{p_n}{|G|}+O(\rho^n/n)
\)
（其中 $\rho<\lambda$ 为某个最坏扭曲谱半径上界）与角色正交性可得
\(
p_{n,\chi}=O(\rho_\chi^n/n)
\)
（某个 $\rho_\chi<\lambda$），从而级数 $\sum_{n\ge 1}p_{n,\chi}\lambda^{-n}$ 绝对收敛，故 $P_\chi(1)$ 存在且有限。把 $P_{\chi_0}(r)$ 的对数奇性与其余 $P_\chi(1)$ 的有限极限代回傅里叶反演式，即得到所述展开与闭式。证毕。
\end{proof}

\begin{corollary}[常数张量的归一化一致性]\label{cor:arity-chebotarev-mertens-sum}
在上述记号下有
$$
...
\begin{proof}
对闭式在 $g\in G$ 上求和，使用 $\sum_{g\in G}\chi(g)=0$（$\chi\neq\chi_0$）即得。
\end{proof}
...
\begin{corollary}[常数级边缘偏置：\texorpdfstring{$\chi\bmod 2$}{chi mod 2} 与 \texorpdfstring{$N_e\bmod 2$}{Ne mod 2}]\label{cor:real-input-40-mertens-222-marginal-bias}
...
\begin{theorem}[有限维 Artin--Dirichlet 显式公式：常数张量作为扭曲行列式常数项的有限和]\label{thm:arity-dirichlet-explicit-constant}
...
\end{theorem}
\begin{proof}
对每个 \(j\) 的扭曲行列式 \(L_j\) 在 \(|z|<1/\rho_j\) 内解析且非零，并满足
\(
\log L_j(z)=\sum_{n\ge 1}\Tr(M_j^n)\,z^n/n
\)。
将该恒等式代入 Witt/Möbius 反演（附录 \ref{app:witt-weighted} 的同型公式），在 \(z=z_\star\) 处得到 \(\mathcal{C}(j)=\sum_{k\ge 1}\mu(k)\log L_{kj}(z_\star^k)/k\)。
再由有限阿贝尔群的角色正交关系对 \(r\) 作傅里叶反演（命题 \ref{prop:kernel-chebotarev-fourier} 的同型步骤），并用 \(\sum_r C_r=\mathsf{M}\) 固定平凡项，即得常数张量的显式公式。

尾界来自粗估计
\(
|\log\det(I-zM_j)^{-1}|
\le \sum_{i=1}^d \sum_{m\ge 1}|z\lambda_i|^m/m
\le d\sum_{m\ge 1}(|z|\rho_j)^m/m
\)
并对 \(z=z_\star^k=\lambda^{-k}\) 求和即可。证毕。
\end{proof}

\begin{theorem}[Dirichlet--Mertens 常数张量对标签细化的投影函子性]\label{thm:dirichlet-mertens-functoriality}
...
\end{theorem}
\begin{proof}
由定理 \ref{thm:arity-dirichlet-explicit-constant}，常数张量可由扭曲常数在有限群上的角色傅里叶反演给出；具体地，
\(
C^G_g=\frac{\mathsf{M}}{|G|}+\frac{1}{|G|}\sum_{\chi\neq 1}\overline{\chi(g)}\,\mathcal{C}(\chi)
\)。
对给定 \(h\in H\) 求纤维和并交换求和次序，得到
\[
\sum_{\pi(g)=h}C^G_g
=\frac{\mathsf{M}}{|H|}
\frac{1}{|G|}\sum_{\chi\neq 1}\left(\sum_{\pi(g)=h}\overline{\chi(g)}\right)\mathcal{C}(\chi).
\]
注意：当 \(\chi\) 可写为 \(H\) 的角色 \(\psi\) 的拉回 \(\chi=\psi\circ\pi\) 时，内层纤维和等于 \(|\ker\pi|\overline{\psi(h)}\)；而当 \(\chi\) 不来自拉回时，因有限阿贝尔群角色在纤维上的正交性，内层和为 \(0\)。因此上式恰化为 \(H\)-分层的反演式，从而 \(\sum_{\pi(g)=h}C^G_g=C^H_h\)。证毕。
\end{proof}
...
\begin{lemma}[单位圆扭曲的偶性与无三次项]\label{lem:unit-circle-evenness}
...
\end{lemma}
\begin{proof}
由 $M_{-\theta}=\overline{M_\theta}$ 得 $M_{-\theta}$ 与 $M_\theta$ 的谱互为共轭，因此谱半径相等：$\rho(-\theta)=\rho(\theta)$。
又因主特征值分支简单，$\Lambda(\theta)$ 在 $0$ 的邻域解析，且 $\Lambda(-\theta)=\overline{\Lambda(\theta)}$；于是
\(
|\Lambda(\theta)|^2=\Lambda(\theta)\Lambda(-\theta)
\)
为解析偶函数，且在 $\theta=0$ 处取正值 $\Lambda(0)^2$，故其平方根 $|\Lambda(\theta)|=\rho(\theta)$ 在邻域内亦为实解析偶函数。
因此其对数的展开只含偶次项，得到结论。证毕。
\end{proof}
...
命题 \ref{prop:arity-335-kappa-variance} 给出 \(\kappa_\infty=\sigma_\xi^2/2\) 的“压力二阶导数”口径。
...
\begin{proposition}[最坏角色方向的协方差判据：\texorpdfstring{$t^\top\Sigma t$}{t^T Sigma t} 最小化]\label{prop:arity-335-worst-direction-cov}
...
\begin{theorem}[大模数极限律：最坏扭曲亏损的缩放极限]\label{thm:arity-335-limit-law}
...
=\kappa_p.
$$
则在命题 \ref{prop:arity-335-kappa-variance} 的假设与记号下，
$$
...
\end{theorem}
\begin{proof}
由命题 \ref{prop:arity-335-kappa-variance} 的渐近式
\(
1-\rho_{3,3,p}/\lambda
=
\frac{\sigma_\xi^2}{2}\left(\frac{2\pi}{p}\right)^2+O(p^{-4})
\)
两边同乘 \((p/2\pi)^2\) 即得 \(D(p)=\sigma_\xi^2/2+O(p^{-2})\)，从而极限存在并等于 \(\kappa_\infty=\sigma_\xi^2/2\)。证毕。
\end{proof}

\begin{corollary}[扩散标度：混合时间 \texorpdfstring{$\tau_{\mathrm{mix}}(p)$}{tau_mix(p)} 为 \texorpdfstring{$O(p^2)$}{O(p^2)}]\label{cor:arity-335-diffusive-mixing}
...
\begin{corollary}[near-coboundary \texorpdfstring{$\Rightarrow$}{->} 小偏差：带 \texorpdfstring{$4\sin^2(\pi/p)$}{4 sin^2} 的缩放上界]\label{cor:arity-335-eps-trig-bound}
...
\begin{proposition}[小角主曲线与普适塌缩：二次主项 \texorpdfstring{$+$}{+} 四次修正]\label{prop:arity-335-master-curve}
...
\begin{remark}[常数张量的交互阶指纹：ANOVA/Hoeffding 正交分解]\label{rem:dirichlet-mertens-anova-fingerprint}
...
这为“常数级漂移张量的交互阶结构”与“最坏扭曲慢模的几何方向”之间建立了一个可核验的桥接指纹。
\end{remark}
