...
就关键结构（\(\Pi(\lambda,y)\) 的平面自同构椭圆化 \(u=y-\lambda^2\)、对偶临界三次 \(16\lambda^3-9\lambda^2+1\) 与 Lee--Yang 三次 \(P_{\mathrm{LY}}(y)\) 的消元对应、以及泛型单值/伽罗瓦群的满对称性）在可公开检索的文献与数据库索引范围内作定位检索，未见与本文同一记号与证明强度下的直接同构陈述；该核验仅提供“可检索范围内未见同构”的否证接口，并不构成对未收录文献的穷尽性排除。
\end{remark}
