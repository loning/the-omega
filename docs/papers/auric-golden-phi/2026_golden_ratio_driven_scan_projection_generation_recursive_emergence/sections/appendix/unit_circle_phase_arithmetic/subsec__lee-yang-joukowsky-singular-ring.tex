...
\input{para__lee-yang-trace-mobius-invertibility}
...
\input{para__lee-yang-branch-cover-square-root}
\input{para__lee-yang-orthogonal-dual-imaginary-projection}

\input{para__lee-yang-chebyshev-semiconjugacy}

\paragraph{余弦极坐标族的单位圆反推与奇异环推前}
对任意整数 \(n,d\ge 1\)，考虑极坐标形式给出的复平面轨迹
\begin{equation}\label{eq:leyang-polar-cos-family}
z(\theta):=r(\theta)e^{\mathrm{i}\theta},\qquad r(\theta):=\cos\!\Bigl(\frac{n}{d}\theta\Bigr)\qquad(\theta\in\RR).
\end{equation}
下述命题把该族严格表示为单位圆上的 Joukowsky 迹读出与弦中点轨迹，并给出其在坐标门 \(J\) 下推前到 Lee--Yang 奇异环的闭式表达。

\begin{proposition}[单位圆反推与 Joukowsky 迹表示]\label{prop:leyang-polar-family-unit-circle-joukowsky}
令 \(\alpha:=e^{\mathrm{i}\theta/d}\in\TT\)。
则对任意实 \(\theta\) 有严格恒等式
...
\end{equation}
因此曲线 \(z(\theta)=r(\theta)e^{\mathrm{i}\theta}\) 可被理解为：在单位圆上取两点 \(\alpha^{d+n}\) 与 \(\alpha^{d-n}\)，其算术平均（弦中点）随 \(\theta\) 演化所形成的轨迹。
\end{proposition}
\begin{proof}
由 \(\alpha^n=e^{\mathrm{i}(n/d)\theta}\) 得
\(
W(\alpha^n)=\alpha^n+\alpha^{-n}=2\cos(\frac{n}{d}\theta)
\)，从而得到 \eqref{eq:leyang-polar-radius-joukowsky}。
再乘以 \(e^{\mathrm{i}\theta}=\alpha^d\) 即得 \eqref{eq:leyang-polar-position-chord-midpoint}。证毕。
\end{proof}

\input{para__lee-yang-polar-complex-lissajous}

\begin{proposition}[奇异环坐标门下的显式推前]\label{prop:leyang-polar-family-pushforward-to-singular-ring}
沿用命题 \ref{prop:leyang-polar-family-unit-circle-joukowsky} 的记号，定义
\begin{equation}\label{eq:leyang-polar-family-t-def}
t(\theta):=J(\alpha^{2n}).
\end{equation}
则对一切 \(\theta\) 满足 \(\cos(\frac{n}{d}\theta)\neq 0\) 有闭式恒等式
\begin{equation}\label{eq:leyang-polar-family-t-closed}
\boxed{\;
t(\theta)
=-\frac{1}{\bigl(W(\alpha^n)\bigr)^2}
=-\frac{1}{4\cos^2(\frac{n}{d}\theta)}
=-\frac{1}{(2r(\theta))^2}\; }.
\end{equation}
从而 \(t(\theta)\in(-\infty,-\tfrac14]\)，并且该坐标仅由径向幅度 \(r(\theta)\) 决定；当 \(\cos(\frac{n}{d}\theta)=0\) 时，\(t(\theta)\) 沿 \((-\infty)\) 发散。
\end{proposition}
\begin{proof}
取 \(z=\alpha^n\) 并应用命题 \ref{prop:jouwkowsky-inverse-square} 的恒等式 \(J(z^2)=-1/W(z)^2\)，得到
\(
t(\theta)=J(\alpha^{2n})=-1/(W(\alpha^n))^2
\)。
再由 \(W(\alpha^n)=2\cos(\frac{n}{d}\theta)=2r(\theta)\) 得 \eqref{eq:leyang-polar-family-t-closed}。
当 \(|\alpha|=1\) 时 \(W(\alpha^n)\in[-2,2]\)，从而 \(t(\theta)\in(-\infty,-\tfrac14]\)；
在零点处 \(W(\alpha^n)=0\) 给出发散。证毕。
\end{proof}

\begin{lemma}[奇异环到单位圆的显式反演]\label{lem:leyang-singular-ring-inversion}
对任意 \(t\in(-\infty,-\tfrac14]\)，取
\[
a:=\pm\frac{1}{\sqrt{-t}}\in[-2,2],
\]
并解二次方程
\begin{equation}\label{eq:leyang-joukowsky-trace-quadratic}
z+z^{-1}=a\qquad\Longleftrightarrow\qquad z^2-az+1=0.
\end{equation}
则两根为
\begin{equation}\label{eq:leyang-joukowsky-trace-roots}
\boxed{\;
z_{\pm}=\frac{a\pm \mathrm{i}\sqrt{4-a^2}}{2},\qquad |z_{\pm}|=1,\qquad z_-=\overline{z_+}=z_+^{-1}\; }.
\end{equation}
因此 \(t=-1/a^2\) 与单位圆上一对共轭点 \(\{z,z^{-1}\}\subset\TT\) 的 Joukowsky 迹 \(a=W(z)\) 精确等价。
\end{lemma}
\begin{proof}
由 \(t<0\) 得 \(a\in\RR\) 且 \(|a|\le 2\)。
方程 \eqref{eq:leyang-joukowsky-trace-quadratic} 的判别式为 \(a^2-4\le 0\)，故两根共轭，且由求根公式得到 \eqref{eq:leyang-joukowsky-trace-roots}。
又 \(z_+z_-=1\) 强制 \(|z_{\pm}|=1\) 且 \(z_-=\overline{z_+}=z_+^{-1}\)。
最后，由命题 \ref{prop:jouwkowsky-inverse-square} 有 \(t=J(z^2)=-1/W(z)^2\)，令 \(a=W(z)\) 即得 \(t=-1/a^2\)。证毕。
\end{proof}
...
考虑映射
\[
\Theta:\ \theta\longmapsto t(\theta)=-\frac{1}{4\cos^2(\frac{n}{d}\theta)}\in(-\infty,-\tfrac14],
\]
...
则 \(\Theta(\theta)=t\) 等价于 \(\cos^2\varphi=c^2\)。
在每个长度为 \(\pi\) 的周期区间上，方程 \(\cos^2\varphi=c^2\) 恰有两解；区间 \([0,2\pi n)\) 含 \(2n\) 个周期，故总解数为 \(4n\)，得到 \eqref{eq:leyang-polar-family-preimage-4n}。
\par
端点 \(t=-\tfrac14\) 对应 \(\cos^2\varphi=1\)，其解为 \(\varphi=k\pi\)（\(0\le k\le 2n-1\)），故 \(\card{\Theta^{-1}(-\tfrac14)}=2n\)。
...
\begin{proposition}[单位圆提升与二维 Lee--Yang 坐标]\label{prop:leyang-lissajous-unit-circle-uplift}
令 \(z=e^{\mathrm{i}t}\in\TT\)。
定义环面相位闭环
\[
\gamma_{a,b,\delta}:\TT\to\TT^2,\qquad
\gamma_{a,b,\delta}(z):=\bigl(e^{\mathrm{i}\delta}z^a,\ z^b\bigr).
\]
则 \eqref{eq:leyang-lissajous-family} 可被表示为该闭环的 Joukowsky 实投影：
\begin{equation}\label{eq:leyang-lissajous-joukowsky-projection}
\boxed{\;
\bigl(x(t),y(t)\bigr)
=\left(\frac12W(e^{\mathrm{i}\delta}z^a),\ \frac12W(z^b)\right),\qquad z=e^{\mathrm{i}t}. \;}
\end{equation}
进一步，定义二维 Lee--Yang 提升
\[
\Lambda_{a,b,\delta}(z):=\bigl(J((e^{\mathrm{i}\delta}z^a)^2),\ J((z^b)^2)\bigr),
\qquad z\in\TT.
\]
则对任意 \(z\in\TT\) 有 \(\Lambda_{a,b,\delta}(z)\in(-\infty,-\tfrac14]^2\)；
并且在 \(x(t)\neq 0\)、\(y(t)\neq 0\) 时成立闭式读出
\begin{equation}\label{eq:leyang-lissajous-leyang-readout}
\boxed{\;
\Lambda_{a,b,\delta}(e^{\mathrm{i}t})
=\left(-\frac{1}{4x(t)^2},\ -\frac{1}{4y(t)^2}\right).\;}
\end{equation}
\end{proposition}
\begin{proof}
由 \(W(e^{\mathrm{i}\theta})=2\cos\theta\) 得
\[
\frac12W(e^{\mathrm{i}\delta}z^a)
=\frac12\left(e^{\mathrm{i}(at+\delta)}+e^{-\mathrm{i}(at+\delta)}\right)
=\cos(at+\delta)=x(t),
\]
同理 \(\frac12W(z^b)=\cos(bt)=y(t)\)，从而得到 \eqref{eq:leyang-lissajous-joukowsky-projection}。
再由命题 \ref{prop:jouwkowsky-inverse-square} 的恒等式 \(J(\zeta^2)=-1/W(\zeta)^2\) 得
\[
J((e^{\mathrm{i}\delta}z^a)^2)=-\frac{1}{W(e^{\mathrm{i}\delta}z^a)^2}=-\frac{1}{(2x(t))^2}=-\frac{1}{4x(t)^2},
\]
同理 \(J((z^b)^2)=-1/(4y(t)^2)\)，即 \eqref{eq:leyang-lissajous-leyang-readout}。
由于 \(|e^{\mathrm{i}\delta}z^a|=|z^b|=1\)，由命题 \ref{prop:jouwkowsky-inverse-square} 的第二条可知两分量均落在 \((-\infty,-\tfrac14]\) 上。证毕。
\end{proof}

\input{para__lee-yang-lissajous-chebyshev-resultant}
...
\input{para__lee-yang-rose-lissajous-fiberproduct-leakage}
...
\input{thm__lee-yang-fibonacci-uplift-rigidity-chebyshev-parity}
...
\input{conj__singular-ring-finite-anchors}
