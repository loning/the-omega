...
\begin{remark}[Linear-ontology reading of protocol depth and mass]\label{rem:mass_as_scan_density}
Under Axiom~0 (Section~\ref{subsec:axiom0_linearity}), the primitive description is a one-dimensional scan order.
In this interface language, depth shifts and degeneracy penalties are naturally read as \emph{additional local scan length} required to implement and stabilize a readout pattern at a fixed finite resolution.
Equivalently, larger effective depth corresponds to increased local scan density and longer protocol dwell time, which is precisely the operational content of inertia and mass in a scan-based identification dictionary.
This can be read as an oversampling/compression effect:
the 6-DoF lock motivates an $m=6$ anchor window for a $3$D locality dictionary (Section~\ref{subsec:6dof_lock}), while a resolution uplift to larger $m$ (e.g.\ to $m=10$ near the electroweak threshold in Section~\ref{subsec:p3_resolution_jumps}) attempts to resolve higher-bit texture in the same locality budget.
Heuristically, excess information that cannot be unfolded spatially is absorbed as additional structure along the linear scan (``knotting'' of the readout stream), increasing the required scan length and therefore the effective inertia.
In this sense, \emph{mass is the localized latency of processing high-resolution data on low-resolution hardware}.
Physically, this corresponds to a knotted Hilbert stream: once the information density (effective $m$) exceeds the local geometric capacity (effective $6$ DoF), the directed readout must fold recursively into itself to store high-resolution structure within a bounded neighborhood.
Such knotting resists acceleration (inertia) and forces additional scan steps to traverse local structure, providing a direct interface route to time-dilation-like effects in a scan-based matching dictionary.
At the macroscopic level, \emph{gravity is the latency of processing high-resolution data on low-resolution hardware} organized into a locality dictionary: time dilation and redshift correspond to additional scan steps required to traverse knotted regions.
The quantitative content of this interpretation in the present paper is the auditable closed template \eqref{eq:mass_template} together with the bounded-complexity rigidity signal for its integer coefficients (Proposition~\ref{prop:rhat_rigidity}).
\end{remark}
