...
沿用定理 \ref{thm:killo-godel-semidir-affine-2adic} 与推论 \ref{cor:killo-infinite-stream-2adic-holographic-point} 的设定，并记
\[
...
\]
故 \(X([a])\subseteq X_n(a)+B^nT_v\)。反之，若
\[
X(\mathbf e)\in X_n(a)+B^nT_v
\]
而 \(\mathbf e\notin[a]\)，则 \(\mathbf e\) 与 \(a\) 的延拓在某个 \(j\le n\) 处首次分歧；由第一条可知该差值属于
\[
B^{j-1}T_v\setminus B^jT_v,
\]
从而不可能落在 \(B^nT_v\) 中，矛盾。故
\[
X([a])=\bigl(X_n(a)+B^nT_v\bigr)\cap X(\mathcal E^{\NN}).
\]
