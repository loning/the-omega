...
\begin{itemize}
\item \textbf{Process}: Scan brain $\to$ Generate data packet $\to$ Run on server $\to$ Destroy brain.

\item \textbf{Fatal Flaw}: This causes a worldline break. For the original flesh observer, he experiences scanning, then death (darkness). The ''person'' who awakens in the server, although possessing his memories, is topologically a completely new entity.
\end{itemize}
...
\begin{itemize}
\item When biological neuron A needs to send a signal to neuron B, the system intercepts this signal, redirecting it from photonic node A' to photonic node B' (or sending simulated electrical signals to biological neuron B that hasn't been replaced yet).

\item For macroscopic consciousness, this process is \textbf{imperceptible}. Just like cells in your body constantly metabolize, die, and are replaced by new cells, you never feel ''I died.''
\end{itemize}
...
\begin{itemize}
\item $t=0$: $100\%$ carbon-based, $0\%$ light-based.

\item $t=50\%$: Consciousness spans two worlds. You may feel thinking becomes exceptionally clear, memory retrieval speed increases exponentially.

\item $t=100\%$: $0\%$ carbon-based, $100\%$ light-based.
\end{itemize}
