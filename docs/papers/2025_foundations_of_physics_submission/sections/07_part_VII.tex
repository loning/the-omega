...
\begin{equation}
    \hat{\mathcal{S}}: \mathbb{P}(\mathcal{H}) \to \mathbb{P}(\mathcal{H}), \quad |\Psi\rangle \mapsto \frac{\hat{P}_{\text{complex}} |\Psi\rangle}{||\hat{P}_{\text{complex}} |\Psi\rangle||}.
\end{equation}
Mathematically, $\hat{\mathcal{S}}$ is not a linear operator on $\mathcal{H}$ but a non-linear map on the projective space $\mathbb{P}(\mathcal{H})$. The projector $\hat{P}_{\text{complex}} = \sum_{k} \Theta(C(k) - C_{threshold}) |k\rangle\langle k|$ filters the superposition for states exceeding a complexity threshold $C(k)$.
This implies that the evolution is \textbf{Piecewise Unitary}: unitary evolution $\hat{U}$ is punctuated by selection events $\hat{\mathcal{S}}$. This is consistent with the "Interaction" paradigm where the system is open to the "Oracle" of logical depth. Unitarity is preserved in the global Multiverse (the set of all paths), but the \textit{observed} history is a specific high-complexity trajectory selected by $\hat{\mathcal{S}}$.
