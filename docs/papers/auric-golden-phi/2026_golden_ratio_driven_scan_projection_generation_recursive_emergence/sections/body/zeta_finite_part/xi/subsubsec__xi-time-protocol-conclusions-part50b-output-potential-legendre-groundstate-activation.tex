...
\begin{theorem}[输出位势局部 Cram\'er 函数的单侧偏斜与四阶局部几何]\label{thm:xi-output-potential-local-legendre-skew-steep}
...
\]
因此，输出位势的局部 Cram\'er germ 既不是 Gaussian 对称型，也不是关于 \(\mu\) 的偶函数；其中心附近具有严格左偏的局部涨落方向与正的四阶陡峭性。
\end{theorem}
\begin{proof}
由关系
\[
...
{P''(\Theta(\alpha))^5}.
\]
在 \(\alpha=\mu\) 时有 \(\Theta(\mu)=0\)，故上述三式立即退化为
\[
I''(\mu)=\frac{1}{\sigma^2},\qquad
I^{(3)}(\mu)= -\frac{\kappa_3}{\sigma^6},\qquad
I^{(4)}(\mu)=\frac{3\kappa_3^2-\sigma^2\kappa_4}{\sigma^{10}}.
\]
于是对 \(I\) 在 \(\mu\) 处作 Taylor 展开可得
\[
I(\mu+\delta)
=
\frac{I''(\mu)}{2}\delta^2+\frac{I^{(3)}(\mu)}{6}\delta^3+\frac{I^{(4)}(\mu)}{24}\delta^4+O(\delta^5),
\]
这正是所示四阶 Cram\'er germ。

前两条的符号由 \(\sigma^2>0\) 与 \(\kappa_3<0\) 直接给出。对四阶项，代入同表的闭式后可化简为
\[
...
0.
\]
因此 \(I^{(4)}(\mu)>0\)。两条数值近似由上述显式公式直接代入得到。

最后，若 \(I\) 在 \(\mu\) 邻域内关于 \(\mu\) 反射对称，则其所有奇阶导数在 \(\mu\) 处都必须为零；这与
\(
I^{(3)}(\mu)>0
\)
矛盾。故 \(I\) 不是偶函数。由于原始累积量满足 \(\kappa_3<0\)，这等价于中心涨落的三阶偏斜方向为左偏；同时 \(I^{(4)}(\mu)>0\) 给出正的四阶局部陡峭性。证毕。
\end{proof}

\begin{corollary}[输出位势局部偏差的严格方向性不等式]\label{cor:xi-output-potential-local-legendre-one-sided-asymmetry}
...
>0.\;}
\]
因此在真实输入 \(40\) 状态核的输出位势下，同等幅度的右偏差比左偏差具有更高的指数代价；换言之，局部中尺度偏差呈现严格左偏斜。
\end{corollary}
\begin{proof}
...
\]
由于 \(\kappa_3<0\)，该主项严格为正，故对充分小的 \(\delta>0\) 差值必为正。证毕。
\end{proof}
...
\end{proof}
\noindent 该推论只控制 \(j\) 有界的局部窗口；关于全局最坏模态 \(j_{\mathrm{worst}}\) 是否对所有 \(1\le j\le m-1\) 都最终由 \(j=1\) 实现，本文在此仅保留表 \ref{tab:real_input_40_output_potential_dirichlet_twists} 的审计事实，而不额外作出超出该窗口的解析声称。
...
上述结果表明：饱和端点的能垒缺口、局部 Legendre 几何的二次数域封闭、均值附近的非对称陡峭性、唯一小根的激活支结构，以及固定单位根模态的四阶展开、有界窗口内 \(j=1\) 的最终支配和四阶修正的二次量级收益，实际上都由同一条输出位势代数曲线统一控制。端点零温极限、中心极限定域与单位根方向的 Dirichlet 扭曲并非彼此孤立的机制，而是同一解析对象在实轴、局部对偶几何与 cyclotomic 边界上的不同投影。
