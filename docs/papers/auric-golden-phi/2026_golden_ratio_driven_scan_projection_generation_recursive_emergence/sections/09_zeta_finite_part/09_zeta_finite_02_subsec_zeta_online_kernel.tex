...
\begin{corollary}[\texorpdfstring{$m^{-8}$}{m^-8} 精度与 \texorpdfstring{$m^2$}{m^2} 扩散尺度]\label{cor:sync-rho-gap-m8-diffusive}
沿用推论 \ref{cor:sync-rho-gap-m2} 的记号，并令 \(t=2\pi/m\)。
由注 \ref{rem:sync-rho-pressure-asymp} 中 \(P(\theta)\) 的 \(\theta^8\) 展开，得到
\[
\rho_m
=3\exp\!\Bigl(
