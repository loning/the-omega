...
在局域并行口径下，“加法轨道寄存器（轨道残差）”与“乘法素数寄存器（唯一分解）”的作用等价性见附录 \ref{app:bridge-rh-obligations} 的命题 \ref{prop:orbit-register-eq-prime-register}；事件编码 $\mathrm{code}/\mathrm{decode}$ 的完全形式化与 unitary slice 审计的数值证书协议见附录 \ref{app:inject-code-audit}。
