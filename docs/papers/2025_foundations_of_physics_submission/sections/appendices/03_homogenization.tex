...
Let $\Lambda \subset \mathbb{R}^d$ be a cut-and-project set defined by the projection $P: \mathbb{R}^n \to \mathbb{R}^d$ ($n > d$) of the strip $S = \mathbb{R}^d \times W$, where $W \subset \mathbb{R}^{n-d}$ is a compact window; see, e.g., \cite{Senechal1995,BaakeGrimm2013AperiodicOrder}.
...
\end{equation}
This follows from Weyl-type equidistribution for cut-and-project sets; see, e.g., \cite{Hof1995,BaakeGrimm2013AperiodicOrder}.
\end{lemma}
...
For coined walks on regular lattices, rigorous long-wavelength Dirac limits (including variable-mass and gauge-coupled extensions) are available in the QW/QCA literature; see, e.g., \cite{Arrighi2014,Dariano2014,Bisio2015}. Extending such results to cut-and-project substrates requires controlling the quasicrystal structure factor together with the spatial variation of internal-coordinate (phason) fields. In this manuscript we use the ergodic averaging statement (Lemma~\ref{lemma:ergodic}) and explicit 1D diagnostics (Appendix~\ref{app:fib_dqca}) as the checkable backbone of the quasicrystal program.\par
\noindent
The corresponding higher-dimensional target is stated as Conjecture~\ref{conj:quasicrystal_dirac} and reduced to the two quantitative control tasks in Proposition~\ref{prop:quasicrystal_homogenization_reduction}.

\begin{proposition}[Higher-dimensional quasicrystal Dirac limit as a rigidity reduction]
\label{prop:quasicrystal_homogenization_reduction}
Let $\Lambda\subset\mathbb{R}^d$ be a regular cut-and-project (model) set with compact window $W$ and consider a finite-range coined walk/PQCA step $U_\varepsilon$ on $\Lambda$ with microscopic scale $\varepsilon>0$.
Assume that the local coin data are given by a bounded, sufficiently regular function of the internal coordinate (and a slowly varying phason displacement), so that local coefficients admit orbit traces on $W$ in the sense of Lemma~\ref{lemma:ergodic}.\par
\noindent
Then the higher-dimensional continuum-limit problem may be decomposed into two quantitative control tasks:
\begin{itemize}
    \item \textbf{(Structure-factor control).} Control the finite-window structure factor/diffraction content (dominant Bragg peaks and their weights) in the long-wavelength band relevant to a Dirac/EFT expansion \cite{Hof1995,BaakeGrimm2013AperiodicOrder}.
    \item \textbf{(Phason/rigidity control).} Control the spatial variation of the internal-coordinate data (phason strain) so that window perturbations induce only a controlled density of local flips/defects in the induced readout/texture; in scan--projection language this is governed by orbit-calculus discrepancy bounds and window-symmetric-difference stability \cite{Ma2025HPA}.
\end{itemize}
Under these controls, the same analytic-logarithm mechanism used in the translation-invariant proof above yields an effective long-wavelength Hamiltonian whose leading symbol is Dirac-type (with coefficients given by the orbit-trace averages), and whose remainder is perturbatively small in $\varepsilon$ and in the phason-strain scale (cf.\ the band-limited regime in the proof of Theorem~\ref{prop:dirac_limit} and the orbit-trace normalization of Convention~R1).\par
\end{proposition}

\paragraph{Bridge: discrete step to an effective Hamiltonian (analytic logarithm regime).}
Several continuum-limit arguments in this manuscript (Dirac-limit diagnostics, MDR expansions) implicitly use the same elementary bridge: a one-step unitary $U_\varepsilon$ close to the identity admits an analytic principal logarithm on a band-limited sector, yielding an effective Hamiltonian $H_{\mathrm{eff}}^{(\varepsilon)}\equiv \frac{i}{\varepsilon}\log U_\varepsilon$.
This is the mechanism used explicitly in the proof of Theorem~\ref{prop:dirac_limit} above, and it also underlies the analytic/parity expansion used in Appendix~\ref{app:mdr_symmetry}.
In particular, the compactness of the unitary spectrum and the choice of a discrete difference-based momentum coordinate provide a rigid example of an arc--chord modified dispersion relation; see Appendix~\ref{app:mdr_symmetry} and \cite{Ma2025HPA}.

\subsection{Diophantine property (golden case)}
Quantitative control of near-commensurabilities in quasiperiodic constructions is often organized by Diophantine (badly-approximable) bounds. For the golden ratio $\phi=\tfrac{1+\sqrt5}{2}$, continued-fraction theory gives the sharp estimate \cite{Cassels1957Diophantine}
\begin{equation}
...
\label{app:three_gap}
The same bad-approximability property that controls the Diophantine bounds above also governs the arithmetic structure of intrinsic phase-orbit readouts (Part~II), modeled by an irrational rotation $x_{n+1}=x_n+\alpha_{\mathrm{rot}}\ (\mathrm{mod}\ 1)$ with $\alpha_{\mathrm{rot}}=\phi^{-1}$. A classical result (the Three-Gap/Three-Distance Theorem) states that for any irrational $\alpha_{\mathrm{rot}}$, the ordered set of points $\{n\alpha_{\mathrm{rot}}\}_{n=0}^{N-1}$ partitions the circle into at most three distinct gap lengths \cite{Slater1967}.
Moreover, when $N$ equals a denominator $q_k$ of a continued-fraction convergent of $\alpha_{\mathrm{rot}}$, the partition exhibits a particularly rigid two-gap structure, and the closest returns occur at such $q_k$.

For the golden case $\alpha_{\mathrm{rot}}=\phi^{-1}$, the continued fraction is eventually constant, so the convergent denominators are Fibonacci numbers (up to an index shift). Consequently, the strongest near-returns and the most uniform finite sampling of the phase orbit occur on Fibonacci time scales. This provides a concrete number-theoretic underpinning for the Fibonacci ``resonant eras'' discussed in Part~VI.
