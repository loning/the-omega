...
\noindent
结合结论 \ref{con:xi-time-part9n-s4-all-subgroup-genera} 与以上诸条结论，\(S_4\)-Hurwitz 商塔中这条以 Chevalley--Weil 重数为起点的链条已在几何、表示与算术三侧同时闭合：一方面，\(C_6\) 作为 \(H\) 与 \(Z\) 的规范化纤维积被唯一识别，全纯微分空间按四个非平凡不可约表示张量分层，\(X/C_2\) 与 \(X/C_3\) 的 genus 与 Jacobian 同源型由 Burnside--Kani--Rosen 关系唯一补全，\(J(X)\) 的有理投影代数下界及 \(\operatorname{Prym}(C_6/H)\) 的平方根刚性亦从同一分块同步抽出；另一方面，良素处局部 Euler 因子的平方/立方块与两类 Prym 的最小同源代价继续由群论分块与极化缺陷严格锁定。
