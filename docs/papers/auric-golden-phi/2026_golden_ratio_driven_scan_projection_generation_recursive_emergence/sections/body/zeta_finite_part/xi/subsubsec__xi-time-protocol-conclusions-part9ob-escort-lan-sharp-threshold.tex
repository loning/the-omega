...
\begin{corollary}[escort 温度间最优判别的末位单比特退化]\label{cor:xi-time-part9ob-escort-np-lastbit-collapse}
沿用定理 \ref{thm:xi-time-part9ob-escort-loglik-onebit-expfamily} 的记号。对任意固定 \(p,q\ge 0\)，令
\[
...
\]
更具体地，若 \(q>p\)，则对充分大 \(m\) 而言，似然比 \(\pi_{m,q}/\pi_{m,p}\) 在 \(A_0\) 上严格大于在 \(A_1\) 上的取值；若 \(q<p\)，则情形相反。因而一切最优 Neyman--Pearson 判别都只依赖末位 \(w_m\)，其非平凡拒绝域由 \(A_0\) 与 \(A_1\) 两块生成。
\end{corollary}
\begin{proof}
...
\log\frac{\pi_{m,q}(w)}{\pi_{m,p}(w)}
=
A_m(q,p)-(q-p)w_m\log\varphi+O(\varepsilon_m).
\]
因此两块上的似然比差恰为
\[
(p-q)\log\varphi+O(\varepsilon_m).
\]
若 \(q>p\)，则该差对充分大 \(m\) 为正，故似然比在 \(A_0\) 上严格更大；若 \(q<p\)，则在 \(A_1\) 上严格更大。于是 Neyman--Pearson 最优检验不再需要读取整个 \(w\)，只需读取末位并在对应块上作决定。证毕。
\end{proof}
