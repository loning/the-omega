...
以下条目把固定冻结相中的 escort Rényi 熵谱、全微态精确反演所需比特率的有限层闭式，以及分枝三次场、零温四次场与 RH 临界五次场之间的算术独立性，压缩为一组可直接调用的终端结论。

\begin{theorem}[冻结相中 escort Rényi 谱的正阶完全塌缩与 \texorpdfstring{$q=0$}{q=0} 跃迁]\label{thm:xi-fixed-freezing-escort-renyi-spectrum-collapse}
沿用结论 \ref{con:xi-terminal-fixed-freezing-escort-oracle-sharp-threshold} 的记号。固定
\[
...
则成立：
\begin{enumerate}
  \item 对任意 \(q\in[1,\infty]\)，有
  \[
  \lim_{m\to\infty}\frac1m H_q(\pi_{m,a})=g_*.
  \]
  \item 对任意 \(q\in(0,1)\) 且 \(aq>a_{\mathrm c}\)，同样有
  \[
  \lim_{m\to\infty}\frac1m H_q(\pi_{m,a})=g_*.
  \]
  \item 若记
  ...
  \]
\end{enumerate}
因此，一切正阶 Rényi 熵率在冻结相中统一塌缩到 \(g_*\)；而当 \(\log\varphi>g_*\) 时，Rényi 熵率在 \(q=0\) 处发生真实跃迁，因而并不连续。
\end{theorem}
\begin{proof}
...
\]
若 \(q>1\)，则 \(aq>a>a_{\mathrm c}\)；若 \(0<q<1\)，额外假设即为 \(aq>a_{\mathrm c}\)。于是定理 \ref{thm:fold-pressure-freezing-threshold} 同时适用于指数 \(a\) 与 \(aq\)，从而
\[
...
g_*.
\]
这便证明了全部有限正阶、且满足冻结条件的 Rényi 熵率塌缩。
\par
