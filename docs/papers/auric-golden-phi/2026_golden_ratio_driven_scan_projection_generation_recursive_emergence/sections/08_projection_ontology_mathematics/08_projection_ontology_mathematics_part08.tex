...
\paragraph{负矩—Schatten 左尾通道的终结式输出（审计接口）}\label{par:pom-left-tail-schatten-terminal}
...
该通道把“低多重度层”的经验描述推进为可复算证书体系：所有量均可由表 \ref{tab:fold_conditional_expectation_singular_spectrum} 及其派生表 \ref{tab:fold_conditional_expectation_left_tail_bases}、\ref{tab:fold_conditional_expectation_effective_support} 直接核验。

\medskip\noindent\textbf{结论（LT-1）：左尾自由能密度常数族在审计窗口内呈现稳定底数。}
令
\[
...
\]
在 2026-02-07 审计快照所覆盖的偶数网格 \(m=8,10,\dots,24\) 上取经验底数
\[
...
\medskip\noindent\textbf{结论（LT-2）：负矩通道无需 moment-kernel 编译；其信息可由 \(E_m\) 的 Schatten 谱直接读出。}
...
\medskip\noindent\textbf{结论（LT-3）：两类有效支撑规模给出奇异值能量的双尺度浓缩，并在 \(m=24\) 出现稳定分离。}
...
\medskip\noindent\textbf{结论（LT-4）：低多重度层存在无参数左尾证书，且在 \(m=24\) 可给出数值级下界。}
...
\medskip\noindent\textbf{结论（LT-5）：空间谱不确定性下界把右尾压缩与左尾稀薄绑定为不可兼得约束。}
...
\medskip\noindent\textbf{结论（LT-6）：双尾证书在同一审计接口上闭合为分位夹逼带，并可与 prime-register 预算并置。}
...
\[
\mathbb{P}\bigl(d_m(X)\le d_{-}(m,s)\bigr)\ \text{有显式下界（由 LT-4）},
\qquad
