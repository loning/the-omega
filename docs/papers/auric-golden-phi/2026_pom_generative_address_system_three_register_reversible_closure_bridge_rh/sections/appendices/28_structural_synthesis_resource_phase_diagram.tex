...
\begin{itemize}
  \item \textbf{碰撞空值阈值（prime-register 配额）：}在区间读出/CRT 外置方案下，为把多对一压缩以失败概率 $\le\varepsilon$ 提升为近似可逆，prime-register 的模积预算 $P_m$ 必须超过一个显式阈值；等价地，可写为
  $$
  \log P_m\ \ge\ m\,\gamma_m(\varepsilon)+\log 2,
  $$
  其中 $\gamma_m(\varepsilon)$ 表示在分辨率 $m$ 与容许失败 $\varepsilon$ 下的可核验误差指数（由所选碰撞指标与证书验证程序确定）。
