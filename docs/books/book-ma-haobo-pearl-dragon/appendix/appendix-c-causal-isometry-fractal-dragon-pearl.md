...
把离散世界投影为连续世界，最容易犯的错误，是把“距离”当成唯一的几何不变量。但在相对论语境里，更底层的不变量不是欧氏距离，而是**因果结构**。
...
**因果等距**：在"源代码视角"里，若事件 $A$ 是事件 $B$ 的必要前置条件（记为 $A \prec B$），那么在物理时空的投影中，事件 $A$ 必须位于事件 $B$ 的过去光锥内，即

$$A \in J^{-}(B)$$

这看似只是把常识写成了公式，但它真正的含义是：在离散到连续的映射中，我们允许长度、面积、曲率以不同分辨率被渲染；但**不允许**因果偏序在投影中断裂或倒置。
...
- [D. 上帝的编译器与递归宇宙](appendix-d-gods-compiler-recursive-universe.md)
