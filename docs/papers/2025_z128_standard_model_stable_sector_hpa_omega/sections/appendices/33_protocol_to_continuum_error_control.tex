...
\paragraph{Scope.}
\InterfaceTag The objects whose uncertainty we track are the reconstructed overhead proxy field $\widehat\chi(x)$ (Section~\ref{app:chi_reconstruction_protocol}) and its downstream weak-field representatives $\widehat\Phi(x)$ and $\widehat\rho_{\mathrm{eff}}(x)$ (Section~\ref{app:overhead_to_gravity_closure}).
...
\AuditTag All expectation/probability statements in this appendix are conditional on a declared protocol state $(m,n,K)$ and on an explicit sampling rule for the data stream (Appendix~\ref{subsec:tick_cap_joint_protocol_state}): the kernel component $K$ fixes the weighting rule under which $\mathbb{E}(\cdot)$ is taken, while $n$ and $m$ fix the discretization scale and windowed observables.
...
\textsf{\footnotesize[Prot]} The reconstruction pipeline outputs a grid field $\widehat\chi_h$ at some addressing resolution (Hilbert order $n$) and word resolution $m$ (Section~\ref{app:chi_reconstruction_protocol}).
To compare with a continuum representative $\chi$ (e.g.\ the smooth field entering the action representative in Section~\ref{app:cap_continuum_action_closure}), we treat $\widehat\chi_h$ as a piecewise-constant or interpolated field on a grid with spacing $h$.
...
Fix a window length $m$ and a reconstruction rule that produces samples $Z_1,\dots,Z_K$ of a folding-derived statistic (e.g.\ $Z_j=g_m(N_j)$ or any bounded proxy used in Section~\ref{app:chi_reconstruction_protocol}).
...
At larger $m$, one can either compute (or upper bound) the range of the chosen statistic from the audited Fold$_m$ tables/scripts, or replace $g_m$ by a bounded defect-rate proxy (Section~\ref{app:chi_reconstruction_protocol}).
\end{remark}
...
In the reconstruction protocol of Section~\ref{app:chi_reconstruction_protocol}, write
\[
...
\paragraph{First derivatives (for the \texorpdfstring{$\gamma$}{gamma} fit).}
When fitting $\gamma$ via rotation curves in Section~\ref{app:overhead_to_gravity_closure}, one needs $\chi'(r)$.
...
\subsubsection{Propagation to \texorpdfstring{$\rho_{\mathrm{eff}}$}{rho\_eff}, \texorpdfstring{$\Phi$}{Phi}, and the \texorpdfstring{$\gamma$}{gamma} fit}
\label{subsec:prot_to_cont_propagation_to_gravity}

\paragraph{Discrete \texorpdfstring{$\rho_{\mathrm{eff}}$}{rho\_eff} from reconstructed \texorpdfstring{$\chi$}{chi}.}
...
\paragraph{\texorpdfstring{$\Phi$}{Phi} from \texorpdfstring{$\chi$}{chi}.}
...
\paragraph{The \texorpdfstring{$\gamma$}{gamma} fit: WLS variance and propagation.}
Section~\ref{app:overhead_to_gravity_closure} uses the one-parameter weighted least-squares estimator
\[
...
\]
Moreover, under standard regularity conditions ensuring asymptotic normality of the weighted least-squares estimator (e.g.\ Lindeberg-type conditions for the weighted sum), a $(1-\delta)$ confidence interval can be reported as
$
...
As a distribution-free conservative alternative under finite variance only, Chebyshev's inequality gives
$
\mathbb{P}\bigl(|\widehat\gamma-\gamma|\ge t\bigr)\le \Var(\widehat\gamma)/t^2,
$
hence with confidence at least $1-\delta$ one may report
$
|\widehat\gamma-\gamma|\le \sqrt{\Var(\widehat\gamma)/\delta}.
$
\end{proposition}
...
At audit level, this is handled by: (i) declaring the derivative/smoothing rule, (ii) sweeping the step/regularization scale, and (iii) reporting the stability of $\widehat\gamma$ under these counterfactual baselines (Section~\ref{app:chi_reconstruction_protocol}).
A fully corrected errors-in-variables treatment is possible but is beyond the minimal self-contained scope here; see, e.g., standard measurement-error models \cite{Fuller1987,Carroll2006}.
\end{remark}
