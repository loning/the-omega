...
In addition to entropy bounds, we also use standard kinematic delay/lapse templates as external targets at the matching layer: Appendix~\ref{app:time_mass_delay} records the operational Wigner--Smith proxy and the GR reference formulas \eqref{eq:schwarzschild_lapse} and \eqref{eq:shapiro_delay}.
...
In particular, the time-symmetric spatial slice has two asymptotically flat ends ($\rho\to\infty$ and $\rho\to 0$) glued at the minimal surface $\rho=\rho_h$ (the Einstein--Rosen throat).
\end{proposition}
\begin{proof}
...
In particular, the only concrete object fixed in this paper is the protocol-level pointer-jump model of Definition~\ref{def:wormhole_pointer_jump}, together with the delay/overhead dictionaries used for matching (Appendix~\ref{app:time_mass_delay}).
\end{remark}
