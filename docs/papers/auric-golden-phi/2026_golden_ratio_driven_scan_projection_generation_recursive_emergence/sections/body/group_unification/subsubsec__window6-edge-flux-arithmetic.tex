...
\begin{conjecture}[\texorpdfstring{$\ZZ_2$}{Z2}–\texorpdfstring{$\ZZ_3$}{Z3} 挠率拼接的 \texorpdfstring{$\ZZ_6$}{Z6} 读出]\label{conj:window6-z2z3-to-z6-readout}
在 window-\(6\) 的审计接口下，boundary sheet-flip 给出一个规范的 \((\ZZ_2)^3\) 中心子格（推论 \ref{cor:fold-bin-boundary-center-z2}），
而 edge--flux skeleton 的 Smith 指纹给出一个非平凡的 \(\ZZ_3\) 挠率来源（推论 \ref{cor:window6-edge-flux-mod3-obstruction}）。
猜想：存在一个无需额外自由度的函子性拼接，使得可观测层出现一个有限化同态
\[
\Psi:\ (\ZZ_2)^3\times(\ZZ_3)\longrightarrow \ZZ_6,
\]
并且任一与该读出相容的 \(U(1)\) 型观测（即任一群同态 \(\rho:\ZZ_6\to U(1)\)）的像必为 \(6\) 次单位根。
在该猜想成立时，任一由 \(\rho\circ \Psi\) 所诱导的加性电荷读出只能取值于 \(\frac{1}{6}\ZZ/\ZZ\)。
\end{conjecture}
...
证毕。
\end{proof}
\input{sections/generated/eq_window6_mod571_spectral_collision_audit}
