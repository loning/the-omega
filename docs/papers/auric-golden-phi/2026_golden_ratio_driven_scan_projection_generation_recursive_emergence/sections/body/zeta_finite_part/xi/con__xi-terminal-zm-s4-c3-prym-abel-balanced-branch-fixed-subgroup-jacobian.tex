...
由单首多项式判别式公式
\[
\Disc_w(H)=\Res_w\!\bigl(H,\partial_wH\bigr)
=4^4\Res_w\!\bigl(H,w(w^2-A)\bigr)
=256\,H(0;X)\,\Res_w\!\bigl(H,w^2-A\bigr).
\]
再由
\[
H(0;X)=A^2-B^2f,\qquad
\Res_w\!\bigl(H,w^2-A\bigr)=\bigl(-B^2f\bigr)^2,
\]
得
\[
...
\begin{conclusion}[Prym 极化型的刚性与三维因子极化型约束]\label{con:xi-terminal-zm-prym-polarization-type-rigidity-11112}
设
\[
E=\widetilde{C}/S_3,\qquad
\pi:C=\widetilde{C}/C_3\longrightarrow E
\]
为由 \(C_3\le S_3\) 诱导的二次覆盖，记
\[
P:=\operatorname{Prym}(C/E),
\]
\(\lambda_P\) 为由 \(\Jac(C)\) 主极化诱导的 Prym 极化。
则成立：
\begin{enumerate}
  \item \(|\ker(\lambda_P)|=4\)；
  \item \(\dim P=5\) 且
  \[
  \mathrm{type}(\lambda_P)=(1,1,1,1,2);
  \]
  \item 若在 \(\QQ\)-同源意义下 \(P\sim_{\QQ}J_{\mathrm{LY}}\times B\)（\(\dim J_{\mathrm{LY}}=2,\ \dim B=3\)），
  则可选同源 \(\varphi:J_{\mathrm{LY}}\times B\to P\) 使
  \[
  \varphi^*\lambda_P=\lambda_{J_{\mathrm{LY}}}\boxtimes \lambda_B,
  \]
  且
  \[
  \mathrm{type}(\lambda_B)=(1,1,2).
  \]
\end{enumerate}
\end{conclusion}
\begin{proof}[证明要点]
由结论 \ref{con:xi-terminal-zm-c3-quotient-to-hly-minimal-ramification-type}，\(g(C)=6\)；
又 \(g(E)=1\)，故 \(\dim P=g(C)-g(E)=5\)。
对分歧二次覆盖 \(\pi:C\to E\)，设分歧点数为 \(r\)。
由 Hurwitz 公式
\[
2g(C)-2=2(2g(E)-2)+r
\]
得 \(r=10\)。
分歧二次覆盖 Prym 极化型的一般公式给出
\[
\mathrm{type}(\lambda_P)=\bigl(1^{\,r/2-1},\,2^{\,g(E)}\bigr),
\]
故此处
\[
\mathrm{type}(\lambda_P)=(1,1,1,1,2),
...
并由
\[
|\ker(\lambda_P)|=\Bigl(\prod_i d_i\Bigr)^2
\]
得 \(|\ker(\lambda_P)|=4\)。

再由结论 \ref{con:xi-terminal-zm-s4-c3-prym-leyang-equivalence} 的同源分解
\(
P\sim_{\QQ}J_{\mathrm{LY}}\times B
\)，
可取与幂等分解相容之同源 \(\varphi\)，使拉回极化分裂为外积。
由于 \(J_{\mathrm{LY}}\) 为 Jacobian，\(\lambda_{J_{\mathrm{LY}}}\) 主极化型为 \((1,1)\)，
故核大小 \(4\) 完全由 \(B\) 因子贡献，即 \(|\ker(\lambda_B)|=4\)。
结合 \(\dim B=3\) 与极化型整除链条件，唯一可能为 \((1,1,2)\)。证毕。
\end{proof}
