...
\begin{conclusion}[全迹矩与例外幂和的精确差分：非例外谱通道的封闭贡献]\label{con:xi-terminal-replica-softcore-trace-exceptional-difference}
对任意整数 \(m\ge 1\) 与 \(q\ge 2\)，成立严格恒等式
\[
\boxed{\;
\Tr\!\bigl((T^{(q)})^m\bigr)-S_m(q)=2^{-m}\Bigl(L_m^{q}-\Omega_m(q)\Bigr)\; } .
\]
因此除例外谱之外的全部谱部分对第 \(m\) 阶迹矩的贡献完全由单一差项 \(L_m^{q}-\Omega_m(q)\) 给出。
\end{conclusion}
\begin{proof}[证明要点]
...
\begin{conclusion}[\texorpdfstring{$q$}{q}-方向最小线性递推的显式特征多项式：分拆根与 Lucas--Pell 二次根]\label{con:xi-terminal-replica-softcore-q-recurrence-characteristic-upper-bound}
固定 \(m\ge 1\)，令
\[
\mathcal B_m:=\{B_\lambda:\lambda\vdash m\}\subset\ZZ_{>0}
\]
为分拆像的数值集合（允许不同 \(\lambda\) 给出相同 \(B_\lambda\) 的重合）。
记
\[
P_m(x):=\prod_{b\in\mathcal B_m\cup\{L_m\}}(x-b)\in\ZZ[x],
\qquad
Q_m(x):=\bigl(x^2-L_m x+(-1)^m\bigr)\prod_{b\in\mathcal B_m}(x-b)\in\ZZ[x].
\]
则数列
\[
q\longmapsto 2^m\Tr\!\bigl((T^{(q)})^m\bigr)
\quad\text{满足以 }P_m\text{ 为特征多项式的齐次线性递推},
\]
而数列
\[
q\longmapsto 2^m S_m(q)
\quad\text{满足以 }Q_m\text{ 为特征多项式的齐次线性递推}.
\]
换言之，\(\Tr((T^{(q)})^m)\) 与 \(S_m(q)\) 在 \(q\)-方向上的全部指数率分别由 \(\mathcal B_m\cup\{L_m\}\) 与 \(\mathcal B_m\)（并加上二次 Lucas--Pell 因子）完全控制。
\end{conclusion}

\begin{conclusion}[\texorpdfstring{$q$}{q}-方向有理型生成函数：例外幂和的 C-finite 刚性]\label{con:xi-terminal-replica-softcore-q-ordinary-genfunc-partial-fractions}
固定 \(m\ge 1\)，将 \(S_m(q)\) 按结论 \ref{con:xi-terminal-replica-softcore-partition-exceptional-closed-form} 的右端闭式延拓至 \(q\ge 0\)。
并定义普通生成函数 \(\mathcal G_m(z):=\sum_{q\ge 0}S_m(q)\,z^q\)。
\[
\boxed{\;
\mathcal G_m(z)
=\frac1{2^m}\left(\frac{1}{1-L_m z+(-1)^m z^2}
\;+\;\sum_{\lambda\vdash m}\frac{c_{m,\lambda}}{1-B_\lambda z}\right)\; } .
\]
因此，对固定 \(m\)，序列 \(q\mapsto S_m(q)\) 必为常系数线性递推序列；其极点集合由 \(\{B_\lambda^{-1}:\lambda\vdash m\}\) 与二次因子 \(1-L_m z+(-1)^m z^2\) 的根完全决定。
\end{conclusion}
\begin{proof}[证明要点]
由结论 \ref{con:xi-terminal-replica-softcore-partition-exceptional-closed-form} 的右端闭式（视作对 \(q\ge 0\) 的定义延拓），有
\(
2^mS_m(q)=\Omega_m(q)+\sum_{\lambda\vdash m}c_{m,\lambda}B_\lambda^{q}
\)。
...
\)；
并对每个 \(\lambda\) 逐项求和得 \(\sum_{q\ge 0}B_\lambda^{q}z^q=(1-B_\lambda z)^{-1}\)，代回并除以 \(2^m\) 即得所示有理分式表达。证毕。
\end{proof}
...
\begin{conclusion}[主导谱与严格间隙：\texorpdfstring{$q\to\infty$}{q->infty} 的指数级集中]\label{con:xi-terminal-replica-softcore-exceptional-moments-exponential-hierarchy}
固定整数 \(m\ge 3\)，令
\[
\alpha_0:=2^m,
\qquad
...
\]
并且分拆指数底数的前三大值严格满足 \(B_{1^m}=2^m>B_{(2,1^{m-2})}=3\cdot 2^{m-2}>B_{(3,1^{m-3})}=5\cdot 2^{m-3}\)，且对其余 \(\lambda\notin\{1^m,(2,1^{m-2}),(3,1^{m-3})\}\) 有 \(B_\lambda\le 9\cdot 2^{m-4}<5\cdot 2^{m-3}\)。
因此 \((\varphi/2)^m\le 5/8\) 且 \(\delta_m=5/8\)，从而余项可写为 \(O((5/8)^q)\)，并且 \(\Omega_m(q)\) 在该尺度下严格次指数主导。
\end{conclusion}
\begin{proof}[证明要点]
...
其余含 \(1\) 的 necklace 至少含两个 \(0\)。
若零计数为 \(r=2\)，则单簇型给出 \(\tau=F_{5}2^{m-3}=5\cdot 2^{m-3}\)，而非单簇型唯一对应于分拆 \((2,2,1^{m-4})\) 并满足 \(\tau=F_{4}^2\,2^{m-4}=9\cdot 2^{m-4}\)。
若 \(r\ge 3\)，由结论 \ref{con:xi-terminal-replica-softcore-fixed-zero-count-trace-extremal} 得 \(\tau(\eta)\le F_{r+3}2^{m-r-1}\le F_{6}2^{m-4}=8\cdot 2^{m-4}\le 9\cdot 2^{m-4}\)。
