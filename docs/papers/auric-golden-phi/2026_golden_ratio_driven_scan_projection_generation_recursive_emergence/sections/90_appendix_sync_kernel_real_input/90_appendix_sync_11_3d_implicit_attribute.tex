...
f(\lambda;u)=\lambda^3-2\lambda^2-(u+1)\lambda+u,
\]
RH-sharp 等号点由负根主导，故必有 \(\lambda_+(u)=x^2\)、\(\lambda_-(u)=-x\)（\(x>1\)）。
因此
\[
f(\lambda;u)=(\lambda-x^2)(\lambda+x)(\lambda-r),
\qquad r=2-x^2+x,
\]
从而
\[
...
x^5-3x^3-x^2+2x-1=0.
\]
对 \(x\) 消元即回到命题 \ref{prop:real-input-40-collision-rhk-window} 的五次方程；因此临界 \(u_R\) 是该五次的唯一正实根（从而为代数数）。
\end{remark}
