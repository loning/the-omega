...
def main() -> None:
    ...
    run([py, "scripts/exp_centerwired_decoder_demo.py", *(["--force"] if args.force else [])], cwd=cwd)
    ...
    if bam_pausing_opt:
        ...
        if not stop_candidates.exists():
            ...
            if not py_pysam:
                ...
                run([py, "scripts/exp_riboseq_pause_bam_window_centerwired_features.py", *(["--force"] if args.force else [])], cwd=cwd)
    ...
    if not refseq_quick:
        run(
            [
                py,
                "scripts/exp_stop_context_plus4_stratified_controls.py",
                "--k",
                str(int(args.refseq_stop_window)),
                *(["--force"] if args.force else []),
            ],
