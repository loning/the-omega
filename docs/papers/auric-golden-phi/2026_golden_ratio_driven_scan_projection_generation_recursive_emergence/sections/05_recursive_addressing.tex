...
\begin{corollary}[\texorpdfstring{$\sigma$}{sigma}-代数的保守性：递归可见域不扩张]\label{cor:recursive-addressing-visible-sigma-conservative}
沿用命题 \ref{prop:recursive-addressing-refinement} 的记号。
设新层概念族 \(\mathcal{C}^{(L+1)}=\{C_1^{(L+1)},\dots,C_{r_{L+1}}^{(L+1)}\}\) 由若干派生柱事件 \(C^{(L+1)}_{a,t}\) 选取而成（见上文派生柱事件的定义）。
令
\[
\mathcal{G}^{(L+1)}:=\sigma\bigl(T^{-t}C_j^{(L+1)}:\ 1\le j\le r_{L+1},\ t\in\ZZ\bigr)
\]
为由新层概念及其平移生成的 \(\sigma\)-代数。
则成立包含关系
\[
...
\begin{proof}
由命题 \ref{prop:recursive-addressing-refinement}，每个派生柱事件 \(C^{(L+1)}_{a,t}\) 都属于 \(\mathcal{G}^{(L)}\)。
而每个 \(C_j^{(L+1)}\) 按构造选自此类派生柱事件族，故 \(C_j^{(L+1)}\in\mathcal{G}^{(L)}\)；
再由 \(\mathcal{G}^{(L)}\) 在平移 \(T^{-t}\) 下的封闭性与 \(\sigma\)-代数生成的最小性，\(\mathcal{G}^{(L+1)}\subseteq\mathcal{G}^{(L)}\) 成立。证毕。
\end{proof}
