...
| 协议 RG 算子闭合（operator closure） | 把 uplift+coarse-graining 明确化为 $16\\times16$ 的协议 RG 算子 $F_n$ 与加权族 $\widehat F_n(t)$；并用张量母空间给出二点核/方差的 resolvent-trace 口径；配套给出可复现的谱半径/极点屏障与 predict-vs-recompute 审计表 | Math/Audit/Prot | `subsec:kernel_operator_closure`, `app:protocol_rg_operator_closure`, `tab:kernel_rg_operator_sanity`, `tab:kernel_rg_operator_backreaction`, `tab:kernel_rg_resolvent_trace_audit`, `tab:kernel_rg_weighted_pole_barrier` | [x] |
