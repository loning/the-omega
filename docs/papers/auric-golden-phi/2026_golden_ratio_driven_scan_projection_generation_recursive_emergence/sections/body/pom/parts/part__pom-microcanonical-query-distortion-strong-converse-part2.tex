...
\begin{conjecture}[Schur--凹性与均匀极大原理]\label{conj:pom-diagonal-rate-schur-concavity}
固定 \(\delta\in[0,1)\)，映射 \(w\mapsto R_w(\delta)\) 在单纯形内点上为 Schur--凹：若 \(v\prec w\)，则
\[
R_v(\delta)\ \ge\ R_w(\delta).
\]
等价地，在固定支撑大小 \(n\) 下，\(R_w(\delta)\) 的最大值由均匀分布 \(U_n\) 取得。
该猜想与端点恒等式 \(R_w(0)=H(w)\)、二点字母表的闭式解（定理 \ref{thm:pom-diagonal-rate-binary-closed-form}）以及均匀分布的显式公式（定理 \ref{thm:pom-diagonal-rate-uniform-closed-form}）相容，并与缩放分布的主序插值（定理 \ref{thm:pom-diagonal-rate-pi-majorization-chain}）所揭示的单调结构一致。
\end{conjecture}
