...
% (User input window)
% 2026-02-17 20:15:59（Asia/Singapore）
...
\[
Z_m(q)=\frac{R_m}{(K_m-1)!}\,q^{K_m-1}M_m^{\,q}\Bigl(1+O\!\bigl(q^{-1}\bigr)\Bigr).
\]
从而最大纤维与其重数可由比值在 \(q\)-轴上严格抽取：
\[
M_m=\lim_{q\to\infty}\frac{Z_m(q+1)}{Z_m(q)},
\qquad
K_m-1=\lim_{q\to\infty}q\left(\frac{Z_m(q+1)}{M_m Z_m(q)}-1\right).
\]
\end{theorem}
\begin{proof}
将
...
\]
由 \((1-M_m z)^{-K_m}=\sum_{q\ge 0}\binom{K_m+q-1}{K_m-1}M_m^{\,q}z^q\) 与解析因子的系数抽取（或等价的有限部分分式展开）得到
\[
Z_m(q)=R_m\,\binom{K_m+q-1}{K_m-1}M_m^{\,q}+O\!\bigl(q^{K_m-2}M_m^{\,q}\bigr),
\]
而
\(
\binom{K_m+q-1}{K_m-1}=\frac{q^{K_m-1}}{(K_m-1)!}\bigl(1+O(q^{-1})\bigr)
\)
给出主项展开。
比值极限由
\(
Z_m(q+1)/Z_m(q)=M_m\bigl(1+(K_m-1)/q+O(q^{-2})\bigr)
\)
直接推出。证毕。
\end{proof}
