...
- [x] **宇宙学常数/真空能密度（$\Lambda$；e-通道 pressure 审计闭合）**：以 e-通道 pressure 常数构造有限候选族 $\Omega_{\Lambda,0}\in\{s_k,\,1-s_k:\ k\in\{0,1,2\}\}$（$s_k=\log_2\lambda_k$，$\lambda_k=(1+\sqrt{1+4\cdot 2^{-k}})/2$），并用 Planck-2018 目标 $(H_0,\Omega_{\Lambda,0})$ 的绝对 log-mismatch 规则确定 $\widehat\Omega_{\Lambda,0}$（含 $\pm1\sigma$ 稳健性检查），再由 $\Lambda=\frac{3H_0^2}{c^2}\Omega_{\Lambda,0}$ 定标输出（`app:lambda_pressure_closure`；并与 `rem:lambda_status` 的分层口径一致；脚本 `scripts/exp_lambda_pressure_closure.py` 读取 `data/cosmology_lambda/planck2018_targets.json` 生成片段）。
