...
本小节给出三条严格超出上述“等价式 + 位置压缩律”的\emph{定量}结论：离线零点不仅被压入端点薄层，而且在 \(\mathscr{C}_{\zeta}\) 上诱导一条留数强度同阶压缩律；并且该盘内极点在视界多极矩 \(\{c_n\}\) 上强迫指数型增长尾项，从而在矩谱半径意义下给出一条与 Ramanujan temperedness 同型的二分判据。
