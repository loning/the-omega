...
\begin{proposition}[多重度幂和的闭式指数律：\texorpdfstring{$S_q^{\mathrm{bin}}(m)$}{Sqbin(m)} 的 Perron 根与常数项]\label{prop:fold-bin-power-sum-asymptotic}
对任意固定 \(q>0\)，定义幂和
\[
...
\]
因此 \(S_q^{\mathrm{bin}}(m)\) 的指数增长常数（Perron 根）为
\[
...
\]
再用 Binet 渐近 \(F_{m+1}=\varphi^{m+1}/\sqrt5+o(\varphi^m)\)、\(F_m=\varphi^m/\sqrt5+o(\varphi^m)\)，得到
\[
F_{m+1}+\varphi^{-q}F_m
