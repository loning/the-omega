\section[统一编码与证明携带审计]{地址—残差—证书的统一 Gödel 化编码与证明携带审计：WSOS 证书、Rouché 模板、见证提取与统一编译}\label{app:godel-wsos-rouche}

本附录补齐并推进两类此前缺口：\textbf{(i)} 地址/残差/证书材料的统一自然数编码层（Gödel 化、可判定互逆）；\textbf{(ii)} unitary slice 审计接口的\textbf{证明携带化}与\textbf{可复现}数值证书协议，并在此基础上给出一个可执行的 BridgeRH \textbf{有限区域零点计数证书模板}以及“第四寄存器需求”的\textbf{可判定见证提取机制}。最后给出统一的“运算涌现编译定理”，覆盖四则、商余、布尔运算与（在可计算语义域内的）ZFC 构造性运算族。

\subsection{统一 Gödel 编码层：\texorpdfstring{$\Addr$}{Addr} 的可判定注入与可判定反解}\label{app:godel-addr-enc}

附录 \ref{app:inject-code-audit} 已给出 prime-register 残差承载所需的 $\mathrm{code}/\mathrm{decode}$ 与 Gödel（素数幂）编码。本节进一步给出一个\textbf{统一的注入/编码层}，把“对象—残差—证书（含数值审计材料）”压缩为单个自然数，以便在离线审计、跨系统传输与 PW 可判定核验片段中统一处理。
...
附录 \ref{app:inject-code-audit} 使用 Cantor 配对 $\pi$ 及其逆算法，其逆过程依赖整数平方根。定义 \ref{def:eo-pair} 的偶奇配对提供一种不涉及平方根的替代；两者均给出 $\NN^2\simeq\NN$ 的可判定双射，本文后续编码层可任选其一。为突出“纯整除/赋值”的核验口径，本附录在统一编码中优先使用 $\langle\cdot,\cdot\rangle_2$。
\end{remark}
...
\paragraph{有理数/多项式/矩阵的有限数据编码（证书材料接口）}
在本文“证明携带”语境中，证书材料最终需落在\textbf{整数或有理数}上。为此，固定以下标准编码（均为可判定互逆）：
\begin{itemize}
...
\end{itemize}
以上仅是一个最小接口：\emph{任何在 PW 证书、prime-register 残差与数值审计协议中出现的“有限数据对象”均可归约为有限整数/有理数/多项式/矩阵，从而统一编码进 $\NN$。}

\subsubsection{地址—残差—证书（ARC）四元组与统一编码}

为把“对象—残差—证明—审计”统一压缩到自然数层，本节把可审计地址对象抽象为四元组记录。

\begin{definition}[ARC 地址子域（记录形呈现）]\label{def:arc-record}
定义一个用于审计与证书协议的\textbf{可审计地址子域} $\Addr_{\mathrm{ARC}}\subseteq\Addr$：其元素可被呈现为四元组记录
$$
A=(m,x,r,c)\in\NN^4,
$$
其中：
\begin{itemize}
  ...
  \item $r$ 为 prime-register 残差的编码（例如：事件序列经 $\mathrm{code}$ 后用 $\mathrm{gcode}$ 编码，或等价的有限支撑指数向量编码）；
  \item $c$ 为证书包（PW 证书、审计协议头、区间/有理证书材料等）的统一有限数据编码。
\end{itemize}
...
$$
并且 $\mathrm{Dec}$ 的定义域恰为 $\mathrm{Enc}(\Addr_{\mathrm{ARC}})$。因此 $\mathrm{Enc}$ 给出一个\textbf{完全形式化、可判定互逆}的自然数编码层，可作为后续“证明携带化数值协议”与 PW 证书包的统一承载接口。
\end{theorem}
...
\begin{remark}[从抽象 $\Addr$ 到可审计编码层]
POMmin（附录 \ref{app:pommin-formal}）仅要求 $\Addr$ 可数；本文的审计协议实际使用的地址均具备\textbf{有限语法表示}（例如有限门复合词与有限参数），因而可按定义 \ref{def:arc-record} 的“有限数据记录形”实现。定理 \ref{thm:arc-godel-invertible} 提供的并非新的数学公理，而是把既有“对象—残差—证书”的有限信息压缩到 $\NN$ 的\textbf{可回放审计层}：任何可核对证书均可编码为自然数并在可判定片段中核验其一致性。
\end{remark}

\subsection{unitary slice 审计的证明携带化：从稳定性判据到 WSOS 证书}\label{app:wsos-proof-carrying}

正文第 \ref{sec:fourth-register} 节已指出：在 unitary slice 类型锁定下，“临界线零点”可被编译为某个代数判据（例如单位圆内根、或 $[-2,2]$ 上根约束），从而可由根区间检查器核验。本节给出一种\textbf{完全离线可核对}的路线：把稳定性判据进一步转写为区间上的半正定性，并以\textbf{有理 WSOS（加权 SOS）证书}作为证明携带载体。

\subsubsection{从单位圆稳定性到区间半正定：Schur--Cohn 型归约（接口化陈述）}

在可控模型族中，unitary slice 上常见的结构是：对 $u\in\mathrm{Phase}$ 的重参数化 $x=u+u^{-1}\in[-2,2]$，得到一族关于 $z$ 的多项式
$$
\Phi(z;x)\in\QQ[x][z].
$$
记 $\Phi(\cdot;x)$ 的“Schur 稳定性”（全部根满足 $\abs{z}<1$）为 $\mathsf{SchurStable}(\Phi(\cdot;x))$。经典 Schur--Cohn 判据给出如下接口化归约（对具体矩阵形式不作展开）：

\begin{assumption}[Schur--Cohn 归约接口]\label{ass:schur-cohn-reduction}
存在一个由 $\Phi$ 的系数以有理运算构造的\textbf{对称矩阵多项式}
$$
H(x)\in\QQ[x]^{n\times n},\qquad H(x)^\top=H(x),
$$
使得对每个 $x\in[-2,2]$，
$$
\mathsf{SchurStable}(\Phi(\cdot;x))\quad\Longleftrightarrow\quad H(x)\succeq 0.
$$
\end{assumption}

该接口把“单位圆内根”转写为\textbf{一维紧区间上的半正定性}，从而可引入 WSOS 证书实现证明携带核验。
...
\begin{theorem}[unitary slice 审计的证明携带化（WSOS 版本）]\label{thm:unitary-slice-wsos}
在假设 \ref{ass:schur-cohn-reduction} 下，若存在 $\mathcal{C}_{\mathrm{WSOS}}$ 使得 $\mathrm{CheckWSOS}(H,\mathcal{C}_{\mathrm{WSOS}})=\mathrm{true}$，则对所有 $x\in[-2,2]$ 有 $\mathsf{SchurStable}(\Phi(\cdot;x))$ 成立；因此 unitary slice 上的稳定性成为一个\textbf{完全离线可核对}的结论。并且核验器 $\mathrm{CheckWSOS}$ 仅用到整数多项式加乘与相等判定，故可被编译为本文可判定核验层的一部分（例如在附录 \ref{app:pw-fragment} 的检查器框架上添加整数/多项式编码运算）。
\end{theorem}
...
\subsection{BridgeRH 的有限区域零点计数证书模板：Rouché--同伦不等式的可审计化}\label{app:rouche-certificate}
...
\begin{itemize}
  \item $\mathcal{M}$：边界网格（四条边的分段）、舍入模型头（精度 $p$、外向舍入方向、算法版本等）与可用的导数/Lipschitz 上界记录，用于把离散采样提升为连续边界的上/下界；
  \item $\mathcal{B}_{\mathrm{err}}$：对 $\abs{F-G}$ 在 $\partial R$ 上的上界证书；
  \item $\mathcal{B}_{\min}$：对 $\abs{F}$ 在 $\partial R$ 上的下界证书。
\end{itemize}
证书的数值后端可取附录 \ref{app:inject-code-audit} 的外向舍入区间协议：对每个边界小段输出包含真值的复区间盒并据此给出模长上/下界（参见 \cite{DaumasLesterMunoz2007} 的形式化区间算术路径）。
\end{definition}
...
$$
进一步，若能以 unitary slice 审计协议在 $\Re(s)=\tfrac12$ 上给出 $\Xi_\zeta$ 的零点下界计数证书，并与上式总数相等，则推出 $R_T$ 内全部经典零点均位于临界线（即有限高度 RH 的证书化版本）。
\end{corollary}

\begin{remark}[位置证书（可选强化）]
若将 $R_T$ 进一步划分为一族小矩形/小圆盘 $\{R_j\}$，并分别给出 $\mathcal{C}_{\mathrm{Rou}}(F,G;R_j)$，则可把“零点计数一致”细化为“每个子区域内零点重数一致”，从而得到零点位置的可审计定位（以区域盒作为精度）。
\end{remark}

\subsection{第四寄存器需求的可判定见证提取：从 PSD 失败到径向自由度}\label{app:witness-fourth-register}

第 \ref{sec:fourth-register} 节与附录 \ref{app:strictification} 给出了“径向模式必须引入第四寄存器”的结构性论证。本节把该论断提升为\textbf{可判定见证提取}：一旦三寄存器闭包下的 unitary slice soundness 失败，即可提取一个有限的有理反例见证，作为迫使扩张接口/寄存器的可核对材料。
...
命题 \ref{prop:wsos-soundness} 给出“存在 WSOS 证书 $\Rightarrow$ 半正定成立”的充分条件；定理 \ref{thm:psd-failure-witness} 给出“半正定失败 $\Rightarrow$ 存在可核对反例见证”的必要条件。二者合并为一个\textbf{证明携带的判别接口}：检查器要么接受并输出 WSOS 证书，要么拒绝并输出有理见证；两种输出均可离线核验，且均可被统一编码进 $\NN$。
\end{remark}

\subsection{运算涌现的统一编译定理：算术、布尔与构造性 ZFC}\label{app:unified-compiler}

附录 \ref{app:ops-zfc} 给出了“运算涌现”的可审计定义链与 ZFC 可定义运算的生成模板。本节给出一个统一的\textbf{编译定理}：在已实现基本算术门与证书核验的前提下，可把广泛的运算族统一编译为“地址化程序 + 证书包”，并在可判定核验层中离线验证。

\subsubsection{语义域：总函数与部分函数的分离}

对 $\NN$ 上的算术与布尔运算，语义为总函数/总谓词；对集合论运算，若要求覆盖任意外延集合则不可避免出现不可计算性。因此我们采用附录 \ref{app:ops-zfc} 的口径：以 \textbf{构造性语义域}（例如 hereditarily finite 集合 HF，或“由代码表示的可构造对象族”）为承载对象，把集合运算视为对\textbf{代码}的运算，从而回到 $\NN$ 上的可计算函数。

\subsubsection{统一编译与证书化}
...
\begin{theorem}[统一运算涌现编译（证书携带版）]\label{thm:unified-compilation}
在以下前提下：
\begin{itemize}
  \item 稳定算术层已实现并可审计（正文第 \ref{sec:emergent-arithmetic} 节）；
...
\begin{enumerate}
  \item 四则、整除、商余、最大最小、比较谓词与布尔联结均在同一编译框架下涌现；
  \item 在 HF（或等价的构造性编码域）上，配对、并、差、笛卡尔积、有限幂集与有限替换等集合运算可被化为 $\NN$ 上的（原始递归）可计算函数，从而同样可被编译并证书化；
  \item 对超出该构造性语义域的对象，求值器以 $\NULL$ 表示“不可在当前可审计地址体系内定义/核验”的严格边界（与 POM 的空值语义一致）。
\end{enumerate}
\end{theorem}
...
一旦该门槛以统一方式通过，则“有限高度零点一致性”可被完全证书化；并在极限意义上为全局结论提供清晰可分解的 proof obligations 结构。
