...
\noindent
上述五条结果把附录 \(H\) 的若干局部构造压缩为一条统一刚性链：有限深度 branch register 是 prime-slice 外置的前缀模型，prime-slice 的最小复杂度由活跃分支层精确锁定；进入模数投影后，全部信息亏损又被 Smith 曲率谱完全分类，并在 divisibility 格上形成精确 valuation；最终，这条素数寄存器刚性可以直接投射到整数椭圆半群这样的显式几何对象，迫使其在结构层上表现出无限的加法寄存器秩。
