...
\[
\operatorname{Idem}(\mathcal S_n):=\{N\in\mathcal S_n:\ N\diamond N=N\},
\]
\[
\operatorname{Idem}_k(\mathcal S_n):=
\{N\in \operatorname{Idem}(\mathcal S_n):\ |\operatorname{Im}(f_N)|=k\}
\qquad (1\le k\le n).
\]
则有精确计数公式
\[
\boxed{\;
\bigl|\operatorname{Idem}_k(\mathcal S_n)\bigr|=\binom{n}{k}k^{n-k}
\qquad (1\le k\le n),
\;}
\]
从而
\[
\boxed{\;
\bigl|\operatorname{Idem}(\mathcal S_n)\bigr|
=
...
\end{theorem}
\begin{proof}
由定理 \ref{thm:killo-finite-semigroup-prime-valuation-arithmetization} 与推论 \ref{cor:killo-projection-idempotent-prime-register}，\(\operatorname{Idem}(\mathcal S_n)\) 与集合 \([n]\) 上的全部幂等自映射一一对应。
...
沿用定理 \ref{thm:killo-finite-semigroup-prime-valuation-arithmetization} 的 \((\mathcal S_n,\diamond)\) 记号，并设
\[
Q_n:=\prod_{i=0}^{n-1}q_i,
\qquad
\mathcal K_n:=\{Q_n^c:\ c\in[n]\}\subset \mathcal S_n.
\]
则：
\begin{enumerate}
  ...
  \[
  \mathcal K_n=\{N\in\mathcal S_n:\ |\operatorname{Im}(f_N)|=1\};
  \]
  \item 对任意 \(a,b\in[n]\)，有
  \[
  \boxed{\;
  Q_n^a\diamond Q_n^b=Q_n^a,
  \;}
  \]
  故 \(\mathcal K_n\) 作为子半群同构于一个 \(n\) 元左零带；
  \item \(\mathcal K_n\) 是 \((\mathcal S_n,\diamond)\) 的唯一极小双边理想：任意非空双边理想 \(I\subseteq \mathcal S_n\) 都满足 \(\mathcal K_n\subseteq I\)。
\end{enumerate}
\end{theorem}
\begin{proof}
...
\[
f_{Q_n^a\diamond Q_n^b}=f_{Q_n^a}\circ f_{Q_n^b}=c_a\circ c_b=c_a,
\]
故 \(Q_n^a\diamond Q_n^b=Q_n^a\)。

最后证明第 (4) 条。任取非空双边理想 \(I\subseteq \mathcal S_n\)，取 \(N\in I\)。对任意 \(a,b\in[n]\)，由于 \(I\) 为双边理想，有
\[
Q_n^a\diamond N\diamond Q_n^b\in I.
\]
而在变换半群侧，
\[
f_{Q_n^a\diamond N\diamond Q_n^b}
=
...
故
\[
Q_n^a\diamond N\diamond Q_n^b=Q_n^a\in I.
\]
