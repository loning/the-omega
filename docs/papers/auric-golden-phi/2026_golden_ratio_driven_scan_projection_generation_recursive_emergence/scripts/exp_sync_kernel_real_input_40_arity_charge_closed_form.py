...
def _write_tex_coboundary_audit(path: Path, rows: Sequence[EdgeAuditRow]) -> None:
    ...
    lines.append(
        "\\caption{真实输入 40 状态核的 essential 20 状态核心上，arity-charge 的 $0/1$ 共边界证书的逐边核验表。"
        "对每条允许边 $u\\to v$，列出输入和位 $d\\in\\{0,1,2\\}$、输出位 $e\\in\\{0,1\\}$，一步电荷"
        " $\\chi=e-\\mathbf{1}_{\\{d=2\\}}$，以及势函数 $V$（定理~\\ref{thm:real-input-40-arity-charge-coboundary}）"
        " 与规范化后可观测量 $g=\\chi+V(v)-V(u)$。该表共列出 "
        + str(len(rows))
        + " 条边，且逐行满足 $g\\in\\{0,1\\}$，从而可审计地排除负电荷回路。}"
    )
...
def _write_tex_density_audit(path: Path, rows: Sequence[DensityAuditRow]) -> None:
    ...
    lines.append("\\end{tabular}")
    lines.append(
        "\\caption{真实输入 40 状态核的 essential 20 状态核心上，arity-charge 密度上界（定理~\\ref{thm:real-input-40-arity-charge-density-bound}）"
        "的逐边有限证书核验表。这里 $g(e)\\in\\{0,1\\}$ 为同调规范后的边指标（定理~\\ref{thm:real-input-40-arity-charge-coboundary}），"
        "$w(e)=1-2g(e)\\in\\{+1,-1\\}$，$h:\\Sigma_{\\mathrm{ess}}\\to\\{-2,-1,0\\}$ 为势函数。"
        "表中逐行满足 $w(e)+h(u)-h(v)\\ge 0$，从而沿闭轨求和推出 $\\sum_{e\\in\\gamma}g(e)\\le |\\gamma|/2$。}"
    )
