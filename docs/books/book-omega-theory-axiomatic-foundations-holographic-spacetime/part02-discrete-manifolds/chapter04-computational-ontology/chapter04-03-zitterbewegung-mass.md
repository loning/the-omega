...
![颤动质量](../../assets/chapter04-03-zitterbewegung-mass.png)
...
![颤动质量陷阱](../../assets/chapter04-03-zitterbewegung-mass-trap.png)
...
其中 $\hat{v}_{cl} = c^2 \hat{p} \hat{H}^{-1}$ 是我们熟知的经典群速度（$< c$），而 $\hat{\xi}(t)$ 是一个频率极高（$\omega \approx 2mc^2/\hbar$）、振幅极小（$\sim \lambda_c$ 康普顿波长）的震荡项。这个震荡就是 **颤动**。
...
**定义 4.1 (有效质量)**：
质量 $m$ 是粒子改变前进方向的 **转移概率密度**。在 DQCA 中，这对应于硬币算符的混合角 $\theta$。
...
2. **有质量粒子 (电子/夸克)**：对应于 $\theta \neq 0$。欧米伽单元内部的八元数乘法结构迫使波函数在传播时发生相位的 **非交换旋转**。这种旋转在投影到 4D 时空时，表现为左右手性的周期性转换。

**定理 4.3 (质量-频率等价原理)**：
一个费米子的静止质量 $m_0$ 严格等价于其在欧米伽晶格上的 **手性翻转频率 (Chirality Flip Frequency)** $\nu_{flip}$。
