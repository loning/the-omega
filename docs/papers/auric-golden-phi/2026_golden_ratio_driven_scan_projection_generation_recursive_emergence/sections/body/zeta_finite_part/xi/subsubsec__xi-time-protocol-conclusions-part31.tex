...
\begin{conclusion}[例外幂和的分拆闭式：\texorpdfstring{$S_m(q)$}{S_m(q)} 的全阶可计算分解]\label{con:xi-terminal-replica-softcore-partition-exceptional-closed-form}
...
\]
特别地，当 \(m\ge 3\) 时有 \((\varphi/2)^m\le 5/8\)，故 \(\delta_m=5/8\)，从而余项可写为 \(O((5/8)^q)\)。
...
其余含 \(1\) 的 necklace 至少含两个 \(0\)；若其零计数为 \(r\ge 2\)，由结论 \ref{con:xi-terminal-replica-softcore-fixed-zero-count-trace-extremal} 得
\(
\tau(\eta)\le F_{r+3}2^{m-r-1}\le 5\cdot 2^{m-3}=(5/8)\alpha_0
\)。
