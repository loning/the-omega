...
\begin{remark}[Non-claims: dynamics and curvature]\label{rem:intro-nonclaims}
To reduce the risk of over-interpretation, we make explicit what is \emph{not} claimed. The constructions reviewed here do not, by themselves, derive Einstein dynamics, reconstruct a Lorentzian metric, or compute curvature invariants from first principles. They provide a protocol-level notion of refinement time and candidate causality interfaces under explicit assumptions; any mapping to geometric curvature/dynamics requires additional interface hypotheses and controlled limits that are outside the scope of this review.
\end{remark}

\subsection{Terminology note: what ``superspace'' means (and what it does not)}\label{sec:intro:superspace-terminology}

Throughout the paper, the word \emph{superspace} is used exclusively in the quasicrystal/model-set sense of a \emph{cut-and-project embedding space} $G=E\times H$ (Section~\ref{sec:superspace:cps}). It is a \emph{representation space} for aperiodic patterns in $E$, not a claim about physical extra dimensions. In particular, this is \emph{not} the same as:
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Supersymmetric superspace} in quantum field theory, where one adjoins anticommuting (Grassmann) coordinates to spacetime and works with superfields.
  \item \textbf{Wheeler's superspace} in canonical general relativity, i.e.\ the space of 3-metrics modulo diffeomorphisms (configuration space of geometrodynamics).
\end{itemize}

No Grassmann structure and no configuration-space-of-metrics structure is assumed or used below. We use the superspace backbone because it provides a periodic object layer with explicit dual modules and a controlled projection mechanism, which makes the geometry-to-statistics interface auditable.
