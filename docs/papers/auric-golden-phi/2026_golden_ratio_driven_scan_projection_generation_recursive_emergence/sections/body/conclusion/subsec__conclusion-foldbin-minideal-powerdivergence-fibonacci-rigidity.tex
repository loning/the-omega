...
\noindent
因此，在 bin-fold 口径下出现了两类互补刚性。其一，审计偶数窗的最小退化层在对象层切出一个由 \(F_{m/2}\) 与 \(F_m\) 精确控制的纯矩阵理想，并在阿贝尔化层诱导出一个规范的 Fibonacci anomaly block；其相对秩在正则子覆盖外推下进一步锁定到 \(\varphi^{-2}\)。其二，相对均匀基线的幂散度极限谱在函数层退化为二指数系统，既满足精确二阶递推，又能由单个 \(\chi^2\)-型常数恢复 \(\varphi\) 并回收整条谱。由此，最小退化层的对象刚性与输出非均匀性的函数刚性便在同一 Fibonacci 机制下合并为彼此对应的两侧表述。
