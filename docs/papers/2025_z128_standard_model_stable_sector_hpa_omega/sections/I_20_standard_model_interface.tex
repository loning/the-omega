...
In this paper, we formulate this as an implementation budget: compensation has a cost, and the effective response/selection favors minimal discrepancy subject to protocol constraints (Axiom~\ref{ax:cap}).
...
\AuditTag The particle-as-type identification above is complemented by a unified interface contract used later:
...
\paragraph{A rank-3 Lie-seed refinement of the \texorpdfstring{$18\oplus 3$}{18+3} interface (interface).}
\InterfaceTag The same rigid split also singles out a compact ``GUT-seed'' candidate at the algebraic level:
interpreting the three boundary labels as Cartan-like directions and the $18$ cyclic labels as root-like directions locks the simple type to $B_3/C_3$ (dimension $21$, Weyl group order $48$), with a $6+12$ two-length fingerprint inside $X_6^{\mathrm{cyc}}$.
See Section~\ref{sec:folding_core} (after Corollary~\ref{prop:cyc_bdry_6}) and Appendix~\ref{subsec:fold6_b3c3_seed_generated}.
...
To connect this protocol-level scalar doublet representative to a Standard Model EFT embedding without adding a new primitive $X_6$ label, we close the scalar uplift quantum numbers by a bounded-family CAP rule: within a finite candidate family, the unique minimizer that admits the standard renormalizable Yukawa structures is $(SU(3),SU(2))_Y=(1,2)_{1/2}$ (Appendix~\ref{app:scalar_interface_audits}, Table~\ref{tab:higgs_quantum_numbers_cap_closure}).

\begin{figure}[t]
\centering
\fbox{\parbox{\textwidth}{\centering\textit{[omitted: figures/adaptive/higgs\_geometry/higgs\_doublet\_structure\_m10]}}}
...
\begin{figure}[t]
\centering
\fbox{\parbox{\textwidth}{\centering\textit{[omitted: figures/adaptive/higgs\_geometry/higgs\_doublet\_structure\_m10\_3d]}}}
...
\subsection{Hilbert-screen lattice QFT bridge and perturbative diagrammatics (interface)}
\label{subsec:hilbert_screen_lattice_qft_bridge}

\paragraph{Scope.}
\InterfaceTag The protocol language of this paper already contains the core primitives needed to host a lattice-style quantum field theory on the Hilbert screen: a finite locality graph, vertex/edge data carrying internal redundancy, and deterministic coarse-graining/RG steps.
Here we record a minimal bridge to standard perturbative QFT combinatorics (Wick contractions and Feynman diagrams) \emph{without} adding new theorem-level inputs to the folding core.
The role of this bridge is to identify a concrete class of finite-dimensional Gaussian measures and local interaction functionals on the screen for which diagrammatics is a purely finite, auditable calculation.
This bridge fixes a finite carrier and its exact Wick/graph combinatorics at finite resolution; it does not claim a continuum limit, Lorentz covariance, or constructive renormalization on $\RR^{1,3}$ (see Appendix~\ref{app:lattice_qft_bridge_hilbert_screen}, \paragraph{Scope boundary}).

\paragraph{Unified spacetime dictionary (six-line contract).}
\InterfaceTag We use the following manuscript-wide dictionary:
\begin{itemize}
\item tick-time: the ontic scan order $t$ (Axiom~\ref{ax:readout_sequentiality});
\item space-as-display: the Hilbert-screen display graph $G_n$ (Section~\ref{sec:hilbert_addressing});
\item lattice-QFT carrier: $\Lambda_{T,n}=\ZZ_T\times V(G_n)$ (Appendix~\ref{app:lattice_qft_bridge_hilbert_screen}, Section~\ref{subsec:lattice_qft_spacetime_dictionary});
\item scale-time/RG-time: resolution coordinates ($m$ or $r$) organizing effective descriptions (Proposition~\ref{prop:rg_dictionary_r});
\item delay-time: operational time observables defined from responses over tick-time (Appendix~\ref{app:k4_delay_audit}; Appendix~\ref{app:protocol_horizon_tick_trap});
\item wormhole-like channels: paid pointer-jump shortcuts with explicit ledgers and wormhole-on/off counterfactual baselines (Definition~\ref{def:wormhole_pointer_jump}; Section~\ref{sec:mass_spectrum_closure}).
\end{itemize}

\paragraph{Screen as a finite lattice.}
Fix a Hilbert order $n$ and let $G_n$ be the corresponding display graph (Definition~\ref{def:addressing_map_graph}).
We treat $V(G_n)$ as the lattice sites and $E(G_n)$ as the nearest-neighbor links.
Protocol locality (bounded $L^1$ jumps of the Hilbert scan) supplies the locality discipline needed for lattice actions.

\paragraph{Discrete spacetime and shortcut channels (dictionary alignment).}
\InterfaceTag The tick-first stance of this paper provides a natural fully finite lattice-QFT carrier: tick-time supplies a discrete time axis and Hilbert addressing supplies a spatial locality graph.
Concretely, an audit-facing spacetime carrier is the finite product
$\Lambda_{T,n}:=\ZZ_T\times V(G_n)$ with unit tick steps in $\ZZ_T$ and spatial neighbor edges in $E(G_n)$ (Appendix~\ref{app:lattice_qft_bridge_hilbert_screen}, Section~\ref{subsec:lattice_qft_spacetime_dictionary}).
Within the manuscript-wide black-hole/wormhole vocabulary, any wormhole-like shortcut is \emph{not} a new geometric premise but a paid protocol-level pointer-jump edge that augments the readout graph (Definition~\ref{def:wormhole_pointer_jump}); such channels are admissible only under an explicit shortcut ledger and a wormhole-on/off counterfactual baseline (Section~\ref{sec:mass_spectrum_closure}).
\AuditTag Two protocol-side witnesses are recorded for this vocabulary: (i) a shortest-path distance reduction audit on the augmented readout graph (Appendix~\ref{app:wormhole_pointer_jump_audit}), and (ii) a finite-rank update $\Delta$ bookkeeping witness in the operator dictionary reporting induced logdet and delay-proxy increments (Appendix~\ref{app:equivalence_ladder_certificates}, Table~\ref{tab:wormhole_finite_rank_delay_logdet_audit}).
\AuditTag These two witnesses close the same shortcut idea on two carriers: the graph-side witness is purely geometric (shortest paths on an augmented readout graph), while the operator-side witness is a dictionary-only encoding (finite-rank update bookkeeping of resolvent/determinant proxies).

\paragraph{Field content from the \texorpdfstring{$18\oplus 3$}{18+3} interface.}
On sites $x\in V(G_n)$ we place:
(i) a family of Grassmann fields $\{\psi_f(x)\}_{f\in\mathcal{F}_{\mathrm{SM}}}$ corresponding to the $18$ closed chiral multiplets (Theorem~\ref{thm:labeling_unique}; Table~\ref{tab:sm_labeling_table}),
and (ii) when the scalar-sector uplift dictionary is activated, a complex Higgs-doublet site field $H(x)\in\CC^2$ (Proposition~\ref{prop:scalar_sector_closed}; Appendix~\ref{app:scalar_interface_audits}).
On links $(x,y)\in E(G_n)$ we place a triple of gauge-link variables representing the three compensation classes,
\[
U_{xy}^{(1)}\in U(1),\qquad U_{xy}^{(2)}\in SU(2),\qquad U_{xy}^{(3)}\in SU(3),
\]
to be read as a lattice realization of compensating transport (Proposition~\ref{prop:gauge_compensation}) in a continuum dictionary where the CAP-minimal factors are $U(1)\times SU(2)\times SU(3)$ (Proposition~\ref{prop:channel_to_gauge}).

\paragraph{A minimal Gaussian anchor and the propagator.}
To connect to perturbative diagrammatics, choose a finite-dimensional quadratic form (a ``free kernel'') on the screen.
One canonical protocol-native choice is to reuse the explicitly audited block-quotient RG operator at the anchor (the $16\times 16$ matrix $F_n$ in the balanced-chain construction; Appendix~\ref{app:reproducibility}) and build from it a symmetric positive kernel $K$ on the selected site/quotient variables.
The corresponding Gaussian theory is determined by $K$; its two-point function (propagator) is the inverse kernel $G=K^{-1}$ (or, equivalently, the resolvent family $(K-zI)^{-1}$ used in resolvent-trace audits).
All of these objects are finite matrices at fixed resolution, hence computable and auditable.

\paragraph{Local interactions and Wick \texorpdfstring{$\to$}{->} Feynman graphs.}
Given the closed SM quantum numbers and the scalar uplift dictionary, one may add local interaction terms (nearest-neighbor gauge-covariant couplings, Higgs potential, and Yukawa terms) exactly as in lattice gauge theory.
Expanding expectations in the interaction about the Gaussian measure yields a finite perturbation series whose coefficients are sums over Wick contractions; each contraction is a propagator factor $G(x,y)$, and the resulting combinatorics is encoded by Feynman graphs on the lattice.
In this sense, the Hilbert-screen protocol supplies a concrete finite carrier that reproduces the diagrammatic skeleton of standard QFT while remaining within the tick\,+\,CAP audit discipline.
Appendix~\ref{app:lattice_qft_bridge_hilbert_screen} records a formal audit-facing version of this bridge and a minimal reproducible anchor calculation.
It also records a visual carrier for the ``internal dimension'' viewpoint: at fixed spatial axes on the Hilbert screen, uplift in $m$ exposes additional suffix-bit channels that behave as an internal fiber over each site (Figures~\ref{fig:internal_dimension_fractal_unfolding_2d} and \ref{fig:internal_dimension_fractal_unfolding_3d}).
