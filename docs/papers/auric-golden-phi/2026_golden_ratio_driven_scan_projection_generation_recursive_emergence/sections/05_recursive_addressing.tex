...
对任意候选地址 \(a\in A^{(L+1)}\)，在地址未被给定之前不对其派生概念赋值；一旦地址 \(a\) 被选定，概念值即为二元投影 \(\ind_{C_a^{(L+1)}}(x)\)。
换言之，递归地址化在语义上可被视为一种\emph{可读出前缀}的内生化机制：地址作为前缀先于取值；当地址缺失时，评价谓词 \(x\mapsto \ind_{C_a^{(L+1)}}(x)\) 不构成一个可执行的总定义读出（其失败不应被误读为数值 \(0\)），并按本文约定以 \(\mathsf{NULL}\) 表示“不可寻址/不定义”。
从而层级递归满足：
\begin{itemize}
