\paragraph{加权 clique--Euler 并集公式与共振整函数系数刚性}
在前一段已经把谱零点的 \((\ZZ_2)^r\)-轨道抵消、零余类有限交截的 Helly--\(2\) 刚性、flag nerve 结构以及六倍窗上的归一化零点密度率固定下来之后，还可继续沿同一条算术--解析主线抽取出下列五条结果。它们分别刻画谱零点集合的精确加权 clique--Euler 公式、碰撞上界的约数兼容图闭式、共振整函数 \(\mathcal F_\varphi\) 的对数 Taylor 级数与倒数零矩、其偶系数的显式 Newton 递推与严格对数凹性，以及截断深度对相对误差预算的可审计控制。
