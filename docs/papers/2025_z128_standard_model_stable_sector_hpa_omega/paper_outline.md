...
- **H.3 Wish 更新算子（Iface+Open）**
 ...
 - **H.4 解释层统一图景：复指数、幺正球面与“扫描轨迹”（\AuditTag；not used in proofs）**
...
- **建议新增（附录/补充模块；按需进入主文或附录）**
  - `sections/appendices/00_wish_motive_definitions.tex`：Wish/Motive/teleological dynamics 的严格定义与模板性定理（以复用为主）。
  - （可选）`sections/appendices/36_arrow_of_time_semigroup_notes.tex`：若 F.0 的材料过长，可把细节证明/变体模板外置到该附录并在主文引用。
