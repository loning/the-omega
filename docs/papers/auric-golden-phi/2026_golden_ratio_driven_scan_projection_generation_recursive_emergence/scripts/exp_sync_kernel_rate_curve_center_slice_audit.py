...
def main() -> None:
    ...
    tex_lines.append("\\end{corollary}")
    tex_lines.append("")
    tex_lines.append("\\begin{corollary}[单位根扭曲的收敛域阈值（$p=5$ 被排除）]\\label{cor:rate-center-omega-threshold}")
    tex_lines.append("令 $\\omega_p=e^{2\\pi i/p}$。则 $|\\omega_p-1|=2\\sin(\\pi/p)$。")
    tex_lines.append("由")
    tex_lines.append("$$")
    tex_lines.append(f"|\\omega_5-1|\\approx {d5_str}>R_\\star,\\qquad |\\omega_7-1|\\approx {d7_str}<R_\\star,")
    tex_lines.append("$$")
    tex_lines.append("并利用 $2\\sin(\\pi/p)$ 随 $p$ 单调递减，可得对所有 $p\\ge 7$ 均有 $|\\omega_p-1|<R_\\star$。")
    tex_lines.append("因此任何围绕 $u=1$ 的局部解析展开，在 $u=\\omega_p$ 处对 $p\\ge 7$ 处于严格收敛域内，而在 $p=5$ 上必超出该域。")
    tex_lines.append("\\end{corollary}")
    tex_lines.append("")
