...
def _write_dirichlet_mertens_tensor_tex(
    *,
    ...
    lines.append(
        f"% ({m1},{m2},{m3}) Dirichlet--Mertens constants tensor for (chi mod {m1}, N_- mod {m2}, N_2 mod {m3})."
    )
    lines.append(
        f"\\paragraph{{$(({m1},{m2},{m3}))$：Dirichlet--Mertens 常数张量（可复现输出）}}"
    )
    lines.append(
        f"按 $a=\\chi(\\gamma)\\bmod {m1}$ 分 {m1} 张 ${m2}\\times {m3}$ 切片（行 $b=N_-(\\gamma)\\bmod {m2}$，列 $c=N_2(\\gamma)\\bmod {m3}$），常数项为："
    )
    for a in range(m1):
        lines.append("$$")
...
def main() -> None:
    ...
    if not args.no_output:
        ...
        # Write LaTeX snippets for key tensors.
        if "3x3x3" in triple_class_C and "3x3x3" in triple_rho:
            _write_dirichlet_mertens_tensor_tex(
                m1=3,
                m2=3,
                m3=3,
                class_constants=triple_class_C["3x3x3"],
                rho_map=triple_rho["3x3x3"],
                lam=lam,
                out_path=generated_dir() / "tab_real_input_40_arity_dirichlet_mertens_333.tex",
            )
            print(
                "[sync-kernel-real-input-arity-3d] wrote sections/generated/tab_real_input_40_arity_dirichlet_mertens_333.tex",
                flush=True,
            )

        if "5x5x5" in triple_class_C and "5x5x5" in triple_rho:
            _write_dirichlet_mertens_tensor_tex(
                m1=5,
                m2=5,
                m3=5,
                class_constants=triple_class_C["5x5x5"],
                rho_map=triple_rho["5x5x5"],
                lam=lam,
                out_path=generated_dir() / "tab_real_input_40_arity_dirichlet_mertens_555.tex",
            )
            print(
                "[sync-kernel-real-input-arity-3d] wrote sections/generated/tab_real_input_40_arity_dirichlet_mertens_555.tex",
                flush=True,
            )
