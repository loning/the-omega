...
\begin{remark}[可检索范围内的定位说明]\label{rem:xi-terminal-zm-collision-s2-e2-mobius-search-note}
就二阶碰撞三次 \(P_2\) 与 \(2\)-division 三次 \(D_2\) 之间的显式生成元变换而言（例如上文给出的二次 Tschirnhaus 变换与一次分式变换 \(\phi(\mu)=\mu/(2(\mu-1))\)），在可检索的公开文献与资料范围内，尚未见以同一对象与同一恒等式形式直接陈述者。
本文采用的论证仅依赖于代数恒等式、判别式与消元核对，并提供可复现的脚本接口以逐项审计。
\end{remark}
