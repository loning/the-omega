...
\begin{enumerate}
  ...
  \item \textbf{（补充结论 4：统一阈值的离散化——按 \(\varphi^6\) 的六分支步进）}
  ...
  \emph{可检验形式：}给定任意候选“破缺算子/跃迁规则”，若其作用导致上述任一稳定指纹发生可审计改变，则可在有限层审计中直接否决其“闭层可实现性”。

  \item \textbf{（补充结论 6：adelic anomaly 的强口径——乘积公式式的精确抵消与离散标签的整化）}
  ...
  该强口径同时带来一个硬后果：允许的离散标签/超选择标签（例如 \(\pi_{\mathrm{sheet}}\) 及其与通道纤维的耦合方式）将被“可粘合性”条件一并整化，从而消除一部分建模自由度。
