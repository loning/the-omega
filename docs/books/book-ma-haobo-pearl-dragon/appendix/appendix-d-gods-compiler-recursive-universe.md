...
- **解释器模式 (Interpreter)**：按顺序读取脚本，遇到错误就崩溃，遇到死循环就停在情绪与惯性的重复里。
- **即时编译模式 (JIT Compiler)**：在关键点对目标进行编译优化，把原本漫长的渲染延迟压缩为可控的响应时间。
...
- **局部降熵**：把偏差压回可控范围，使系统重新可编译。
- **全息一致**：你作为宇宙的分形切片，局部自洽会在全息层面推动整体更一致。
