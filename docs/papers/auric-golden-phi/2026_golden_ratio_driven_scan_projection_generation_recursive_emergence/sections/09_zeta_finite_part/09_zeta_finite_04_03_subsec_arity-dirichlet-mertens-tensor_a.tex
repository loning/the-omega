...
$$
\begin{remark}[角色稀疏逼近定理：常数张量的最佳 \(K\)-角色截断]\label{thm:arity-335-best-k-character-truncation}
把均值项剥离，定义去均值张量
\[
\widetilde C_{a,b,c}:=C^{(3,3,5)}_{a,b,c}-\frac{\mathsf{M}}{45}.
\]
并对每个非平凡角色索引 \(j=(j_1,j_2,j_3)\neq(0,0,0)\) 定义角色基函数
\[
\chi_j(a,b,c):=\omega_3^{-j_1 a}\,\omega_3^{-j_2 b}\,\omega_5^{-j_3 c}.
\]
则三维反演式等价于严格的正交展开
\[
\widetilde C=\frac{1}{45}\sum_{j\neq 0} C(j)\,\chi_j.
\]
在 Frobenius 内积
\(
\langle X,Y\rangle:=\sum_{a,b,c}X_{a,b,c}\overline{Y_{a,b,c}}
\)
下，\(\{\chi_j\}\) 两两正交，且 \(\|\chi_j\|_F^2=45\)。
因此对任意大小为 \(K\) 的索引集合 \(J\subset\{j\neq 0\}\)，令
\[
\widetilde C^{(J)}:=\frac{1}{45}\sum_{j\in J} C(j)\,\chi_j,
\]
则误差满足精确恒等式
\[
\boxed{\ 
\bigl\|\widetilde C-\widetilde C^{(J)}\bigr\|_F^2
=\frac{1}{45}\sum_{j\notin J}|C(j)|^2
\ }.
\]
从而在所有 \(|J|=K\) 的 \(K\)-项角色截断中，使误差 \(\|\widetilde C-\widetilde C^{(J)}\|_F\) 最小的 \(J\) 必由 \(|C(j)|\) 最大的 \(K\) 个角色索引组成；最优误差即由“尾部角色能量”完全刻画。
\par
该结论只用到了本节已给出的“扭曲常数 \(C(j)\) \(\Rightarrow\) 三维反演”闭环与角色正交性：它把 44 个非平凡角色常数压缩为 \(K\) 个“机制原子”时的最优策略，从 SVD/启发式挑选提升为可审计的稀疏谱证书。
\end{remark}
