...
\paragraph{Orbit--gravity loop (weak-field interface reading).}
\InterfaceTag In the weak-field gravity dictionary, $\Phi=-\gamma c^2(\chi-\chi_0)$ and (in standard Newtonian reading) base-orbit deflection is controlled by gradients of $\Phi$.
Independently, Appendix~\ref{app:time_mass_delay} records a delay$\to$overhead$\to$lapse dictionary, so that phase-derived delays can be compared to lapse/redshift proxies under a shared $\gamma$ calibration.
This provides an explicit interface loop:
\[
\text{orbit deflection}\ \leftrightarrow\ \nabla\Phi\ \leftrightarrow\ \nabla\chi\ \leftrightarrow\ \text{lapse/delay proxies},
\]
without promoting scattering or renormalization prerequisites to theorem-level inputs.
