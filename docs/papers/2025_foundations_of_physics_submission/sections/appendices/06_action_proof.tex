...
\begin{enumerate}
    ...
    \item \textbf{Matter Coupling}: Standard Model fields $\psi_m$ couple to the metric via the standard covariant matter Lagrangian density $\mathcal{L}_m$ \cite{PeskinSchroeder1995,Carroll2004}.
\end{enumerate}
...
\end{equation}
For a perfect-fluid stress tensor $T^{\mu}{}_{\nu}=\mathrm{diag}(-\rho,p,p,p)$ and cosmological term $\Lambda(t)$, the Einstein equations yield the standard Friedmann system in $\tau$ \cite{Dodelson2003,Carroll2004}:
\begin{align}
