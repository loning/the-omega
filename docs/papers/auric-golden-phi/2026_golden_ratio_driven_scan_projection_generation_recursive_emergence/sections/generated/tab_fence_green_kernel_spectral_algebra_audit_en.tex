...
\caption{Spectral-algebraic audit of Green kernel $K_k(i,j)=\min(i,j)$: verifying Gram decomposition and determinant normalization rigidity $\det(K_k)=1$ (Lemma~\ref{lem:pom-Kk-gram-det}), $(4\sin^2)$ product/sum identities (Corollary~\ref{cor:pom-Kk-sine-product-sum}), Chebyshev characteristic polynomial identification of mixed-boundary Laplacian $L_k=K_k^{-1}$ (Theorem~\ref{thm:pom-Lk-chebyshev-charpoly}), as well as shifted determinant free-energy renormalization constants and spectral gap scale (Corollaries~\ref{cor:pom-Lk-shifted-det-free-energy}, \ref{cor:pom-Lk-gap-pi2}).}
