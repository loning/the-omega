\section{连续极限与波函数涌现 (Continuum Limit and Wavefunction Emergence)}
...
本节的核心任务是填补这一鸿沟。我们将证明 \textbf{定理 4.2}，即在时空网格极其精细的极限下（晶格常数 $\varepsilon \to 0$），DQCA 的离散演化方程严格收敛于连续的狄拉克方程。这一证明具有深远的本体论意义：它表明 \textbf{波函数 $\psi(x,t)$ 并非物理实体的原始状态，而是底层离散信息流在粗粒化视界下的统计包络}。
...
这正是 \textbf{1+1 维狄拉克方程} 的手性表示（Chiral Representation）。为了更清晰地看到这一点，我们可以引入泡利矩阵。定义旋量 $\Psi = \begin{pmatrix} \psi_R \\ \psi_L \end{pmatrix}$。上述方程组可以重写为矩阵形式：
...
\subsection{定理 4.2：DQCA 极限收敛定理}

基于上述推导，我们陈述本章的核心定理，该定理保证了欧米伽理论与现有量子场论在低能区的完全兼容性。

\textbf{定理 4.2 (DQCA 极限收敛定理)}：
设 $\mathcal{A}_\varepsilon$ 为定义在晶格常数为 $\varepsilon$ 的彭罗斯-斐波那契网格上的幺正量子元胞自动机。若其局部更新规则满足：

\begin{enumerate}
\item \textbf{幺正性 (Unitarity)}：$\hat{U}^\dagger \hat{U} = I$；
\item \textbf{局域因果性 (Locality)}：信息传输速度受限于 1 格/步；
\item \textbf{小质量条件 (Small Mass Limit)}：手性混合角 $\theta$ 与时间步长 $\varepsilon$ 成线性关系 $\theta = m \varepsilon$；
\end{enumerate}

则在连续极限 $\varepsilon \to 0$ 下，系统演化算符 $\hat{U}^N$（其中 $N = t/\varepsilon$）所生成的概率幅分布 $\Psi(x,t)$，\textbf{一致收敛 (Uniformly Converges)} 于大质量狄拉克方程 $(i \slashed{\partial} - m)\Psi = 0$ 的解。
...
\subsection{物理诠释：波的虚幻性}

这一推导揭示了量子力学本体论的一个惊人事实：\textbf{"波"并不是基础存在}。

\begin{enumerate}
\item \textbf{概率幅的本质}：复数波函数 $\psi$ 仅仅是欧米伽单元上离散比特状态的 \textbf{计数统计 (Counting Statistics)}。当我们说"电子在某处出现的概率"时，我们实际上是在统计在那一瞬间，全息网络中有多少条离散路径汇聚到了该节点。
\item \textbf{虚数 $i$ 的起源}：在狄拉克方程中神秘出现的虚数单位 $i$，直接源自 DQCA 中的硬币算符 $\hat{C}$（旋转矩阵）。它代表了欧米伽单元内部的 \textbf{90度逻辑旋转} 或 \textbf{拓扑相位积累}。量子力学的"相位"本质上是微观几何的 \textbf{"方位角"}。
\item \textbf{质量的起源}：方程右边的耦合项 $im\psi$ 表明，质量 $m$ 是左手性与右手性分量之间的 \textbf{耦合强度}。在微观上，这意味着质量不是粒子的固有属性，而是粒子在传播过程中发生 \textbf{手性翻转 (Chirality Flip)} 的频率。
\end{enumerate}

因此，量子力学不再是一个令人困惑的公理体系，它是 \textbf{"交互式离散计算系统"} 在我们这种 \textbf{"低分辨率观察者"} 眼中呈现出的平滑近似。就像我们在屏幕上看到的图像是连续的，但放大看全是像素一样，量子场论只是欧米伽理论的宏观像素画。
