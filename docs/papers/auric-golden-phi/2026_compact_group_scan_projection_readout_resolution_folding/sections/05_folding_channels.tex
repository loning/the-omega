...
\subsection{多尺度一致性：折叠通道的因子化与逆极限}

需要强调：随着 $m$ 改变，稳定类型空间的规模 $|X_m|$、平均退化度 $d_m$ 等统计量会系统变化，因此“不同 $m$ 的结果一致”不应理解为不同 $m$ 的原始输出空间与统计结构相同。正确的严格表述是\textbf{跨尺度相容性}：高分辨率输出在投影到低分辨率后复现低分辨率输出。

\begin{definition}[restriction 投影]\label{def:resfold-pi}
对 $m_2\ge m_1$，定义截断投影
\[
...
\begin{assumption}[尺度相容（读出因子化）]\label{assump:resfold-factorization}
对原始窗口读出 $\omega^{(m)}$（长度为 $m$ 的原始串），取 $m_2\ge m_1$ 时的截断 $\omega^{(m_1)}$，并要求折叠映射满足交换关系
\[
\pi_{m_2\to m_1}\bigl(\Fold_{m_2}(\omega^{(m_2)})\bigr)=\Fold_{m_1}(\omega^{(m_1)}).
\]
\end{assumption}

在该假设下，$\{X_m,\pi_{m_2\to m_1}\}$ 构成逆系统，从而可定义一个分辨率无关的统一对象：它的有限 restriction 分别对应各尺度的稳定类型空间。此时，“精度不同”严格对应为：更大 $m$ 提供更长的可见前缀与更细的条件结构，而低分辨率内容作为高分辨率的因子被保留。
