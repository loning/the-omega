...
Q_{26}(y):=B_{+}(y),
\qquad
\widetilde Q_{26}(y):=B_{-}(y).
\]
则在 \(\ZZ[y]\) 中严格成立判别式分解
\[
\boxed{\;
\mathrm{Disc}_{T}\bigl(\Phi_{2}(y,\cdot)\bigr)=-y(y-1)P_{\mathrm{LY}}(y)\cdot Q_{12}(y)^{2}\cdot Q_{26}(y)^{2}\; }.
\]
...
\end{conclusion}
\begin{proof}[证明要点]
由于 \(K/k\) 的伽罗瓦闭包之唯一二次分辨率子域由 \(\sqrt{\mathrm{Disc}_{\lambda}(\Pi)}\) 生成（结论 \ref{con:xi-terminal-zm-generic-galois-s4} 与结论 \ref{con:xi-terminal-zm-discriminant-dual-role}），并且
\(
