...
\input{subsubsec__xi-time-protocol-conclusions-part43-rankone-secular-lemmas}
...
\input{subsubsec__xi-symq-fibonacci-vandermonde-discriminant-resultant-hankel}
...
\begin{remark}[公开索引同形排查说明]\label{rem:xi-replica-softcore-hankel-vandermonde-public-index-note}
围绕 \eqref{eq:xi-replica-softcore-fibonacci-power-hankel-determinant} 与 \eqref{eq:xi-replica-softcore-alternating-geom-vandermonde} 的同型显式乘积结构进行公开关键词检索时，检索结果通常回落到一般 Hankel 行列式、正交多项式与若干 Fibonacci 相关矩阵条目之综述讨论，未见以同一索引结构 \((F_{i+j+3}^{\,q})_{0\le i,j\le q}\) 给出等价乘积公式的可核对来源。
可对照的代表性入口包括 MathOverflow 的相关汇总条目 \url{https://mathoverflow.net/questions/249838/some-hankel-determinants}；
该说明仅是公开索引层面的同形排查，不替代完备文献证明。
\end{remark}
