...
| `Problem4.lean`  | Q4: boxplus-Phi inequality (n=2, n=3 + n=4 quartic-delta algebra + denominator-clearing bridge + B=0 closure + c=0 closure) | Partial |
...
- **Q4 partial**: n=2 exact identity, n=3 strict inequality bridge, n=4 quartic `delta` closed-form algebra identity, denominator-clearing reduction `G4 = Xi4/(D1 D2 D12)`, formal Titu/Engel closure in the quartic `B=0` subfamily, and formal `c=0` closure (including Cauchy mixing bound + monotone-convex `phi` bridge).
