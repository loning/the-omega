...
\par
若 \(\mathrm{rank}\,\mathrm{Hankel}(a)=d\) 且 \(H_0\) 可逆，则 Hankel 列空间由前 \(d\) 列生成且其在前 \(d\) 个坐标上的截断给出一组基；令 \(c_j:=(a_{i+j})_{0\le i\le d-1}\in\RR^{d}\)，并定义线性算子 \(A\) 使 \(Ac_j=c_{j+1}\)（\(0\le j\le d-2\)）。
矩阵恒等式 \(AH_0=H_1\) 给出 \(A=H_0^{-1}H_1\)。
取 \(\alpha=e_0=c_0\) 与 \(\beta\) 为前 \(d\) 项向量即可得到 \(a_m=\beta^{\mathsf T}A^{m}\alpha\)。
\end{proof}
