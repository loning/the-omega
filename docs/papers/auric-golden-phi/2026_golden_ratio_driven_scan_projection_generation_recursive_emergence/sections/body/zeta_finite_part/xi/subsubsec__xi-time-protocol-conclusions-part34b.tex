...
则：
\begin{enumerate}
  \item \(\ker(\pi^\*:J(C)\to J(C_6))=G\)（作为 \(\bar\QQ\)-点子群，且对 \(\mathrm{Gal}(\bar\QQ/\QQ)\) 稳定）；
  \item \(\pi^\*J(C)\sim_{\QQ}J(C)/G\)；
...
\]
且在 \(C_6\) 上 \(L\) 被立方根平凡化，故 \(G=\langle L\rangle\subseteq\ker(\pi^\*)\)。

反向包含由循环覆盖下降理论给出：满足 \(\pi^\*M\simeq\mathcal O_{C_6}\) 的 \(M\in\operatorname{Pic}^0(C)\)
与 \(C_3\)-特征标同构，
\[
\operatorname{Hom}(C_3,\mathbf G_m)\cong\ZZ/3\ZZ,
\]
其生成元正是 \(L\)。故 \(\ker(\pi^\*)=\langle L\rangle=G\)，即得 (1) 与 (2)。
...
\begin{enumerate}
  ...
  并且 \(\Psi\) 诱导
  \[
  J(C_6)/\pi^\*J(C)\ \sim_{\QQ}\ W\ \sim_{\QQ}\ J(\overline{\mathcal R})^2.
  \]
