\paragraph{二维原子层、激活零尺度根与零温四态核心的三尺度分裂}
在真实输入 \(40\) 状态核的二维加权 \(\zeta\) 分母闭式、一参数输出位势的激活分支，以及零温四态地基层已经建立之后，还可把原子层、局部激活层与零温核心层进一步压缩为下列若干条彼此衔接的结构后果。前两条仍对应对原子因子的中心投影式相位盲读出；随后的几条则转入相位敏感的纤维内读出，分别刻画 \(u=1\) 处的极点阶、Abel 有限部平移、激活分支的横截性以及正实轴上的局部谱壁垒；最后再把局部奇性独占与零温 Perron 幅度统一到同一条谱分裂链中。
...
\noindent
以上这些结果表明，真实输入 \(40\) 状态核在二维原子层、\(u=1\) 的局部激活层与 \(u\to+\infty\) 的零温核心层之间存在严格的多尺度分裂：一方面，二维加权 \(\zeta\) 的单原子因子在 ghost 层只向偶长度注入 \(2v^{n/2}\)，并对碰撞权 \(u\) 完全失明；当进一步压到输出完全抑制的切片 \(v=0\) 时，整个二维谱又严格塌缩为唯一一个长度 \(1\) 的 primitive 轨道；另一方面，在碰撞零温缩放下，整个二维核心因子又进一步塌缩为同一原子的单参数模型 \(1-vw\)；在一参数输出位势上，同一原子 Witt 机制则只在 primitive 的 \(n=2\) 层激活，并把 Abel 有限部刚性平移为 \(u z_\star(u)^2\)；再者，激活分支以无二次项的横截方式穿过 \(\lambda=0\)，而正实轴上的区间 \(1<u<4\) 又把原子碰撞与整数级共振共同隔离在外；最后，真正主导大 \(u\) 幅度常数的并不是显式原子谱支 \(\pm\sqrt u\)，而是零温四态核心 \(B_\infty\) 的 Perron 常数。因而在读出口径上，前述几条对应于中心投影式的相位盲读出，而其后的局部极点、激活分支与谱壁垒则共同构成相位敏感的纤维内读出。
