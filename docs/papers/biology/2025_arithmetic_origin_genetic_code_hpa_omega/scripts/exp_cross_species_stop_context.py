...
def analyze_species(
    ...
    *,
    ...
    metrics = ["u_before"]
    if source == "rna":
        metrics.extend(["u_after", "u_diff"])

    def metric_value(ctx: StopContext, metric: str) -> float:
        if metric == "u_before":
            return float(ctx.u_before)
        if metric == "u_after":
            return float(ctx.u_after)
        if metric == "u_diff":
            if math.isfinite(ctx.u_before) and math.isfinite(ctx.u_after):
                return float(ctx.u_after - ctx.u_before)
            return float("nan")
    ...
    for s1, s2 in stop_pairs:
        ...
        for metric in metrics:
            vals1 = [metric_value(c, metric) for c in ctxs1]
            vals2 = [metric_value(c, metric) for c in ctxs2]
            vals1 = [v for v in vals1 if math.isfinite(v)]
            vals2 = [v for v in vals2 if math.isfinite(v)]
...
def generate_latex_summary(
    ...
    *,
    ...
    if metric not in {"u_before", "u_after", "u_diff"}:
        raise ValueError(f"Unknown metric for LaTeX summary: {metric}")
    ...
    metric_field = {
        "u_before": "u_before_mean",
        "u_after": "u_after_mean",
        "u_diff": "u_diff_mean",
    }[metric]

    metric_col = {
        "u_before": r"$\bar{U}_{\mathrm{before}}$",
        "u_after": r"$\bar{U}_{\mathrm{after}}$",
        "u_diff": r"$\overline{\Delta U}$",
    }[metric]

    metric_explain = ""
    if metric == "u_diff":
        metric_explain = r" (where $\Delta U = U_{\mathrm{after}}-U_{\mathrm{before}}$)"

    label = r"tab:cross_species_stop_context"
    if source == "rna":
        label = "tab:cross_species_stop_context_mrna"
        if tag:
            label += f"_{tag}"
        label += f"_{metric}"

    col_uaa = metric_col.replace("$", "") + r"^{\mathrm{UAA}}"
    col_uga = metric_col.replace("$", "") + r"^{\mathrm{UGA}}"
    ...
    lines = [
        ...
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Cross-species stop-context replication (" + src_note + r"; $k=" + str(k) + r"$ codons; " + metric_col + metric_explain + r")}",
        r"\label{" + label + r"}",
        r"\begin{tabular}{llrrrrr}",
        r"\toprule",
        r"Species & Domain & $n$ & $" + col_uaa + r"$ & $" + col_uga + r"$ & $\Delta$ & $d$ \\",
        r"\midrule",
    ]
    ...
    for result in sorted(results, key=lambda r: (r.get("domain", ""), r.get("species", ""))):
        ...
        uaa_mean = float(uaa_stats.get(metric_field, float("nan")))
        uga_mean = float(uga_stats.get(metric_field, float("nan")))

        comp = result.get("pairwise_comparisons", {}).get(f"UAA_vs_UGA__{metric}", {})
    ...
    meta = meta_results.get(f"UAA_vs_UGA__{metric}", {})
...
def main() -> None:
    ...
    parser.add_argument(
        "--latex-metric",
        choices=["u_before", "u_after", "u_diff"],
        help="Which metric to render in the LaTeX summary (default: u_before for --source cds, u_diff for --source rna).",
    )
    ...
    for k in k_values:
        ...
        # Meta-analysis
        metrics = ["u_before"] if str(args.source) == "cds" else ["u_before", "u_after", "u_diff"]
        meta_results: dict[str, dict[str, Any]] = {}
        for metric in metrics:
            for comp_key in ["UAA_vs_UGA", "UAA_vs_UAG", "UAG_vs_UGA"]:
                meta_results[f"{comp_key}__{metric}"] = random_effects_meta(results, f"{comp_key}__{metric}")
        ...
        # Write JSON
        stem = "cross_species_stop_context"
        if str(args.source) == "rna":
            stem += "_mrna"
        if args.domain:
            stem += f"_{args.domain}"
        stem += f"_k{k}"

        out_json = data_root() / "_cache" / f"{stem}.json"
        out_json.parent.mkdir(parents=True, exist_ok=True)
        write_json_atomic(out_json, summary)
        print(f"  Wrote: {out_json.relative_to(root_dir())}")
        
        # Write LaTeX
        default_latex_metric = "u_before" if str(args.source) == "cds" else "u_diff"
        latex_metric = str(args.latex_metric) if args.latex_metric else default_latex_metric
        latex = generate_latex_summary(
            results,
            meta_results,
            k,
            source=str(args.source),
            metric=latex_metric,
            tag=str(args.domain) if args.domain else None,
        )
        ...
        # Print summary
        meta_uaa_uga = meta_results.get(f"UAA_vs_UGA__{latex_metric}", {})
        if not meta_uaa_uga.get("insufficient_data"):
            ...
            print(f"\n  Meta-analysis (UAA vs UGA; {latex_metric}):")
