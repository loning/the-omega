...
\begin{conclusion}[换位商与三循环商 Jacobian 的完全同源分解]\label{con:xi-time-part9n-c2-c3-quotient-jacobian-complete-splitting}
...
结合结论 \ref{con:xi-time-part9n-s4-all-subgroup-genera} 与以上诸条结论，\(S_4\)-Hurwitz 商塔中这条以 Chevalley--Weil 重数为起点的链条已在几何与算术两侧同时闭合：一方面，\(X\)、\(C_6\)、\(Y\) 及各个中间商曲线的圆维预算与 Jacobian/Prym 分块完全一致，未曾单独写出的 \(X/C_2\) 与 \(X/C_3\) 的 Jacobian 同源型亦被唯一补全，而 \(\operatorname{End}^0_{\QQ}(J(X))\) 的显式半单非交换子代数下界也从同一分块中同步抽出；另一方面，良素处局部 \(L\)-因子与两类 Prym 的最小同源代价被极化缺陷严格锁定。
=======
结合结论 \ref{con:xi-time-part9n-s4-all-subgroup-genera} 与以上诸条结论，\(S_4\)-Hurwitz 商塔中这条以 Chevalley--Weil 重数为起点的链条已在几何与算术两侧同时闭合：一方面，\(X\)、\(C_6\)、\(Y\) 及各个中间商曲线的圆维预算与 Jacobian/Prym 分块完全一致，未曾单独写出的 \(X/C_2\) 与 \(X/C_3\) 的 Jacobian 同源型亦被唯一补全；另一方面，\(C_6\) 作为 \(H\) 与 \(Z\) 的规范化纤维积被唯一识别，全纯微分空间按四个非平凡不可约表示张量分层，良素处局部 Euler 因子的平方/立方块与两类 Prym 的最小同源代价亦均被群论分块严格锁定。
