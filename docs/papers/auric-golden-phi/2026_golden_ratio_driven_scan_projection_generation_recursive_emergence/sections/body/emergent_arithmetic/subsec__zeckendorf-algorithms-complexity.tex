...
\input{para__zeckendorf-parallel-propagation-lower-bound}

\subsubsection{单趟在线延迟 \texorpdfstring{$3$}{3}：统一转导器}\label{subsec:online-delay}
存在\textbf{单趟在线}转导器（延迟 3）直接实现归一化；其同步部分可用“余量守恒”给出（\cite{Frougny1999OnlineAddition}）。
...
\input{sections/generated/eq_sync_kernel_10_state_mealy_minimality}
...
\input{sections/generated/eq_sync_kernel_10_state_uniform_input_fingerprint}
