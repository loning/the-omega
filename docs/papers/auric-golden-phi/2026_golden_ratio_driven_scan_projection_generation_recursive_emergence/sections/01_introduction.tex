...
\begin{itemize}
  ...
  \item \textbf{算术与归一化：}把稳定地址上的运算构造为“逐位叠加 $\Rightarrow$ 折叠归一化”的可计算实现，并在稳定语法层给出闭环读出接口（第 7 节与附录 E）。
  ...
  \item \textbf{核对比的统一接口：}在多类归一化核上对齐“零式 $+$ $q$-场”模板，并将 primitive/\(\zeta\) 读出写入可复现的核指纹接口（第 \ref{sec:A-kernel-compare-main} 小节与附录 \ref{app:sync-kernel-A-compare}）。
\end{itemize}
