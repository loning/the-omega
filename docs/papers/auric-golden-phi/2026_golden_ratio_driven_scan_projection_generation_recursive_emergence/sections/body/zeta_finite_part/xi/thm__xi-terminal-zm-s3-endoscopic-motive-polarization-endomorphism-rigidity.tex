...
\begin{remark}[检索核验说明（最小外部一致性检查）]\label{rem:xi-terminal-zm-s3-endoscopic-novelty-index-note}
围绕“\(J(C_6)\sim J(C)\times J(R)^2\)”“\(\#C_6=\#C+2\#R-2(q+1)\)”与
\(A_2\)-型极化矩阵
\(
\begin{psmallmatrix}2&-1\\-1&2\end{psmallmatrix}
\)
的联合表述，在公开索引层面未发现与本判别式脊塔口径完全同构的现成陈述。
严格原创性判定仍依赖于针对 Prym--Kani--Rosen 及相关文献链的系统穷举比对。
\end{remark}
