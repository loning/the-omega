\section{\texorpdfstring{$\mu$ 与 $\alpha$ 的数值计算}{mu 与 alpha 的数值计算} (Numerical Calculation of $\mu$ and $\alpha$)}
...
\begin{itemize}
\item 对于 $S^1$ (圆周): $\text{Vol}(S^1) = 2\pi$
\item 对于 $S^2$ (球面): $\text{Vol}(S^2) = 4\pi$
\item 对于 $S^3$ (超球面): $\text{Vol}(S^3) = 2\pi^2$
\item 对于 $S^5$: $\text{Vol}(S^5) = \pi^3$
\end{itemize}
...
\begin{itemize}
\item 在 $z=1.0$ 处，$\Delta \alpha / \alpha \approx -3.5 \text{ ppm}$
\item 在 $z=3.0$ 处，$\Delta \alpha / \alpha \approx -6.9 \text{ ppm}$
\end{itemize}
