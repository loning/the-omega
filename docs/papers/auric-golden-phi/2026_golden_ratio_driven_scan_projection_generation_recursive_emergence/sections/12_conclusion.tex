...
本文的主线是把有限窗口读出所诱导的微态空间，通过可审计的构造链条组织为稳定类型、可实现的算术结构与可证书化的统计/谱指纹接口。相应的主要结果可按“输入口径—构造机制—桥接与验证”三段收束如下。

\subsection{主要结果（按机制分组）}\label{subsec:core-conclusions}
\paragraph{（输入口径的固定）}
...
解释性综合、边界与可证伪点集中于第 \ref{sec:discussion} 节。桥接接口的接口化断言与相应的可证伪判据汇编于附录 \ref{app:bridgepack}，以避免与正文主证明链混排。
