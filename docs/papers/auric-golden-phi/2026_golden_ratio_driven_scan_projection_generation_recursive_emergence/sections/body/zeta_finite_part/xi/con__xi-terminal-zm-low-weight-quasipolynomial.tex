...
\]
则对每个固定整数 \(k\ge 0\)，序列 \(m\mapsto a_{m,k}\) 为周期 \(2\) 的准多项式：存在整数 \(m_0=m_0(k)\) 与多项式 \(P_k,Q_k\in\QQ[m]\)，使对所有 \(m\ge m_0\) 成立
\[
\boxed{\;a_{m,k}=P_k(m)+(-1)^m Q_k(m)\;}
\]
并且次数满足
\[
\deg P_k=2k+1,\qquad \deg Q_k\le k.
\]
此外，\(P_k\) 的首项系数具有封闭形式
\[
P_k(m)=\frac{1}{2^{k+1}(2k+1)!}\,m^{2k+1}+O\!\left(m^{2k}\right)\qquad(m\to\infty).
\]
\end{conclusion}
\begin{proof}[证明要点]
...
对 \(A_k\) 作部分分式并逐项取系数即可得到（在除去有限个初值项后）
\(
