...
\begin{proposition}[单隐藏位诱导的混合矩簇分解与相关层数]\label{prop:pom-mixed-moment-clusters}
固定整数 \(k\ge 2\)。
在推论 \ref{cor:pom-branch-mass-law} 的分裂计数 \(d_{m,0}(x),d_{m,1}(x)\) 之上，定义\textbf{混合矩簇}
\[
M^{(k)}_{a}(m)
:=\sum_{x\in X_m} d_{m,0}(x)^{a}\,d_{m,1}(x)^{k-a},
\qquad a=0,1,\dots,k.
\]
则 \(k\)-阶纤维矩
\(
...
S_k(m)
=
\sum_{a=0}^{k}\binom{k}{a}\,M^{(k)}_{a}(m).\;}
\]
并且由于 \(\binom{k}{a}=\binom{k}{k-a}\)，上式可重写为只依赖
\(
a=0,1,\dots,\lfloor k/2\rfloor
\)
的一半层数据：令
\[
C^{(k)}_{a}(m):=
\begin{cases}
M^{(k)}_{a}(m)+M^{(k)}_{k-a}(m),& 0\le a<k/2,\\[2pt]
M^{(k)}_{k/2}(m),& k\ \text{为偶数且 }a=k/2,
\end{cases}
\]
则 \(S_k(m)\) 由 \(\{C^{(k)}_{a}(m)\}_{a=0}^{\lfloor k/2\rfloor}\) 完全确定。
其中 \(a=0\)（等价地 \(a=k\)）对应纯分支项 \(\sum_x d_{m,0}(x)^k+\sum_x d_{m,1}(x)^k\)，而
\[
a=1,\dots,\lfloor k/2\rfloor
\]
给出\textbf{恰好 \(\lfloor k/2\rfloor\) 层}“跨分支相关量”（每一层都同时包含 \(d_{m,0},d_{m,1}\)），它们由单隐藏位（两支分裂）这一硬结构不可避免地产生。
\end{proposition}

\begin{proof}
由恒等式 \(d_m(x)=d_{m,0}(x)+d_{m,1}(x)\) 逐点展开二项式
\(
(d_{m,0}+d_{m,1})^k=\sum_{a=0}^k\binom{k}{a}d_{m,0}^a d_{m,1}^{k-a}
\)，
对 \(x\in X_m\) 求和即得第一式。
将 \(a\) 与 \(k-a\) 配对并用 \(\binom{k}{a}=\binom{k}{k-a}\) 得第二部分结论。证毕。
\end{proof}
