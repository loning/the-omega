...
- 2026-01-18 — **COMPLETED**: `ISA-H3C1` Centerwired control-flow features vs Ribo-seq pausing（在 terminal-stop windows 上复用 “centerwired gates” 状态机，提取 gate-hit/refined-fraction/$m=10$ hits 等特征，并检验其对 pause-index 的增量解释力；优先使用 dinuc null-of-null (`ISA-P3`) 的 cache）。Branch: `paper-bio`.
  - Result (2026-01-18): 新增 `scripts/exp_riboseq_pause_bam_window_centerwired_features.py`，生成 `sections/generated/centerwired_gate_features_vs_pausing.tex`（+meta），并已写入 discussion：`sections/06_discussion.tex`；同时纳入 `scripts/run_all.py`（在 BAM pausing block 内，依赖 `ISA-P3` cache）。
  - Key result (2026-01-18): 以“after-window 中是否出现任意 refined-mode codon（$m\\in\\{8,10\\}$）”为主特征（$\mu^\\ast$；10 before + stop + 10 after；$\Delta_{m=10}$ gate=55），在满足 $n\\ge10$ / 组的 tracks 中（n=3）得到弱负向分离：raw pause-index $d=-0.29$ [$-0.61$, $0.04$], $I^2=0\\%$；对 pause-index 先回归掉 dinuc-null $z\\Delta U$ 后（residualized）仍为 $d=-0.25$ [$-0.58$, $0.08$], $I^2=0\\%$。结论：当前数据下该“centerwired refined”特征不支持正向 pausing 解释力，方向上更接近 null/弱负向（并且受 coverage 限制，track 数仍偏少）。
  - Key result (2026-01-18): 更稀有的 $m=10$ hit 特征（after-window 存在 $\Delta=55$ 且 refined）在当前阈值下 meta-analysis 不足（insufficient tracks）。
  - Repro (2026-01-18): `python scripts/exp_riboseq_pause_bam_window_centerwired_features.py --force`（输入：`data/_cache/riboseq_pause_bam_window_dinuc_null.json` + `data/refseq_hsapiens_mrna/stop_context_candidates.jsonl`）。
...
- 2026-01-17 — **COMPLETED**: `H3-3e` raw-read Ribo-seq pausing robustness + expansion（在 `H3-3d` 标准化 BAM/FASTQ pipeline 基础上补充更多独立 studies，并做关键参数稳健性：read-length 过滤 / P-site offset；更新 `sections/generated/riboseq_pause_bam_window.tex` 的随机效应 meta，并同步写回 paper 与本分析文档；CPU 优先，如需 GPU 仅用 A40）。Branch: `paper-bio`.
  ...
  - Robustness (2026-01-17): 新增敏感性 sweep：`scripts/exp_riboseq_pause_bam_window_sensitivity.py` + `sections/generated/riboseq_pause_bam_window_sensitivity.tex`。在 read-length 更严过滤（28–32nt）与简化 P-site offset（+12nt）下 meta 仍为正：$d=0.66$ [0.26, 1.06]（3 tracks）与 $d=0.41$ [0.09, 0.72]（5 tracks）。
...
- 2026-01-16 — **COMPLETED**: `H3-3d` 扩展 raw-read Ribo-seq pausing 复现到 ≥4 independent studies（优先 GSE211536 + 其他 1–2 个 human Ribo-seq series；CPU 优先；如需 GPU 仅用 A40），并在 `sections/generated/riboseq_pause_bam_window.tex` 里追加 track + 更新随机效应 meta，写回 paper 与本分析文档。Branch: `paper-bio`.
  - Progress (2026-01-16): 尝试从 GEO GSE211536 定位 SRA runs：GEO 页可解析到 BioProject PRJNA870767，但 `fetch_sra_runinfo.py` 对 GSE/PRJNA 的 SRA esearch 均返回 0（可能仅有 bigWig、无 SRA raw reads）；因此改为直接在 SRA 以 `RIBO-SEQ[Strategy] AND Homo sapiens[Organism]` 检索并按读长筛选候选 runs。
  - Result (2026-01-16): 新增 3 个 raw-read tracks 并更新片段 `sections/generated/riboseq_pause_bam_window.tex`：`SRP627184_SRR35611920_full`（full 9.08M spots）显示显著分位数分层效应（high $\Delta U$ vs low：$d=0.81$, $p=0.008$, $n_{high}/n_{low}=23/27$；且 $\\rho(PI,\\Delta U)=0.231$，$n=90$, $p=0.029$）；`SRP498469_SRR28491035_10m`（10M spots）给出一致方向的效应量（$d=0.78$, $p=0.055$, $n_{high}/n_{low}=14/13$；且 $\\rho(PI,\\Delta U)=0.427$，$n=42$）；`ERP150947_ERR12549926_10m` 覆盖不足（$n_{corr}=2$，无法报告相关/效应量，不纳入 meta）。剔除覆盖不足 track 后，4-study 随机效应 meta：$d=0.48$ [0.08, 0.87], $I^2=24.9\\%$（CI 排除 0），给出对 H3 的更强正向证据；下一步建议补充更多 studies 并加入长度/P-site 稳健性分析以排除潜在偏差来源。
...
**状态**：✅ 有正向证据（raw-read pausing meta）
...
BigWig (3 studies):     pause-index vs ΔU: ρ=0.12–0.41; meta d(high ΔU vs low)=0.30 [-0.06, 0.66], I²=0% (not decisive)
Raw BAM (8 tracks):     meta d(high ΔU vs low)=0.52 [0.20, 0.83], I²=4.0% (n=5 tracks used)
Sensitivity:            28–32nt midpoint: d=0.66 [0.26, 1.06] (n=3); P-site +12nt: d=0.41 [0.09, 0.72] (n=5)
```
