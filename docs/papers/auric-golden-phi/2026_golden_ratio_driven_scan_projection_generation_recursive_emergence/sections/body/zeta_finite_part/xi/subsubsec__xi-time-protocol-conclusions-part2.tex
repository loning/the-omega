...
本节续接 \S\ref{subsubsec:xi-time-protocol-conclusions}，在不引入新本体假设的前提下，给出结论 32--39 的可审计形式化版本；其条目均可独立引用。

\begin{theorem}[尺度--时间双复形的统一障碍谱序列]\label{thm:xi-scale-time-bicomplex-obstruction}
...
\begin{theorem}[时间--熵帕累托前沿的 Gibbs 刚性与全局温度一致性]\label{thm:xi-time-entropy-pareto-gibbs-global-temperature}
...
\frac{e^{-\lambda\tau(\omega)}}
{\sum_{\omega'\in \mathrm{Fold}^{-1}(x)}e^{-\lambda\tau(\omega')}}.
\]
...
\begin{theorem}[黄金底对数时间标尺刚性]\label{thm:xi-logphi-time-rigidity}
设规模函数 \(S(m)\) 满足 Fibonacci 递推
\[
...
\begin{theorem}[时间--分辨率的 Galois 伴随与稳定类型不动点]\label{thm:xi-time-resolution-galois-adjunction}
...
\begin{theorem}[混合曲率零判据：加法--乘法统一时间的充要条件]\label{thm:xi-mixed-curvature-zero-criterion}
...
\begin{theorem}[临界切片的压力判据：零漂移与零 Lyapunov 的共同成立]\label{thm:xi-critical-slice-pressure-zero-drift-lyapunov}
...
\begin{theorem}[时间 \(\zeta\)-数据的可识别性：决定 Markov 同构类与余圈同调类]\label{thm:xi-time-zeta-identifiability-markov-cohomology}
...
\begin{theorem}[有限时间可分辨阈值的互信息指数律]\label{thm:xi-finite-time-distinguishability-mutual-information-rate}
设微态过程 \((\omega_t)\) 平稳遍历，观测 \(x_t=\mathrm{Fold}(\omega_t)\)，\(\mathcal F_t:=\sigma(x_0,\dots,x_t)\)。
记时间 \(t\) 下平均不可区分类对数为 \(\Delta(t)\)。
在正熵与有限记忆可实现条件下，极限
\[
