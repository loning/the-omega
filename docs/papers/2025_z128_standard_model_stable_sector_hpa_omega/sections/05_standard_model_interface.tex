...
We first record the protocol-level necessity of compensation under cross-site consistency, and then state the channel-to-gauge identification as an explicit interface hypothesis.
...
\begin{assumption}[Three-channel gauge-factor identification (interface)]\label{as:channel_to_gauge}
In a continuum limit, the three compensation classes induced by the $\varphi$--$\pi$--$\e$ defect channels are identified with the three gauge-factor connections of the Standard Model.
\end{assumption}
...
The three-factor structure is not assumed as a premise, but posed as a mapping target via Assumption~\ref{as:channel_to_gauge}.
...
\end{proposition}
\begin{proof}
This is standard; see, e.g., \cite{PeskinSchroeder1995,WeinbergQFT1995}.
\end{proof}
