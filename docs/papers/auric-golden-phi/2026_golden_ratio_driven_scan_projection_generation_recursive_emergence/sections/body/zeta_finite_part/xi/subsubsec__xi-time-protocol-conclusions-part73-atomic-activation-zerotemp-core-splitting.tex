...
\noindent
以上这些结果表明，真实输入 \(40\) 状态核在二维原子层、\(u=1\) 的局部激活层与 \(u\to+\infty\) 的零温核心层之间存在严格的多尺度分裂：一方面，二维加权 \(\zeta\) 的单原子因子在 ghost 层只向偶长度注入 \(2v^{n/2}\)，并对碰撞权 \(u\) 完全失明；另一方面，在碰撞零温缩放下，整个二维核心因子又进一步塌缩为同一原子的单参数模型 \(1-vw\)；在未加权点 \(u=1\)，同一二周期原子同时精确控制 full/core \(\zeta\) 的极点阶差与 Abel 有限部常数的代数平移；再者，激活分支以无二次项的横截方式穿过 \(\lambda=0\)，而正实轴上的区间 \(1<u<4\) 又把原子碰撞与整数级共振共同隔离在外；最后，真正主导大 \(u\) 幅度常数的并不是显式原子谱支 \(\pm\sqrt u\)，而是零温四态核心 \(B_\infty\) 的 Perron 常数。因而在读出口径上，前两条对应于中心投影式的相位盲读出，而其后的局部极点、激活分支与谱壁垒则共同构成相位敏感的纤维内读出。
