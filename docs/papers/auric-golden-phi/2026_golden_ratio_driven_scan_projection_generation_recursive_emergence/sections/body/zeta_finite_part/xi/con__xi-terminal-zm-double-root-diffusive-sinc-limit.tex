...
\end{cases}
\qquad
S(u):=\operatorname{sinc}\!\left(\sqrt{\frac{u}{2}}\right).
\]
...
其中
\(
\frac12 S(u)=\sum_{k\ge 0}\frac{(-u)^k}{2^{k+1}(2k+1)!}
=\frac{\sin(\sqrt{u/2})}{\sqrt{2u}}
\)
以连续延拓在 \(u=0\) 处取值 \(1/2\)。
...
\[
\boxed{\;
m^{2}\bigl(-\rho_{m,j}\bigr)\longrightarrow 2\pi^{2}j^{2},
\qquad
\rho_{m,j}=-\frac{2\pi^{2}j^{2}}{m^{2}}+O\!\left(\frac1{m^{3}}\right)\;}
\qquad(m\to\infty),
\]
等价地，定义 \(\sigma_{m,j}:=m\sqrt{-\rho_{m,j}/2}\)，则 \(\sigma_{m,j}\to \pi j\)，体现出边缘零点在 \(\sqrt{-y}\) 坐标下的等间距“时钟过程”主阶。
\end{conclusion}
\begin{proof}[证明要点]
...
零点定标由 Hurwitz 定理对上述一致估计与 \(S(u)\) 的简单零点 \(u=2\pi^{2}j^{2}\) 应用而得；
...
\[
\boxed{\;
\mathcal N_m(U)=\left\lfloor \sqrt{\frac{U}{2\pi^2}}\right\rfloor+o(1)\;}
\]
从而在长度尺度 \(m^{-2}\) 的窗口内，零点计数呈 \(\sqrt{U}\) 增长的 Weyl 型指数。
此外，极限核 \(S(u)=\operatorname{sinc}(\sqrt{u/2})\) 具有 Weierstrass 乘积分解
\[
\boxed{\;
S(u)=\prod_{j=1}^{\infty}\left(1-\frac{u}{2\pi^2 j^2}\right)\;}
\]
其零点集合 \(\{2\pi^2 j^2\}_{j\ge 1}\) 与单位区间 Dirichlet 拉普拉斯算子 \(-\frac{\dd^2}{\dd x^2}\) 的本征值 \(\{\pi^2 j^2\}_{j\ge 1}\) 仅差一个固定尺度因子 \(2\)。
因此，扩散标度下的归一化极限可视为一个显式谱行列式对象，从而把离散折叠—投影体系在 \(y=0\) 边缘的零点刚性与连续二阶算子谱结构对齐。
\end{conclusion}
\begin{proof}[证明要点]
由结论 \ref{con:xi-terminal-zm-double-root-diffusive-sinc-limit} 的零点定标，固定 \(U\) 时 \(\mathcal N_m(U)\) 收敛到 \(S(u)\) 在区间 \([0,U]\) 内零点数目，而 \(S(u)=0\) 当且仅当 \(u=2\pi^2 j^2\)，故得到所示计数律。
...
\begin{remark}[可检索范围内的定位说明]\label{rem:xi-terminal-zm-diffusive-sinc-search-note}
关于该四阶递推族 \(Z_m\) 在 \(y=0\) 边缘的扩散标度极限核 \(u\mapsto \operatorname{sinc}(\sqrt{u/2})\)（等价地 \(m^{-1}Z_m(-u/m^2)\to \sin(\sqrt{u/2})/\sqrt{2u}\)）以及由此导出的近零根量子化常数 \(2\pi^2\)，在可检索的公开文献与资料范围内，尚未见针对同一多项式族与同一归一化口径的直接表述。
本文的论证基于生成函数在二重极点附近的局部留数展开与 Hurwitz 定理，因而在给定递推与初值之内可自洽复核。
\end{remark}
