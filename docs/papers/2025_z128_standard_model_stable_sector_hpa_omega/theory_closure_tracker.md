...
| Higgs/标量扇区（uplift 依赖） | $21$ 稳定类型不包含 Higgs；标量作为 uplift/coarse-graining 依赖的接口闭合/审计。补充：在 $m=10$ uplift 上给出一个确定性的“几何双重态”代表构造（2D/3D Hilbert screens），并输出可机检 JSON 证书与图（正文 Fig.~`fig:higgs_doublet_structure_m10_2d` / `fig:higgs_doublet_structure_m10_3d`；附录 `subsec:higgs_doublet_uplift_geometry`） | Iface/CAP/Audit | `rem:higgs_not_in_21`, `prop:scalar_sector_closed`, `subsec:higgs_doublet_uplift_geometry`, `fig:higgs_doublet_structure_m10_2d`, `fig:higgs_doublet_structure_m10_3d`, `scripts/fig_higgs_geometry_candidates_m10.py`, `scripts/fig_higgs_doublet_structure_m10.py`, `scripts/fig_higgs_doublet_structure_m10_3d.py` | [x] |
...
- [x] **氢原子核（质子）的最小结构构造（色 singlet 复合载体；Iface+Match+Audit）**：在不触及禁闭/质量隙强闭合的前提下，把质子作为由已标号的夸克自由度张量化得到、并投影到 $SU(3)$ 色 singlet 的最小复合载体；同时显式登记边界：不从 $m=6$ 锚点深度模板推导重子质量谱；不进入质子衰变闭合（需额外 protocol-to-observation 桥）。  
  - 位置：`sections/appendices/68a_proton_hydrogen_nucleus_interface.tex`（`\label{app:proton_hydrogen_nucleus_interface}`）
- [x] **氢原子（质子+电子）的最小复合系统结构（Iface+Match+Audit）**：把氢原子作为复合系统 $\mathcal{H}_p\otimes\mathcal{H}_e$ 的接口封装（复用 tensor-product/POVM 的最小结构包），并以 vendored 匹配集记录 Bohr 半径与基态电离能等参考目标；同时把“基态电离能”作为\emph{复合束缚能尺度}映射到 $r(\mu)$ 阶梯的 $m_{\mathrm{eff}}$ 仅作尺度标签，显式避免与“单窗局域物质 $m\ge 6$”的可接受性门槛混淆。  
  - 位置：`sections/appendices/68b_hydrogen_atom_interface.tex`（`\label{app:hydrogen_atom_interface}`）
