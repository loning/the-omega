\section{算术的涌现：在稳定地址空间构造 $(\oplus,\otimes)$}

本节给出“从无到有”的算术生成：自然数对象、加法与乘法并非作为原语预置，而由稳定语法与规范形归一化在 POMmin 下定义，并证明与经典半环同构。为避免不必要的拓扑复杂性，本节将稳定地址空间取为 Zeckendorf 合法数字的有限支撑部分，即第 \ref{def:Z-fiber} 节的 $Z$；记号上亦可将其视为 $X_\infty$ 的有限支撑子对象。
...
$$
等价地，亦可通过值映射定义 $c\oplus d := \mathrm{Val}^{-1}(\mathrm{Val}(c)+\mathrm{Val}(d))$。
\end{definition}
...
定义
$$
c\,\underline{\oplus}\, d := \underbrace{c\oplus c\oplus\cdots\oplus c}_{\mathrm{Val}(d)\text{ 次}},
\qquad
\mathrm{Val}(d)=0 \text{ 时取 } 0_Z,
$$
其中 $0_Z$ 为全零序列。则对任意 $c,d\in Z$，有
$$
c\,\underline{\oplus}\, d = c\otimes d.
$$
\end{theorem}
...
$$
\mathrm{Val}(c\,\underline{\oplus}\, d)=\mathrm{Val}(c)\mathrm{Val}(d)=\mathrm{Val}(c\otimes d),
$$
再由双射性推出相等。
\end{proof}

同理，可通过后继算子 $S(c)=\Fold_\infty(c+e_1)$ 将 $\oplus$ 内生为后继迭代，从而得到更极简的原语集（以“后继 + 折叠”生成算术）。
