...
def write_induced_tex(summary: Dict[str, float], out_path: Path) -> None:
    lines: List[str] = []
    lines.append("% Auto-generated by scripts/exp_emergence_space_holonomy_induced.py")
    lines.append("\\begin{tabular}{lr}")
    lines.append("\\toprule")
    lines.append("Quantity & Value\\\\")
    lines.append("\\midrule")
    lines.append(f"mean Hol!=0 (iface-A) & {summary['holA_mean']:.4f}\\\\")
    lines.append(f"mean Hol!=0 (bitdiff-A) & {summary['holB_mean']:.4f}\\\\")
    lines.append(f"mean Hol!=0 (dirbit-A) & {summary['holC_mean']:.4f}\\\\")
    lines.append(f"mean touch interface & {summary['touch_mean']:.4f}\\\\")
    lines.append(f"corr(HolA, touch) & {summary['corrA_touch']:.4f}\\\\")
    lines.append(f"corr(HolB, touch) & {summary['corrB_touch']:.4f}\\\\")
    lines.append(f"corr(HolC, touch) & {summary['corrC_touch']:.4f}\\\\")
