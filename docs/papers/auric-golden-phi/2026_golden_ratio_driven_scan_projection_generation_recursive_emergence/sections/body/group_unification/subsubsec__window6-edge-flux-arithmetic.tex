...
\begin{theorem}[\(\ZZ[1/6]\)-局部化下的 Kirchhoff--Green 单阶阻塞与 \texorpdfstring{$p_\star=571$}{p*=571}-进刚性]\label{thm:window6-kirchhoff-green-single-order-padic-rigidity}
...
\begin{corollary}[Green 核在 \(\ZZ[1/6]\) 上的最小公共分母之 \texorpdfstring{$p_\star$}{p*}-进阶为 \(1\)]\label{cor:window6-green-kernel-pstar-valuation-one}
...
\begin{conjecture}[\(\ZZ[1/6]\)-局部化下 Kirchhoff 新素数的单阶性]\label{conj:window-kirchhoff-prime-single-order-localization}
