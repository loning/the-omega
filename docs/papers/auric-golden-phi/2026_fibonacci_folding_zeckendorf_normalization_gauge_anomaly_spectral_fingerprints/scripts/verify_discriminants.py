...
def run() -> str:
    ...
    assert_simplify_zero(disc_fact - expected, "Disc_lambda Pi(lambda,y) factorization mismatch.")
    ...
    if len(neg_real) != 1:
        raise AssertionError(
            f"Expected cubic to have exactly one negative real root, found: {neg_real} "
            f"(roots={roots})"
        )
    y_LY = neg_real[0]
    if abs(y_LY - (-1.13445)) > 5e-4:
        raise AssertionError(f"y_LY is outside the expected range: got {y_LY}, expected about -1.13445")

    # q=2 fingerprint
    P2 = lam**3 - 2 * lam**2 - 2 * lam + 2
    disc2 = int(sp.discriminant(P2, lam))
    assert_equal(disc2, 148, "Disc(P2) should be 148.")
    sf = _squarefree_part(disc2)
    assert_equal(sf, 37, "Squarefree part of Disc(P2) should be 37.")
