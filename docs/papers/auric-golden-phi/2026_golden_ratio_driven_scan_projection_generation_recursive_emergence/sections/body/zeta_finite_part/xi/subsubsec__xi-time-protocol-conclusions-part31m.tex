\begin{conclusion}[\texorpdfstring{$\Omega_m(q)$}{Omega\_m(q)} 的表示论刻画：\texorpdfstring{$\mathrm{Sym}^q(M^m)$}{Sym^q(M^m)} 的迹与 Lucas--Chebyshev 多项式性]\label{con:xi-replica-softcore-exceptional-symq-evaluation-coupling}
令
\[
...
\]
因此 \(\Omega_m(q)=h_q(\alpha,\beta)\)（二元完全齐次对称多项式），并满足
\[
