...
\noindent
由此，bin-fold 纤维群胚代数上的三类有限维尺度已被明确分离：Morita--\(K\) 理论只保留 Fibonacci 块数，PI 理论只读取最大纤维，而最小忠实表示维数则精确回到全部 \(2^m\) 个微态；与此同时，六倍窗零余类又把碰撞与 KL 两类经典信息缺口严格锁定在 \(\varphi^{-m/2}\) 的黄金半尺度上。因而，退化谱直方图不再只是静态组合摘要，而是同时支配表示尺度、Morita 类型、PI 阈值与经典信息缺口的统一刚性对象。
