...
- 2026-01-16 — **CLAIMED**: `H3-3d` 扩展 raw-read Ribo-seq pausing 复现到 ≥4 independent studies（优先 GSE211536 + 其他 1–2 个 human Ribo-seq series；CPU 优先；如需 GPU 仅用 A40），并在 `sections/generated/riboseq_pause_bam_window.tex` 里追加 track + 更新随机效应 meta，写回 paper 与本分析文档。Branch: `paper-bio`.
  ...
  - Result (2026-01-16): 新增 2 个 raw-read tracks 并更新片段 `sections/generated/riboseq_pause_bam_window.tex`：`SRP627184_SRR35611920_full`（full 9.08M spots）显示显著分位数分层效应（high $\Delta U$ vs low：$d=0.81$, $p=0.008$, $n_{high}/n_{low}=23/27$；且 $\\rho(PI,\\Delta U)=0.231$，$n=90$, $p=0.029$）；`ERP150947_ERR12549926_10m`（10M spots）覆盖不足（$n_{corr}=2$，无法报告相关/效应量，不纳入 meta）。当前可用 3-study 随机效应 meta：$d=0.39$ [-0.11, 0.88], $I^2=40.1\\%$（仍不显著但方向转正）；下一步需要再补 ≥1–2 studies（优先 $n_{corr}\\gg 50$）以收敛 CI 并排查异质性来源。
