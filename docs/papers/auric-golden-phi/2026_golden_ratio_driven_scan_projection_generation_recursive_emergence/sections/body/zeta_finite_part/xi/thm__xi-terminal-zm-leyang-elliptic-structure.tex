...
对公开文献与数据库索引的可检索范围，使用 \(\Pi(\lambda,y)\)、\(y=x^2+u\)（或 \(y=x^2+Y\)）以及 \(256y^3+411y^2+165y+32\) 等关键表达式作定位检索，未见将本文所用椭圆化同构及其分歧--单值群结构作为明确命题表述的条目；可直接检得者主要为同源类 \(37\mathrm a\)（强 Weil 曲线 \(37\mathrm a1\)）的标准不变量与最小模型数据。\cite{LMFDBEllipticCurve37a1}
该核验仅提供“可检索范围内未见同构”的否证接口，并不构成对未收录文献的穷尽性排除。
\end{remark}
