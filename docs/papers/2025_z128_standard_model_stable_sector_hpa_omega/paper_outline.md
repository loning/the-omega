...
- **两原语不变**：tick（Axiom~readout sequentiality）与 CAP（bounded-complexity closure）仍是唯一原始输入；Wish/Motive 只能作为接口层对象或闭合目标函数，不得被当作新增公理。
- **层级不变**：保持 \MathTag/\InterfaceTag/\MatchTag/\AuditTag 的语义边界，并采用与其他论文一致的层纪律：
  - **closed-theory / closed layer**：只允许显式定义、有限构造、可审计的定理链（对应 [Math]/[Prot]/[CAP] 的闭合内容）。
  - **interface layer**：把 closed invariants 连接到可操作物理量/实验代理的最小字典（对应 [Iface]/[Match]）。
  - **interpretation / programmatic layer**：叙事映射与模型建议（对应 [Prog]），不得作为 closed 证明链前提。
...
- **审计不变**：`sections/appendices/23_audit_overview.tex` 的契约与 `sections/appendices/11_inference_ledger.tex` 的状态台账必须继续成立；任何新叙事段落应能回指到台账条目。
- **生成物不手改**：`sections/generated/*.tex` 为脚本输出；脚本为真源；重构只允许改“引用/组织”，不允许改生成文件内容。
- **标签不破坏**：尽量不改 `\label{...}`；若必须移动段落，优先保持 label 名称并检查交叉引用。
- **依赖关系不反转**：主文重排可以改变“阅读顺序”，但不得改变 `fig:inference_map` 的依赖箭头语义（solid/dashed/dotted 的含义不变）。
...
- **[Math]**：有限构造/计数/映射的定理级结果（folding core、计数、表格的数学事实）。
- **[Prot]**：可执行的有限协议构造（离散连接/holonomy、重建管线等）。
- **[Iface]**：物理识别/语义字典（物理对象=等价类；能量/质量=频率字典；延迟/钟慢字典等）。
- **[CAP]**：有限候选族上的确定性闭合（含 objective 与 tie-break）。
- **[Match]**：外部 PDG/CODATA/单位/方案/阈值为比较输入，不可当作前提。
- **[Prog]**：programmatic/interpretation 层的映射语言（例如“teleology”“生命/观察者叙事”“Wish 更新”）；必须显式标注为 not used in proofs。
- **[Open]**：明确未闭合项（如跨观测 $\gamma$ 一致性等）。

### 1.2 “Wish→Motive→CAP”的位置（不增新公理的写法）

- **Wish（Iface；更学术表述：protocol-stable target datum/structure）**：协议稳定目标对象（不是心理学），在本论文中建议具体化为“可重复、可局域、可跨点一致”的稳定读出类型与周期不变量集合；对象是“数据结构/不变量族”，而非单一数值。
- **Motive（Iface+CAP）**：由 Wish 诱导的审计目标函数族：误差证书（mismatch）+ 实现代价（overhead/Comp）+（可选）熵/不确定性项；CAP 即在显式有限候选族上对该 Motive 的确定性最小化。
- **保持不变点**：tick 与 CAP 仍是原语；Wish/Motive 只在接口层“解释” CAP 选择序列为何与周期三通道一致，不作为数学层前提。
...
建议把下表作为未来重构时的“连通性检查单”：每一行对应主链一条实线边，必须落到某个 RB-* 的证书形态（否则该边只能画成虚线叙事边）。

| 主链跳跃（实线） | 目标闭合输出 | 证书形态（RB） | 最小输入核（第一性 + 标准事实） | 必读参考（先对齐再写） |
|---|---|---|---|---|
| Tick $\to$ CAP | 可审计的确定性闭合算子 | RB-A | tick + 有界复杂度 + tie-break | `2025_protocol_stable_period_data_computational_teleology/sections/05_selection_principle.tex` |
| Tick $\to$ Abel 核/指数半群 | “记忆无关权重 $\Rightarrow$ 指数核”与模态语言的统一 | RB-C | tick 的单向半群 + Abel-first 纪律 | `2025_holographic_hilbert_universe_hpa_omega/sections/appendices/03_abel_finite_part_notes.tex`；`2025_riemann_ground_state_hpa_omega/sections/05_trace_formula_rigidity.tex` |
| CAP $\to$ golden branch | bounded-type/最难逼近扫描的选择刚性 | RB-A/RB-D | discrepancy 证书 + 有限复杂度代理 | `2025_motive_at_infinity_holographic_scanning_principle/sections/07_selection_principle.tex` |
| golden branch $\to$ $\varphi$-grammar | admissible set $X_m$ 与 Fold$_m$ 的总论化 | RB-C | Fibonacci/Zeckendorf 标准事实 | `2025_resolution_folding_phi_pi_e_hpa_omega/sections/06_resolution_folding_map.tex` |
| $\varphi$-grammar $\to$ $\pi$-closure | cyclic closure 与 $18\oplus 3$ 的刚性分解 | RB-C | wrap-around 约束 + adjacency/trace 标准事实 | `2025_resolution_folding_phi_pi_e_hpa_omega/sections/04_pi_constraint_discrete_monodromy.tex` |
| $\varphi,\pi$ $\to$ $\mathrm{e}$-stability | pole barrier / Abel 域稳定模板 | RB-B/RB-D | 归一化变量（$r$）+ Abel-first/finite part 纪律（作为协议规则而非新公理） | `2025_resolution_folding_phi_pi_e_hpa_omega/sections/05_e_constraint_abel_zeta_pole_barrier.tex`；`2025_holographic_hilbert_universe_hpa_omega/sections/appendices/03_abel_finite_part_notes.tex` |
| $\pi$-closure $\to$ anchor | 最小非平凡闭合/最小可审计锚点 | RB-A/RB-C | 有限候选族 + 最小性/唯一性证书 | `2025_physical_constants_geometry_hpa_omega/sections/05_alpha_anchor.tex`（“anchored worked example + gap”写法模板） |
| anchor $\to$ gauge/holonomy | 连接/holonomy 作为补偿数据（forced-by-mismatch） | RB-C/RB-D | 局部一致性/运输补偿的最小结构 | `2025_physical_constants_geometry_hpa_omega/sections/appendices/05_holonomy.tex` |
| gauge $\to$ SM labeling/mass | 有限候选族上的标号闭合与刚性深度/延迟字典 | RB-A/RB-D | 有限搜索 + gap-stability + 残差传播界 | `2025_physical_constants_geometry_hpa_omega/sections/06_running_couplings.tex`；`2025_physical_constants_geometry_hpa_omega/sections/07_masses_mixing.tex` |
| mismatch/overhead $\to$ gravity/dynamics | 以可审计开销场导出 lapse/potential 模板 | RB-B/RB-D | compilation overhead 定义 + 误差预算 + Poisson/Dirichlet 标准事实 | `2025_holographic_hilbert_universe_hpa_omega/sections/07_computational_lapse_gravity.tex`；`2025_holographic_hilbert_universe_hpa_omega/sections/08_minimal_discrepancy_dynamics.tex` |
...
- **Abel 轨道计算 / finite part / resolvent-视角（加强 $\mathrm{e}$ 通道与解析稳定）**
  - 参考：`docs/papers/2025_holographic_hilbert_universe_hpa_omega/sections/appendices/03_abel_finite_part_notes.tex`
  - 参考：`docs/papers/2025_riemann_ground_state_hpa_omega/sections/appendices/03_orbit_calculus_abel_fp.tex`
  - 价值：提供统一的 Abel-first/finite-part 纪律与“单位圆盘解析性/极点障碍”语言，可与 z128 的 zeta/Abel 稳定模板对齐并增强其严密性。
- **HTF / trace-formula rigidity 框架（更硬的“极点障碍→谱约束”模板）**
  - 参考：`docs/papers/2025_riemann_ground_state_hpa_omega/sections/05_trace_formula_rigidity.tex`
  - 参考：`docs/papers/2025_holographic_hilbert_universe_hpa_omega/sections/09_trace_formula_rigidity.tex`
  - 价值：把“bounded readout $\Rightarrow$ 单位圆盘全纯”与“谱侧 resolvent 模态 $\Rightarrow$ 内点极点”组织成可复用的刚性论证范式；对 z128 来说可作为更一般的 $\mathrm{e}$-通道解析闭合模板（不必引入 RH 主题本身）。
- **专门的 $\varphi$-$\pi$-$\mathrm{e}$ 分辨率折叠总论文（fold map、像/原像结构、退化度）**
  - 参考：`docs/papers/2025_resolution_folding_phi_pi_e_hpa_omega/sections/06_resolution_folding_map.tex`
  - 参考：`docs/papers/2025_resolution_folding_phi_pi_e_hpa_omega/sections/05_e_constraint_abel_zeta_pole_barrier.tex`
  - 价值：提供更系统的 Fold$_m$ 定义与原像结构、退化度增长、以及更清晰的 $\mathrm{e}$-通道“极点障碍”解释；可用来把 z128 的 folding core 与 uplift/高 $m$ 结构写得更“总论化”。
- **Hecke prime skeleton（把“跨尺度离散闭合”写成可检查约束族）**
  - 参考：`docs/papers/2025_ramanujan_holographic_scanning_principle_hpa_omega/sections/06_hecke_dynamics_prime_skeleton.tex`
  - 参考：`docs/papers/2025_stairway_to_infinity_holographic_renormalization_flow/sections/06_hecke_dynamics_prime_skeleton.tex`
  - 价值：提供一个“素数索引一致性约束族”的可审计模板；若 z128 希望把 RG/阈值跃迁写成更硬的“离散骨架闭合”，可借鉴其结构形式。
- **modular flow / Gauss-map renormalization（跨尺度的动力学母体）**
  - 参考：`docs/papers/2025_ramanujan_holographic_scanning_principle_hpa_omega/sections/03_modular_curve_stage.tex`
  - 参考：`docs/papers/2025_ramanujan_holographic_scanning_principle_hpa_omega/sections/04_scanning_as_modular_flow.tex`
  - 参考：`docs/papers/2025_stairway_to_infinity_holographic_renormalization_flow/sections/04_modular_flow_renormalization.tex`
  - 价值：若 z128 要把 “RG in $r$” 的来源进一步数学化，这套“母空间+流+符号编码”的结构可作为外部但很硬的可迁移模板。
- **Morita 等价 / Fourier exchange（等价语义的更强版本；对偶与尺度交换）**
  - 参考：`docs/papers/2025_stairway_to_infinity_holographic_renormalization_flow/sections/08_S_duality_morita_equivalence.tex`
  - 价值：可作为 `equivalence_semantics` 的增强版背景：把某些“协议等价/基变换/对偶”提升到范畴与 Morita 等价层级；是否引入取决于 z128 是否需要强对偶叙事。
- **CAP-II 的正则化注记（统一 regularization 纪律）**
  - 参考：`docs/papers/2025_computational_action_principle_ii_dynamics_hpa_omega/sections/appendices/03_regularization_notes.tex`
  - 价值：把正则化/有限部分/极限路径作为可审计对象统一描述，可与 z128 的 delay/lapse/Abel 模块互相对齐。
...
- **A.0 Reader-facing contract（Iface+Audit）**
  - 目的：把 `audit_overview` 的契约与 `inference_ledger` 的状态标签前置到主文，成为阅读协议。
  - 输出：读者知道“哪些是 [Math]，哪些是 [Iface]/[CAP]，哪些是 [Match]”。
  - **开工前必读（避免不一致）**：
...
- **A.3 三通道作为 Wish 可实现性的最小分解（Iface）**
  - 预告：$\varphi$（语法/增长率）、$\pi$（闭合/回路/拓扑一致性）、$\mathrm{e}$（解析稳定/极限/半群单向性）。
- **A.4 Motive 的数学模板：目标函数、闭合、与耗散单调性（Math；不作为 folding 前提）**
...
- **C.3 $\mathrm{e}$ 通道：Abel--zeta 解析稳定、指数半群、与时间箭头（Math→Iface）**
  ...
  - **接口点（Iface；不进入 theorem-level folding 依赖链）**：
    ...
    - **积分常数/初值作为匹配输入**：$C\,\exp(\lambda t)$ 中的 $C$ 对应时间零点/尺度的校准选择；在审计上应作为 [Match] 或协议约定出现，而非闭合链内生推出。
  - **开工前必读（pole barrier / Abel-first 纪律）**：
...
- **D.2 规范作为补偿（Prot→Iface）**
  ...
  - **开工前必读（holonomy 模板与“需要刚性才可预测”的提醒）**：
    - `docs/papers/2025_physical_constants_geometry_hpa_omega/sections/appendices/05_holonomy.tex`
- **D.3 手性/反物质/CPT 的协议几何（Prot→Iface）**
...
- **E.1 SM labeling closure（Iface+CAP）**
...
- **E.3 Couplings / CP / mixing closures（Iface+CAP+Match）**
  ...
  - **开工前必读（running/mixing 的对齐；避免不同论文用不同 $r(\mu)$ 约定）**：
    - `docs/papers/2025_physical_constants_geometry_hpa_omega/sections/06_running_couplings.tex`
    - `docs/papers/2025_physical_constants_geometry_hpa_omega/sections/07_masses_mixing.tex`

### Part F — Dynamics: continuum representatives, free energy, RG, cosmology

- **F.0 时间箭头（闭合版）：指数半群、遗忘常数、与单调性证书（Math+Iface）**
  ...
  - **单调性证书（Iface；作为时间箭头的可审计后果）**：
    ...
    - **遗忘常数**：指数解的积分常数等价于时间原点/尺度的校准；在审计与比较中应被显式记录为协议约定或 [Match] 输入，而非当作闭合推理结果。
  - **开工前必读（时间箭头模板的对齐）**：
    - `docs/papers/2025_holographic_hilbert_universe_hpa_omega/sections/08_minimal_discrepancy_dynamics.tex`
    - `docs/papers/2025_holographic_phase_thermodynamics_hpa_omega/sections/03_asm.tex`
    - `docs/papers/2025_riemann_ground_state_hpa_omega/sections/05_trace_formula_rigidity.tex`

- **F.1 等价语义与频率优先字典（Iface）**
  - 内容：物理对象=等价类；可观测=不变泛函；频率作为派生首量。
  - **开工前必读（等价/对偶与 scan-readout 交换模板）**：
    - `docs/papers/2025_stairway_to_infinity_holographic_renormalization_flow/sections/08_S_duality_morita_equivalence.tex`
- **F.2 CAP 闭合作用量与场方程（CAP→Math）**
  - 内容：最小作用量骨架与变分 EOM（Einstein/YM/chi）。
  - **开工前必读（正则化与极限路径可审计化）**：
    - `docs/papers/2025_computational_action_principle_ii_dynamics_hpa_omega/sections/appendices/03_regularization_notes.tex`
- **F.3 热力学闭合：CAP 作为自由能原则（Iface+CAP）**
  - 内容：$\mathcal{F}=E-TS$ 与 CAP objective 的统一形式；力作为响应。
  - **开工前必读（ASM/phase friction；热力学结构对齐）**：
    - `docs/papers/2025_holographic_phase_thermodynamics_hpa_omega/sections/03_asm.tex`
- **F.4 overhead→gravity 与 $\chi$ 重建（Iface+Prot）**
  - 内容：$\chi\to N\to g_{00}\to\Phi$ 与误差控制；为跨观测 $\gamma$ 一致性准备统一接口。
  - **开工前必读（lapse/overhead/Poisson closure 与 time-delay 观测代理）**：
    - `docs/papers/2025_holographic_hilbert_universe_hpa_omega/sections/07_computational_lapse_gravity.tex`
    - `docs/papers/2025_holographic_hilbert_universe_hpa_omega/sections/08_minimal_discrepancy_dynamics.tex`
    - `docs/papers/2025_computational_action_principle_ii_dynamics_hpa_omega/sections/09_scattering_wigner_smith.tex`
- **F.5 量子测量与 Born（Iface）**
  - 内容：POVM/仪器与 Born 规则闭合路线。
  - **开工前必读（POVM/instrument 的最小接口写法对齐）**：
...
- **F.6 RG 与宇宙学（Iface）**
  ...
  - **开工前必读（跨尺度母体与离散骨架）**：
    ...
    - `docs/papers/2025_ramanujan_holographic_scanning_principle_hpa_omega/sections/06_hecke_dynamics_prime_skeleton.tex`

### Part G — Validation and falsifiability (and open closures)

- **G.1 可证伪命题与协议化实验通道（Iface+Audit）**
  - 内容：P1–P6 作为主文的收束，不作为推理前提。
  - **开工前必读（可证伪写法与误差预算的组织方式）**：
    - `docs/papers/2025_physical_constants_geometry_hpa_omega/sections/10_falsifiability.tex`
- **G.2 鲁棒性/反事实族/局限性（Audit+Open）**
  - 内容：counterfactual sweeps、look-elsewhere、family sensitivity。
  - **开工前必读（gap-stability 与鲁棒性模板）**：
    - `docs/papers/2025_motive_at_infinity_holographic_scanning_principle/sections/07_selection_principle.tex`
- **G.3 下一步闭合（Open）**
...
- **H.1 观察者作为自读写头（Iface）**
- **H.2 主动重整化（colliders as forced zoom）（Iface）**
- **H.3 Wish 更新算子（Iface+Open）**
  - 给出最小形式：结构出现→可观测/可控协议族改变→Wish 约束域改变→新的 CAP 闭合序列。
  - **开工前必读（teleology 的接口写法）**：
    - `docs/papers/biology/2025_biological_computational_teleology_hpa_omega/sections/09_evolution_teleology.tex`
...
- **Wish 的数学化（定义闭合）**
  ...
  - **开工前必读**：`docs/papers/2025_protocol_stable_period_data_computational_teleology/sections/03_wish_protocol_stable_period_data.tex`
- **Motive 的数学化（functional 闭合）**
  ...
  - **开工前必读**：`docs/papers/2025_protocol_stable_period_data_computational_teleology/sections/05_selection_principle.tex`
- **Teleological dynamics 的数学化（不依赖具体物理）**
  ...
  - **开工前必读**：`docs/papers/2025_protocol_stable_period_data_computational_teleology/sections/06_variational_dynamics.tex`
- **黄金分支的稳定性证书闭合（从直觉升级为 bound）**
  ...
  - **开工前必读**：`docs/papers/2025_motive_at_infinity_holographic_scanning_principle/sections/07_selection_principle.tex`
- **“刚性=gap-stability”模板（鲁棒性数学化）**
  - **目标**：把“唯一极小化 + 二名差距”写成一般引理，用于解释各类 rigidity certificate 的抗扰动性（不需要新的物理输入）。
  - **落点**：Part G.2 或作为 CAP 审计模板的补充 lemma。
  - **状态**：[Math]。
  - **开工前必读**：`docs/papers/2025_motive_at_infinity_holographic_scanning_principle/sections/07_selection_principle.tex`
- **Abel-first / finite part / resolvent 纪律（解析闭合的通用底座）**
...
- **指数半群模板（记忆无关权重；时间箭头的最小数学核）**
  ...
  - **状态**：[Math]（functional equation/差分方程的标准结论）+ [Audit]（把初值/校准常数标注为协议约定或 [Match]）。
  - **开工前必读（对齐符号与模态语言）**：
    - `docs/papers/2025_riemann_ground_state_hpa_omega/sections/05_trace_formula_rigidity.tex`
    - `docs/papers/2025_holographic_hilbert_universe_hpa_omega/sections/appendices/03_abel_finite_part_notes.tex`
- **Trace-formula pole barrier 模板（“全纯域”与“内点极点”不相容）**
...
- **Fold$_m$ 的总论化（像/原像、退化度、窗口族一致性）**
  ...
  - **状态**：[Math]（定义/命题/定理）。
  - **开工前必读**：`docs/papers/2025_resolution_folding_phi_pi_e_hpa_omega/sections/06_resolution_folding_map.tex`
- **Hecke prime skeleton（一致性约束族的可审计模板）**
  - **目标**：把“素数索引一致性约束族 + 残差传播界”作为通用审计模板，供 RG/跨尺度一致性引用。
  - **落点**：主文 F.6 或其附录支撑；只要采用 prime-skeleton 叙事，就必须对齐该模板。
  - **状态**：[Math]（算子/残差界）+ [Audit]（可检验流程）。
  - **开工前必读**：
    - `docs/papers/2025_ramanujan_holographic_scanning_principle_hpa_omega/sections/06_hecke_dynamics_prime_skeleton.tex`
    - `docs/papers/2025_stairway_to_infinity_holographic_renormalization_flow/sections/06_hecke_dynamics_prime_skeleton.tex`
- **Morita 等价与 Fourier exchange（等价语义的硬底座）**
  ...
  - **状态**：[Math]（定理/命题模板）。
  - **开工前必读**：`docs/papers/2025_stairway_to_infinity_holographic_renormalization_flow/sections/08_S_duality_morita_equivalence.tex`
- **正则化注记（统一 regularization 路径的审计语义）**
  - **目标**：把“极限顺序/正则化路径/finite part 规则”写成可审计对象，形成全篇统一纪律。
  - **落点**：建议在 A.0 的契约中声明，并在涉及极限/连续代表的章节统一引用。
  - **状态**：[Audit]（契约）+ [Math]（可用的注记/lemma）。
  - **开工前必读**：`docs/papers/2025_computational_action_principle_ii_dynamics_hpa_omega/sections/appendices/03_regularization_notes.tex`
...
- **(III) programmatic / interpretation（not used in proofs）**
  ...
  - 时间箭头的直觉叙事：用“指数半群/指数模态是变化算子的本征态”来解释单向性与“起点信息只以校准常数残留”的现象；必须标注 not used in proofs
