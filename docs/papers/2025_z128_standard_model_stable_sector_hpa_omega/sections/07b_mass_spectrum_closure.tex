...
\]
and in a one-channel setting $S(\omega)=\e^{\iu\delta(\omega)}$ one has $\tau_{\mathrm{WS}}(\omega)=\dd\delta/\dd\omega$ (Appendix~\ref{app:time_mass_delay}).
...
Throughout this paper this dictionary is used only at the matching layer: it complements the Compton-clock relation below and provides an independent falsifiability route via delay-derived lapse ratios (Section~\ref{subsec:p6_wigner_smith_delay} and Appendix~\ref{app:time_mass_delay}).
...
\end{equation}
where $w_f\in X_6$ satisfies $\mathcal{L}_{\mathrm{SM}}(w_f)=f$, $\kappa:=m/n=2$ for the minimal balanced coupling $(m,n)=(6,3)$ on the chosen $2$D Hilbert screen, $|w|_1$ is Hamming weight, and $g(w)$ is folding degeneracy (Definition~\ref{def:x6_invariants}).
...
\midrule
\input{sections/generated/mass_depth_leave_one_out_summary_rows.tex}%
\end{tabular}
