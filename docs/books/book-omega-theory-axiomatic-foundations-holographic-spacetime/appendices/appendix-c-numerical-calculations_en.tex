\section{Numerical Calculation of \texorpdfstring{$\mu$ and $\alpha$}{mu and alpha}}

\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{assets/appendix-c-numerical-geometry.png}
\caption{Numerical Geometry}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{assets/appendix-c-precision-scale-balance.png}
\caption{Precision Scale Balance}
\end{figure}

\subsection{Geometric Factor of Proton-Electron Mass Ratio}
...
\subsubsection{High-Dimensional Sphere Volume Formula}
...
\begin{itemize}
\item For $S^1$ (circle): $\text{Vol}(S^1) = 2\pi$
\item For $S^2$ (sphere): $\text{Vol}(S^2) = 4\pi$
\item For $S^3$ (hypersphere): $\text{Vol}(S^3) = 2\pi^2$
\item For $S^5$: $\text{Vol}(S^5) = \pi^3$
\end{itemize}
...
\subsubsection{Physical Correction Term Estimation}
...
\subsection{Holographic Expansion of Inverse Fine Structure Constant}
...
\subsubsection{Term-by-Term Numerical Contributions}

\begin{enumerate}
\item \textbf{Torus/Volume Term ($4\pi^3$)}:
...
\subsection{Numerical Simulation Algorithm for Constant Drift}
...
\subsubsection{Discretization of Evolution Equation}
...
\subsubsection{Python Simulation Code}
...
\begin{itemize}
\item At $z=1.0$, $\Delta \alpha / \alpha \approx -3.5 \text{ ppm}$
\item At $z=3.0$, $\Delta \alpha / \alpha \approx -6.9 \text{ ppm}$
\end{itemize}
