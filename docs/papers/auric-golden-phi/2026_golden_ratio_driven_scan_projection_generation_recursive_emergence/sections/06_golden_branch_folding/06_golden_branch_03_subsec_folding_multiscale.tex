...
这提示：大规模“降分辨率重写”并非仅由极端构造触发，在无结构输入下也会以正的密度铺满全串。更进一步，均匀基线下存在一个稳定的\emph{常数密度律}：在典型位置上，朴素截断位与折叠感知位的极限联合律收敛到一张完全有理的 $2\times 2$ 表，从而把线性增长封口为明确的比例常数（定理 \ref{thm:fold-gauge-anomaly-density}）。
\end{remark}
