...
\begin{theorem}[纤维内平衡外置对全部凸审计泛函的普适最优性]\label{thm:conclusion-balanced-externalization-schur-optimality}
固定分辨率 \(m\) 与外置位数 \(B\)，并记
\[
M:=2^B,\qquad
\Omega_m:=\{0,1\}^m,
...
\]
并记 \(\mathbf b(d,M)\) 为由 \(r\) 个 \(a+1\) 与 \(M-r\) 个 \(a\) 组成的平衡向量，则对任意凸函数
\[
\Phi:[0,\infty)\to\RR
\]
都有
\[
\sum_{x\in X_m}\sum_{r\in\{0,1\}^B}\Phi\!\bigl(n_{x,r}(R)\bigr)
\ge
\sum_{x\in X_m}\sum_{j=1}^{M}\Phi\!\bigl(b_j(d_m(x),M)\bigr).
\]
...
\sum_{j=1}^{M}n_j=d
\]
的占用向量中，\(\mathbf b(d,M)\) 是 majorization 意义下的极小元；等价地，任意这类向量都 majorizes \(\mathbf b(d,M)\)。由 Hardy--Littlewood--P\'olya--Karamata 理论，对任意凸函数 \(\Phi\) 都有
\[
\sum_{j=1}^{M}\Phi(n_j)\ge \sum_{j=1}^{M}\Phi\!\bigl(b_j(d,M)\bigr).
\]
...
\Phi(t)=t^q
\]
即可。由于 \(q\ge 1\) 时 \(t\mapsto t^q\) 为凸函数，而平衡向量 \(\mathbf b(d_m(x),2^B)\) 的分量恰由 \(r_x\) 个 \(a_x+1\) 与 \(2^B-r_x\) 个 \(a_x\) 组成，故所示闭式立即成立。证毕。
\end{proof}
...
\paragraph{统一读法}
这组六条结果把 bin-fold 外置预算几何压缩为一条比连续热力学图像更刚性的离散相图：在固定窗口上，纤维内平衡外置同时最小化全部凸审计泛函与全部 \(q\)-碰撞能量；在临界尺度上，这一最优机制收敛为由两条谐波格
\[
\Bigl\{\frac1n\Bigr\}_{n\ge 1},
\qquad
\Bigl\{\frac{\varphi^{-1}}{n}\Bigr\}_{n\ge 1}
\]
支配的无穷折点阶梯；而在 Mellin 侧，整个容量剖面又被一支二原子测度完全唯一决定，并强制导出超临界平台与亚临界幂律爆破。于是，budgeted externalization、critical-scale 容量与二原子极限律三者不再只是平行出现的统计像，而被同一离散平衡机制锁定为一幅统一相图。
