...
def write_holonomy_tex(summary: Dict[str, Dict[str, float]], out_path: Path) -> None:
    lines: List[str] = []
    lines.append("% Auto-generated by scripts/exp_holonomy_spectrum.py")
    lines.append("\\begin{tabular}{lrrrr}")
    lines.append("\\toprule")
    lines.append("Class & $N$ & id-rate & fixed-ratio(avg) & $\\mathbb{E}[|b|]$\\\\")
    lines.append("\\midrule")
    for cls in ["bulk", "interface", "all"]:
        s = summary.get(cls, {})
        lines.append(
            f"{cls} & {int(s.get('N', 0))} & {s.get('id_rate', 0.0):.4f} & {s.get('fixed_ratio_mean', 0.0):.4f} & {s.get('abs_b_mean', 0.0):.4f}\\\\"
        )
